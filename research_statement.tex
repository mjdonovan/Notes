...
\end{cor}
\section{Homotopical algebra for unstable Lie coalgebras}
We will soon be faced with the task of calculating $E^2_{-s,t}(X)=\Ext^s(\F_2[t],H_*^Q(X))$, the $E^2$ page of the Adams spectral sequence for $X$. Before attempting such a calculation one should be equipped with certain homotopical tools.

\subsection{Past work}
For simplicity, I restrict to the case where $X$ is connected. Then, omitting certain details, work of Goerss [] shows that $H:=H^Q_*X$ is an object in a certain comonadic category $\calC^1$ of Lie coalgebras which are compatibly coalgebras for the algebra of $P$-operations [\textbf{or something}]. It's in this category that we're calculating $\Ext$. Write $\calV^1$ for the category of graded vector spaces. Write $\textup{Pr}:\calC^1\to \calV^1$ for the functor whose value in dimension $t$ is $\Hom(\F_2[t],\DASH)$, so that $E^2_{-s,t}(X)=\mathbb{R}^s\textup{Pr}_t(X)$.

Talk about the function $\xi_\textup{alg}$. Creates a pairing on $E^2$, as well as Steenrod operations (without an Adem relation).
...
Demonstrate that the Steenrod operations satisfy the Adem relations. Don't have $\Sigma_4$-equivariance, but may not matter --- see Priddy.

Extend the delta which lives inside the derived functors to one on all of $E^2$, ABSTRACTLY! JUST FROM THE FACT THAT IT'S DERIVED FUNCTORS!
...
Now we'll use the canonical double complex, $E^0_{-s,t}=c(KQc)^{s+1}_t$, in which case we have $E^1_{-s,t}=\Hom(\F_2[t],C^{s+1}(H^Q_*(X)))$, where $C$ is the cofree comonad for the target category for Andre-Quillen homology $H^Q_*$.
