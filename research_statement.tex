...
In defining the notion of a model category, Daniel Quillen \cite{QuillenHomAlg.pdf} formulated the notion of homotopy theory in a variety of contexts. The context of primary interest in the statement is that of simplicial commutative rings. This advance allowed Quillen to define a notion of the (co)homology of algebras in a common framework with the (co)homology of topological spaces. The resulting theory is now known as Andr\'e-Quillen (co)homology. It is of inherent interest in algebraic topology, for example, it appears in the proofs of such interesting results as the Sullivan conjecture \cite{MillerSullivanConjecture.pdf}. It is of interest also to commutative algebraists, for example Michel Andr\'e, after whom the theory is also named for his development of a parallel theory of the simplicial resolution of rings.
...
My research has so far focused on the (unstable) Adams spectral sequence (ASS) for simplicial commutative rings. The topological ASS was introduced by Adams in 1958 \cite{MR0096219}, while the ASS for simplicial algebras was defined by Radelescu-Banu only in 2006 \cite{radelescuBanu.pdf}. Further study of the topological ASS was performed by Bousfield and Kan in a series \cite{BousKanSSeq.pdf,BK_pairings.pdf,BK_pairings_products.pdf} of papers. One part of my research has been to produce analogous results in the context of simplicial commutative rings: completion and convergence results, along with spectral sequence operations (when the base ring is $\F_2$).

The May spectral sequence \cite{MayRestLie.pdf} is a further spectral sequence converging to the $E^2$ page of the ASS. A further component of my research has been to develop a composite functor spectral sequence with target the $E^2$ page of the ASS for a simplicial algebra. In fact, I develop a sequence of composite functor spectral sequences, each converging to the $E^2$ page of the previous. Using these tools, we can produce effective upper bounds on the size of the Adams $E^2$ page, including a vanishing line result.

Now the ASS for topological spaces is notoriously difficult to compute, say, with input a sphere, and the homotopy groups of a topological sphere are notoriously difficult to calculate. On the other hand, the homotopy groups of a simplicial algebra sphere are known \cite{DwyerHtpyOpsSimpComAlg.pdf}, and one might expect the ASS to be simple in this context. Calculations I have made of the $E^2$-page show that this is quite untrue, and the spectral sequence is very far from degenerate. 

The tools and techniques I have developed appear to put within reach the complete calculation of the ASS for a simplicial $\F_2$-algebra sphere. I have made certain computer calculations which have led to a system of conjectured differentials, which if correct, would be all of the differentials in the spectral sequence. The work on verifying that these differentials are correct is ongoing, and must surely depend on understanding the various operations which can be defined on the composite functor spectral sequences I have developed.
