...
In \cite{radelescuBanu.pdf}, Radelescu-Banu shows how to form a homotopically correct Adams resolution for a simplicial commutative algebra $X$. There results a homotopical functor $X\mapsto \hat X$, \emph{completion with respect to abelianization}, and an Adams spectral sequence converging to the homotopy of the completion $\pi_*\hat X$. \textbf{Let's let $\frakX$ be the resolution of $X$...\ blah blah blah.}
...
\subsection{Past work}\label{PastWorkOnE2LevelStructure}
...
I would also like to find a commutation relation between the $\Sq^i$ and the $\delta_i$.
...
\end{prop}
Here, $\overline{D}^1\frakX$ is a version of the $D^1$-construction modified using our knowledge of the structure of $\frakX$. In particular:
...
\item Steenrod operations $\underline{\Sq}^i:E^r_{-s,t}\to E^r_{-s-i-1,2t+1}$; and
\item `$\delta$-operations' $\underline{\delta}_i:E^r_{-s,t}\to E^r_{-s-1,t+i+1}$
\end{enumerate}
\end{cor}
These symbols representing these operations have been underlined, as it's not clear at present what relations they satisfy. However:
\begin{thm}
...
so that the pairing of \ref{liftedOperations} coincides with the pairing on $E^2$ described in \S\ref{PastWorkOnE2LevelStructure}, and the Steenrod operation $\underline{\Sq}^i$ equals the operation $\Sq^{i+1}$ [\textbf{?????}] of \S\ref{PastWorkOnE2LevelStructure}.
\end{thm}
...
\end{prop}
At this point, we may apply the machinery of Blanc and Stover, get some operations from Singer, lift them using the structure I derived on the Stover $W$ comonad and hope for the best.
