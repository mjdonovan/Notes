...
E_{i}^{r}:=&\left.\pi^{-1}(Z_{i}^{r-1})/\beta(B_{i}^{r-1}).\right.
\end{alignat*}
...
certificate $c\in\pi^{-1}(Z_i^{r-1})$, and maps it to $M_i(c)\in P_{i-1}$, pulls
...
homology to get from $E_*^r=\pi^{-1}(Z_{i}^{r-1})/\beta(B_{i}^{r-1})$ to
$E_*^{r+1}=\pi^{-1}(Z_{i}^{r})/\beta(B_{i}^{r})$ has the effect of
...
What's interesting here is that the top row (and thus the abutment) is split
