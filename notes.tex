...
case they must have \emph{died} during the $(i-1)^\text{st}$ period. Similarly,
a person $p\in P_i$ need not be in the image of $T_{i}$, in which case they must
have been \emph{born} during the $i^\text{th}$ period.

We will assume that only the zero person $p\in P_i$ is in the image of
$T_{j}\cdots T_i$ for all $j\leq i$. That is, every nonzero person must have
been born some finite time ago. However, we will not rule out the possibility
that people live for ever.

Now $C_*$ is the graded abelian database consisting of birth and death
\emph{certificates}. If a person $p\in P_{i}$ is born during the
$(i-1)^\text{st}$ year, they report this to the government by submitting a
nonzero birth cirtificate $B_{i}(p)$. Any $p\in P_{i}$ who was not born in that
year submits the zero birth certificate.
...
at the upper left, as the people who die during the $(i-1)\first$ period are
exactly those who received death certificates. It's exact at the upper right, as
those people $p$ who were already alive at time $(i-1)$ are exactly those who
submit the zero birth certificate. It's exact at the bottom, as the database
does not contain any redundant certificates, and of course, all birth
cirtificates map to zero under the `death certificate' map.

In particular, we have an exact couple, consisting of the straight arrows in the
following:
\[\xymatrix@!0@C=1.1cm@R=1.28cm{
\makebox[0cm][r]{$\cdots$\ \ }P_{i-1}\ar[rrrr]^{T_{i  }}&&&&\ar[dll]_{B_{i  }}
\ar@{..>>}[ddl]_{b_{i  }}
P_{i  }\ar[rrrr]^{T_{i+1}}&&&&\ar[dll]_{B_{i+1}}
\ar@{..>>}[ddl]_{b_{i+1}}
P_{i+1}\ar[rrrr]^{T_{i+2}}&&&&\ar[dll]_{B_{i+2}}
\ar@{..>>}[ddl]_{b_{i+2}}
...
quotient group $\coim(M_{i}):=C_i/\ker(M_i)$ consists of the death certificates.
Moreover, the birth and death certificate maps factorise: $B_i=\beta b_i$ and
$M_i=m_i\pi$.
\begin{alignat*}{2}
...
=&\left\{\text{birth certificates $b_i(p)$ for people who will die in the next
$r$ periods}\right\}.\\
E_{pq}^{r+1}:=&\left.\pi^{-1}(Z_{pq}^r)/\beta(B_{pq}^r).\right.
\end{alignat*}
%\footnotetext{Of course, zero is alwa}
We note that under these definitions,
\begin{alignat*}{2}
Z_{i}^\infty
=&\left\{\text{death certificates $x$ for people who have been alive
 forever}\right\}=0.\\
B_{i}^\infty=&\left\{\text{birth certificates $b_i(p)$ for people who are not
 immortal}\right\}.\\
...
consisting of those immortals born during the $i^\text{th}$ period is isomorphic
to the quotient:
\[\left\{\text{all birth certificates issued at time
 $i$}\right\}/\left\{\text{those issued to mortals}\right\}.\]

Now I claim that in passing from $E^{r-1}$ to $E^{r}$, we kill off all of the
mortals which lived for exactly $r$ years.\footnote{We say that a mortal who was
born at $P_i$ but first goes to zero in $P_{i+r+1}$ lived for $r$ years
exactly.} To be more precise, we discard both the birth and death certificates
