...
\Bullet \textbf{Certificates:} The government maintains a graded abelian database $C_*$. The group $C_i$ consists of birth and death certificates relevant to the time period from $(i-1)$ to $i$. More on this later.
...
\Bullet \textbf{Age:} We will say that somebody who was born on $P_j$ and who died at $P_i$ was ``seen at $i-j+1$ times''. This is how we measure age.
...
person $p\in P_i$ is in the image of $T_{j}\cdots T_i$ for all $j\leq i$. That
is, there are no (nonzero) \emph{co-immortals}: every nonzero person must have
been born some finite time ago. That is to say that the inverse limit of the
$P_i$ is zero.
...
\Bullet \textbf{A spectral sequence:} We can form a spectral sequence out of these exact triangles in the usual
way, whose terms are defined as follows:
\begin{alignat*}{2}
Z_{i}^r:=&\left\{x\in \coim(M_i)\ \middle|\ m_i(x)\in\im(P_{i-r}\to P_{i-1})\right\}\\
...
=&\left\{\text{death certificates $x$ for people $m_i(x)\in P_{i-1}$ seen at $r$ or more times}\right\}.\\
B_{i}^r:=&\left.b_i(\ker(P_i\to P_{i+r-1}))\right.\\
=&\left\{\text{birth certificates $b_i(p)$ for people $p\in P_i$ who will die at
or before $P_{i+r-2}$}\right\}.\\
=&\left\{\text{birth certificates $b_i(p)$ for people $p\in P_i$ seen at $r-1$ or fewer times}\right\}.\\
E_{i}^{r+1}:=&\left.\pi^{-1}(Z_{i}^r)/\beta(B_{i}^r).\right.
\end{alignat*}
Note also that in the quotient $\im(B_i)/B_i^r$, the birth certificates that remain are those of people who were seen at $r$ or more times.


\Bullet \textbf{Differentials:} The differential $d_r$ on $E^{r}$ takes a certificate $c\in\pi^{-1}(Z_i^r)$,
...
can be nonzero in $E_r$ is when $p$ was actually born on $P_{i-r}$. That is, when $p$ was seen at exactly $r$ times.

In this case, surveying the life of $p$, we see that their death certificate $c$
maps to their birth certificate $B_{i-r}(p)$ under $d_r$. Taking homology to get
from $E_*^r$ to $E_*^{r+1}$ has the effect of simultaneously annihilating both
the birth and the death certificate of every person seen at exactly $r$ times
simultaneously.
% does not die before the end of the $(i-2)^\text{nd}$ period, i.e.\ iff 

%\footnotetext{Of course, zero is alwa}
\Bullet \textbf{$E^\infty$ page:} We make the following obvious extensions of the above definitions, letting
$Z^\infty_i$ be the intersection $\cap_rZ^r_i$, and $B^\infty_i$ be the
union $\cup_rB^r_i$:
\begin{alignat*}{2}
Z_{i}^\infty
...
\end{alignat*}
Without further hypotheses, it need not that case that $E_i^r\cong E_i^\infty$ for $r\gg0$. However, this is a natural definition of the $E^\infty$ page, in the sense that $E^\infty_i$ can be recovered from knowledge of the differentials in and out of $E^r_i$ for each $i$.

\Bullet \textbf{Abutment: RESUME HERE} Now let $P=\varinjlim P_i$ be the direct limit of the population under the maps
...
Thus we can summarise the abutment of the spectral sequence by saying --- the
