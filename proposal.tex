...
An \emph{abelian object of $s\calA$} is an object $A\in s\calA$ along with a natural abelian group structure on $s\calA(\DASH,A)$. There is an evident category $\mathsf{ab}(s\calA)$ of abelian objects, and a left adjoint to the forgetful functor yielding an adjunction $s\calA\rightleftarrows\mathsf{ab}(s\calA)$. Quillen homology is then defined to be the left derived functors of \emph{abelianisation}, this left adjoint.
...
\begin{prop*}
When $H^*_Q(A)$ is of finite type (for example when $A$ is connected and of finite type) the spectral sequence has $E^2_{st}=(\mathbb{L}_s\textup{Ind}_\calW)(H^*A)^t$.
\end{prop*}
...
\footnote{An object of $\calL$ is to be a non-negatively graded \ensuremath{\F_2}-vector space $W^*$, with a Lie bracket ${W^n}\otimes{W^m}\to W^{n+m+1}$ satisfying the (non-redundant) axiom `$[x,x]=0$'.}
...
Before making sense of the above spectral sequence, we must understand $\calL'$, and the functor $\overline{\textup{Ind}_{[]}}$. For this we introduce the category $\uLLa$ of `unstable $\Lambda^+$-Lie algebras'. An object of this category is to be a graded Lie algebra $L_*$ over \ensuremath{\F_2} (with axiom $[[x,x]]=0$, and no $+1$ shift in grading), along with right operations $\lambda_i:L_n\to L_{n+i}$ for $1\leq i\leq n$ (written $x\mapsto x\lambda_i$), such that:
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
...
\footnote{This comes from an essentially formal Hilton-Milnor decomposition for the coproduct of a finite number of free Lie algebras, where by `Lie algebra' we mean `Lie algebra over $\ensuremath{\F_2}$ subject to the extra axiom $[x,x]=0$'.} for homotopy groups of wedges of spheres in $s\calL$. This, along with the description of the homotopy groups of the free unrestriced Lie algebra given in \cite{6Author.pdf} can be used to demonstrate that $\calL'$ is a category of $+1$-shifted graded objects in $\uLLa$ (thus bigraded \ensuremath{\F_2}-Lie algebras with one of the gradings yielding a $+1$ shift, etc.).
...
Here, the category $\calP$ is obtained from $\calW$ by forgetting the Lie structure. The lower $\textup{Ind}_\textbf{P$\beta$}$ actually looks a little like a $\Tor_\textbf{P$\beta$}$.

Anyway, the hope is that the Adem relations for $P$ operations are such that there is a Koszul calculation of derived $\textup{Ind}_\textbf{P$\beta$}$, and that the Koszul resolution can be enriched with a Lie bracket in such a way that the image in the bottom right category here can actually be lifted naturally back to $\calL$ as one might hope.
...
\begin{itemise}
\setlength{\parindent}{.25in}
\item a Kan-Thurston theorem;
\item a Haugseng-Miller-style $\Lambda$-algebra vs.\ Dyer-Lashov spectral sequence after developing some stable theory;
\cite{RuneMillerCohomLoopSpace.pdf}
\item a comparison with Quillen's spectral sequence;
\item comparison of a stable ASS with some stabilised Quillen spectral sequence;
\item realisation problems for $\calW$-$\Pi$-algebras;
\item realisation problems for $\frakS$-algebras;
\item A stronger Serre dichotomy, as in [Goe3] and [Goe4].
\end{itemise}
\printbibliography
