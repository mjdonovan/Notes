...
\begin{prop*}
When $H^*_Q(A)$ is of finite type (for example when $A$ is connected and of finite type) the spectral sequence has $E^2_{st}=(\mathbb{L}_s\textup{Ind}_\calW)(H_Q^*A)^t=(\Ext^s_{\calW}(H_Q^*A,H_Q^*S^t))^*$.
\end{prop*}
