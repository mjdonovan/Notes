...
\begin{prop}
The image of $f^0(g^1_{i_1}(h^2_{i_1i_2}(\cdots )))\in \Q_{\LL{}}[\Fr{\LL{}}]^{n+1}X$ under the map $j\circ Q(\xi_{\LL{}})$ is given by $q(u(f)(g_{i_1}))(h^1_{i_1i_2}(\cdots ))$.
\end{prop}
\begin{proof}
The element $f^0(g^1_{i_1}(h^2_{i_1i_2}(\cdots )))\in \Q_{\LL{}}[\Fr{\LL{}}]^{n+1}X$ equals the element $u(f)(g^1_{i_1}(h^2_{i_1i_2}(\cdots )))$, since we are calculating in the indecomposables. Thus, we may assume that $f$ is just the identity, so that there is only one index $i_1$, and show that
\[\textup{elt}:=g^1_{i_1}(h^2_{i_1i_2}(\cdots ))\mapsto q(g^1_{i_1})(h^1_{i_1i_2}(\cdots )).\]
...
\subsection{The resolution $\scrW_\bullet X$ and the chain map $Q\xi_\textup{ch}$}
...
\begin{lem}
$(d_i)^{\wedge 2}\xi_\textup{ch}=\xi_\textup{ch}d_{i+1}$ for $i\geq1$, and $(d_0)^{\wedge 2}\xi_\textup{ch}=(\xi_\textup{ch}d_{0})*(\xi_\textup{ch}d_{1})$. Thus $Q(\xi_\textup{ch})$ induces a degree $-1$ chain map $N_*^\textup{fil}(\scrW X)\to N_*^\textup{fil}((\scrW X)^{\wedge 2})$ (and even on unnormalized complexes).
\end{lem}
\begin{proof}
...
This is entirely analogous to the calculation in the proof of \cite[lemma 5.5]{MR1089001}.
\end{proof}
