...
As the top row is a coequaliser diagram of vectorspaces, $j''$ induces a map $j'$ as long as $j''\circ(\alpha_{\sqcup}-\beta_{\sqcup})\circ \textup{in}_{\mathsf{C}\mathsf{C}}=0$. Then $j'$ will induce a map $j$ as long as $j'$ vanishes on nontrivial operations in $X\wedge Y$.

The vectorspace $\mathsf{C}(X\oplus Y)$ is spanned by formal expressions $e^0(x_i,y_k)$. The vectorspace $\mathsf{C}(\mathsf{C}(X)\oplus\mathsf{C}(Y))$ is spanned by doubly-formal expressions $E=e^0(e^1_{i_1}(x_{i_1i_2}),e^1_{k_1}(y_{k_1k_2}))$, and its subspace $\mathsf{C}\mathsf{C}X\wedge \mathsf{C}\mathsf{C}Y$ is spanned by those in which $e^0$ only contains terms which involve some element of both $X$ and $Y$. The image of this expression under the map $(\alpha_{\sqcup}-\beta_{\sqcup})$ is
\[e^0(e^0_{i_1}(x_{i_1i_2}),e^0_{k_1}(y_{k_1k_2}))-e^0(e_{i_1}(x_{i_1i_2}),e_{k_1}(y_{k_1k_2})).\]
Consider then a monomial summand of $e^0$, when $E\in \mathsf{C}\mathsf{C}X\wedge \mathsf{C}\mathsf{C}Y$. If this summand is not simply of the form $[?,?]$, then it has no contribution to the value of $(j''\circ(\alpha_{\sqcup}-\beta_{\sqcup})\circ \textup{in}_{\mathsf{C}\mathsf{C}})(E)$. Thus we may consider only those summands of the form $[e^1_{i_1}(x_{i_1i_2}),e^1_{k_1}(y_{k_1k_2})]^0$, which under $(\alpha_{\sqcup}-\beta_{\sqcup})\circ \textup{in}_{\mathsf{C}\mathsf{C}}$ maps to
\[[e^0_{i_1}(x_{i_1i_2}),e^0_{k_1}(y_{k_1k_2})]^0-[e_{i_1}(x_{i_1i_2}),e_{k_1}(y_{k_1k_2})]^0.\]
Unless both of $e_{i_1}$ and $e_{k_1}$ are just the identity, both of these terms are sent to zero by $j''$. When both are the identity, so that the element we're looking at is $[x,y]^0$, we're calculating
\[j''([x,y]^0-[x,y]^0)=j''(0)=0.\]
This demonstrates that the extension $j'$ exists.
That $j'$ extends to $j$ follows from the fact that $\rho\wedge \rho$ is surjective and the observation that any nontrivial operation on elements of $\mathsf{C}X\wedge \mathsf{C}Y$ will create more complicated expressions than those of the form $[x,y]^0$.
\end{proof}
