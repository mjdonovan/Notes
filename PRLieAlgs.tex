...
\includeversion{AdamsDerivation}
...
Let $\widetilde{A}^n:=c(K\overline{Q})^{n+1}A$, for $A$ a simplicial algebra. Then $\widetilde{A}^\bullet$ is a levelwise cofibrant cosimplicial object, with a Hurewicz morphism to $Q\widetilde{A}^\bullet$. However, $Q\widetilde{A}^\bullet$ is contractible, and the cosimplicial homology coalgebra $\pi_*(Q\widetilde{A}^\bullet)$ is thus a cofree resolution of $H_*(A)$. However, the Hurewicz morphism is in each level an isomorphism onto the primitives, so that
\[\pi_*(\widetilde{A}^\bullet)\overset{\simeq}{\to}P\pi_*(Q\widetilde{A}^\bullet)\overset{}{\to}\pi_*(Q\widetilde{A}^\bullet).\]
Thus, the $E^2$ term is derived primitives on $H_*(A)$. As such, it should admit a product. In fact, the product should shift both gradings by 1, the internal grading $*$ and the filtration grading $\bullet$.
...
HA_+&%r1c4
\\%r1c5
(HA)^{\times2}\ar;"timesOne"^-{\textup{diag}}
\ar@{->>}[uu]
&%r2c1
&%r2c2
&%r2c3
...
\ar@{-->}@/^1em/[rrrrdd]^-(.8){???}
\\\\%r3c5
c(A^2)\ar[r]^-{c(\textup{diag})}
\ar[uu]^{\textup{(ish)}}
&%r4c1
c(A^4)\ar[r]\ar[dd]^-{\beta}
&%r4c2
c(A_+^2\times A)\ar[r]
\ar[dddddd]
&%r4c3
...
&%r5c1
c(A^2\times c(A^2))
\ar@/_1em/[uuuuuur]^-(.7){c(d_0^2\times h)}
&%r5c2
c(c(A^2_+)\times A)
\ar@/_0em/[uuuuuur]^-(.63){c(h\times d_0)}&%r5c3
&%r5c4
%r5c5
}\]
Here, $A$ was being used as a *terrible* shorthand for $\widetilde{A}^n:=c(K\overline{Q})^{n+1}A$, and unadorned superscripts were used to indicate iterated products. If we just write $k$ for the zero-square algebra $(K\overline{Q})^{n+1}A$, then the map $h:c(A^2)\to A$ is defined to be the composite:
...
Now we should check that the outer square (involving $S,T,s_\beta^-sB,sB$) commutes. The right-down composite, using inductively the commutativity of the diagram sought, simply equals the map $S\to sB$ corresponding to the solid square. The map $T\to sB$ also corresponds to the solid square, and the down-right composite is the restriction to $S$, and thus the outer square commutes. This provides a candidate copy of $T$ in $s_\beta sB$ onto which we may map the given copy of $T$.
