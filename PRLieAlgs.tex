...
\Ixcludeversion{CompressingSequencesOfSteenrodOperations}
...
Let me describe the map $I\mapsto(J,K)$. Suppose that $I=(i_\ell,\ldots,i_1)$. Then the entries double from right to left, and we have the inequalities
\[i_n\leq s+i_1+\cdots+i_{n-1}+1\textup{ for all $n=1,\ldots,\ell$}.\]
Choose the smallest $n$ such that the following related but opposite inequality holds:
\[i_{n+1}> s+(i_1-1)+\cdots+(i_{n}-1)=:s'_0.\]
Then we have $\Sq_\textup{v}^{i_{n},\ldots,i_1}e(w)\in F^{n-1,s'_0}$. Then the element we want is
\[\Sq_\textup{h}^{i_{\ell}-2^{\ell-n-1}s'_0,\ldots,i_{n+2}-2s'_0,i_{n+1}-s'_0}\Sq_\textup{v}^{i_{n},\ldots,i_1}e(w)\]
Now admissibility of $J$ is fine since we're just doubling what we've subtracted each time. We should check that the $\Sq_\textup{h}$ appearing have a chance at being nonzero. We're applying $\Sq_h^{i_{n+r+1}-2^{r}s'_0}$ to an element with filtration
\[n+(i_{n+1}-s'_0)+(i_{n+2}-2s'_0)+\cdots +(i_{n+r}-2^{r-1}s'_0),\]
and the inequality we want, after telescoping the powers of two, comes out to
\begin{alignat*}{2}
i_{n+r+1}-2^{r}s'_0
&\leq
(n+(i_{n+1}-s'_0)+(i_{n+2}-2s'_0)+\cdots +(i_{n+r}-2^{r-1}s'_0))+1%
\\
i_{n+r+1}
&\leq
n+i_{n+1}+i_{n+2}+\cdots +i_{n+r}+s'_0+1%
\\
&\ \ \ \ \ =
i_{n+1}+i_{n+2}+\cdots +i_{n+r}+s+i_1+\cdots +i_n+1%
\end{alignat*}
This is exactly the condition required above, constructing a function $I\mapsto(J,K)$. This function is injective --- if two sequences $I$ and $I'$ return the same sequence $J$ of vertical operations, then $I$ and $I'$ must agree for the first section. Then the next entry of each must be too large, and the $s'_0$ for $I$ and $I'$ coincide. Then clearly the $J$ and $J'$ you get determine the next stretches of $I$ and $I'$.

For surjectivity one just needs to show that it's a section of the obvious `converge' map $(J,K)\mapsto I$. \textbf{Must check that this actually returns admissible dude and unstable dude.} Letting $s'_0$ have the same definition as before, one shows that
\[\Sq_\textup{h}^{j_\ell,\ldots,j_{n+1}}\Sq_\textup{v}^{i_{n},\ldots,i_1}e(w)\textup{\quad detects\quad }\Sq_\textup{h}^{j_\ell+2^{\ell-n-1}s'_0,\ldots,j_{n+1}+s'_0,i_{n},\ldots,i_1}w\]
One proceeds by induction on $\ell-n$. Calculate that the left guy is in $F^{n+j_\ell+\cdots +j_{n+1},2^{\ell-n}s'_0}$. Then if I add a $\Sq_\textup{h}^{j_{\ell+1}}$ to it, I'll add a $\Sq_\textup{h}^{j_{\ell+1}+2^{\ell-n}s'_0}$ to the right hand side. Awesome. This demonstrates by induction the numerology of the previous displayed `detection'. That the original map is a section is evident from the observation that in the right dude, $j_{n+1}+s'_0$ is definitely the first guy that's naughty \textbf{which I must check}.
...
Then 
\begin{prop}
For given $w\in E^s$, there is a bijection between sequences $\Sq_\textup{h}^Iw$ and pairs of sequences $\Sq_\textup{h}^K\Sq_\textup{v}^Je(w)$ such that the former is detected by the latter.
\begin{itemise}
\setlength{\parindent}{.25in}
\item $2i_n\leq i_{n+1}\leq s+i_1+\cdots+i_{n}$ for all $n$
\item $2j_n\leq j_{n+1}\leq s+(j_1-1)+\cdots+(j_{n}-1)$ for all $n$
