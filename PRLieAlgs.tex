...
\subsection{$\textup{DEL}_i$ preserves homologies of cycles}
Suppose that $x$ is a permanent cycle of dimension $n$, and $y$ has dimension $n+1$.
\begin{prop}
$\textup{DEL}_i(x)\sim \textup{DEL}_i(x+dy)$ for $i<n$.
\end{prop}
...
\end{alignat*}
\normalsize
Using coordinates (A-C for columns, 1-3 for rows) to denote the image under $\rho$ of an entry in the above $3\times3$ grid, the result follows from the following observations... \textup{I guess}:
\begin{gather*}
\textup{A2}+\textup{B1}=\rho\mathbb{D}_{n-i}((x+dy)\cdot(x+dy)-x\cdot x);\\
\textup{A3}+\textup{B2}=\textup{B3}=\textup{C1}=\textup{C2}=\textup{C3}=0.\qedhere
\end{gather*}
\end{proof}
