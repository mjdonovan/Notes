...
\includeversion{KoszulComplexes_n>1}
...
The term $\Q^{|y|+i}\Q^iy$ admits the application of an Adem relation, as $|y|+i>2i$, but lemma \ref{lemOnAdemChangeInM} implies that $\Q$-Adem relations never return top $\Q$ operations, so that $\Q^{|y|+i}\Q^iy$ vanishes in the quotient. Thus we've shown that $\Psi$ is a functor such that the square commutes.

To check that the triangle commutes is the note that we didn't fail to kill any of the $\LL{n}$-operations over the course of applying the two functors $\Psi$ and $\Ind{\nontop{n}}$. This is easy both in case $n=1$ and in case $n\geq2$. Finally, we'll check that $\Psi(\Fr{\LL{n}}(X))\cong\Fr{\PRLie{n}}(X)$ in either case.

To see that the triangle commutes is simple, both when $n=1$ and when $n\geq2$.  Proposition \ref{PropFreeKandUconstructions} will show that $\Psi(\Fr{\LL{n}}(X))=\Fr{\PRLie{n}}(X)$ in either case.

When $n\geq2$, if $Q^Ib\in\Fr{\LL{n}}(X)$ is one of the basis elements of proposition \ref{PropFreeKandUconstructions}, then $\Q^Ib$ will be the image of a non-top operation except possibly when $I=(2^{\ell-1}|b|,\ldots,|b|)$, in which case $\Q^Ib=\iteratedrestn{b}{\ell}$. In fact, none of the $\iteratedrestn{b}{\ell}$ are involved in the image of a nontop operation, as no $\Q$-Adem relation produces a top $\Q$ operation. Thus, $\Psi(\Fr{\LL{n}}(X))$ has basis those $\iteratedrestn{b}{\ell}$ which are defined, i.e.\ such that either $\ell=0$, or $\ell>0$ and not all of $a_n,\ldots,a_2$ are zero, aftert writing $\|b\|=(a_n,\ldots,a_1)$. This is exactly the basis of $\Fr{\PRLie{n}}(X)$ given in proposition \ref{PropBasesOfFreeLieAlgs}.

The $n=1$ case is simpler: $\Psi$ kills \emph{all} the $P$ operations, leaving only the elements of a Hall basis for $\Fr{\GoodLie{1}}(X)\cong \Fr{\PRLie{1}}(X)$.
\end{proof}
...
Suppose that $X$ is a levelwise free object in $s\nontop{n}$. Then the map $I\Delta a:I\Delta {\BarConst{\nontop{}}X}\to IX$ is a weak equivalence in $s\GR{n}$.
\end{lem}
\begin{proof}
The map $I\Delta a$ is the map on diagonal simplicial objects induced by the map $Ia:I\BarConst{\nontop{}}X\to I\Constant X$ of bisimplicial objects, which is the prolongation of the augmentation maps $Ia:I\BarConst{\nontop{}}X_s\to I\Constant X_s$ of singly simplicial objects for each $s$. For each $s$, $a:\BarConst{\nontop{}}X_s\to \Constant X_s$ is a weak equivalence of cofibrant objects in $s\nontop{n}$, so as $I$ is left Quillen, $Ia:I\BarConst{\nontop{}}X_s\to I\Constant X_s$ is a weak equivalence in $s\GR{n}$. Thus, the map of spectral sequences obtained by filtering by $s$ is an isomorphism from $E^1$.
\end{proof}
...
We are interested in the derived functors $(\derived\Ind{\LL{n}})X\in\GR{n+1}$ for $X\in\LL{n}$, most of all when $n=1$. It is convenient for us to omit the customary asterisk in $\derived_*$, and to think of the derived functors as a single object with a one more grading than $X$ has. We always write the new grading to the left of the previous gradings, as is customary.
...
The Koszul complex $\overline{K}_*X$ is a subcomplex of $\overline{B}_*X$, and % with basis the classes $\Q(I)x$ (for the same $I$ and $x$ as above). 
the inclusion of $\overline{K}_*X$ into $\overline{B}_*X$ is a homology equivalence. The differential of $\overline{K}_*X$ is given by the following formula:
