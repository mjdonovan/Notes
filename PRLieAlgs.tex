...
For simplicial vectorspaces $U,V$, let $\{\Delta_k\}$ be a collection of higher cosimplicial Alexander-Whitney maps, i.e.\ maps
...
\[N^s\pi_tX\overset{\Gamma^i}{\to}N^{s+i}((\pi_tX)^{\otimes2})\overset{N^{s+i}(\Delta^0)}{\to}N^{s+i}\pi_{2t}(X\wedge X)\to N^{s+i+1}\pi_{2t+1}(X)\]
...
For mixed simplicial vector spaces $X$ and $Y$, write $C(X\otimes_\textup{V}Y)$ for the double complex which in degree $(\fraks,\frakt)$ equals the direct sum $\bigoplus_{t+t'=\frakt}X_t^\fraks\otimes X_{t'}^\fraks$. Now write $K_k$ for the composite
...
\begin{prop}
For $2\leq i\leq \frakt$, the function $F_\fraks CX_{\frakt-\fraks}\to F_{2\fraks}C(X\otimes_{\Sigma_2}X)_{\frakt-\fraks+i}$ defined by $x\mapsto \rho(K_{\frakt-\fraks-i}(x\otimes x))$ preserves cycles and homologies. It is linear up to homology whenever $i<\frakt$, and a quadratic refinement of the product when $i=\frakt$ \textbf{if this makes sense}.
\end{prop}
\begin{proof}
Firstly we explain why this function is even defined --- as $x$ has filtration at least $\fraks$, its simplicial degree is uniformly at least $\frakt$, so that $x\otimes x$ has simplicial degree at least $2t$. Thus, in order that the expression $K_{\frakt-\fraks-i}(x\otimes x)$ is defined, we only need that $2(\frakt-\fraks-i)\leq 2\frakt$. In fact, the stronger inequality $2(\frakt-\fraks-i-2)\leq 2\frakt$ holds.

In order to check that cycles are preserved, we calculate $d\rho(K_{\frakt-\fraks-i}(x\otimes x))$ to be
