...
\includeversion{grothendieck collapse}
...
which has dimension
\[(r-1,\textstyle\sum\ell(I_i)+\textstyle\sum\ell(J_j),?, \left(\textstyle\sum2^{\ell(I_i)}+\textstyle\sum2^{\ell(J_j)+1}\right)(D_1+1)-1).\]
The most general source is $\SqShift^L?$, to be found in
\[(0,\ell(L),?,2^{\ell(L)+1/0}(D_1+1)-1).\]
...
The occurrences of length conbinations satisfying these conditions can be checked using the maple code to follow. It will rule out almost everything, and then one needs to think about the remaining grading...
...
\verb|     for j from 0 to 20 do|\\
\verb|      for k from 0 to 20 do|\\
