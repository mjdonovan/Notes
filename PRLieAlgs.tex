...
However, the rightmost four vector spaces in this diagram are 1-dimensional, and the maps $j$, $\textup{inc}$ and $\textup{proj}$ connecting them are isomorphisms. As $\textup{hur}^{\otimes2}(\imath_t,\imath_{t'})$ is nonzero, it's enough to check that the image of $(\imath_{t},\imath_{t'})$ down in $((HX\times HY)^{\otimes2})_\frakt$ is nonzero, but given that the triangle commutes, this image is $\imath_t\otimes\imath_{t'}+\imath_{t'}\otimes\imath_{t}$, which is evidently nonzero.
\end{proof}
...
This diagram constructs both a map $X^\fraks\otimes X^\fraks\to\overline{D}^1X$ and a zig-zag of weak equivalences from $\overline{D}^1X$ to $D^1X$. This also shows that the small square in the top right corner of the diagram of interest commutes.

At this moment, it is opportune to extend the diagram of interest a little.
%\[\xymatrix@R=4mm{
...
\[\mathclap{\xymatrix@R=4mm{
\pi_t(X^\fraks)\otimes \pi_{t'}(X^\fraks)\ar@{~>}[dd]_-{j\circ(h\otimes h)}
\ar[r]^-{\textup{sEZ}}
\ar@{..>}[ddddr]^-(.2){A}
&%r1c1
\pi_{\frakt}(X^\fraks\otimes X^\fraks)\ar[r]
&%r1c2
\pi_{\frakt}((\overline{D}^1X)^\fraks)&%r1c3
\pi_{\frakt+1}(X_\textup{fl}^{\fraks+1})\ar[l]_-{\partial}
\ar@{~}[dd]^-{\textup{zig-zag}}_-{\cong}
\\%r1c4
&%r2c1
&%r2c2
...
Although some arrows are decorated for emphasis, all of the arrows in this diagram have already been defined [although I haven't given $A$ a name, and maybe $A$ isn't actually well defined, but instead is only defined on pure tensors]. The composite of the dotted maps equals the composite of the wavy maps, so it serves to investigate the composite
