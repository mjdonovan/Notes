...
We'll define the $r^\textup{th}$ derived functor $(\dupdown{r}{c}F)$ of $F$ with respect to Andr\'e-Quillen homology. The definition is inductive: one sets $(\dupdown{0}{b}F)(X)=F(cX)$, and
...
For clarity, we'll record the construction of $(\dupdown{1}{c}F)(X)$, $(\dupdown{2}{c}F)(X)$ and the map between them, in the following diagram. Note that every composable pair of  parallel arrows is to be a homotopy fiber sequence.
...
Moreover, this construction is natural in the functor $F$ and homotopical as long as $F$ preserves weak equivalences between cofibrant objects. Finally, given a morphism $(c,\epsilon)\to (c',\epsilon')$ of cofibrant replacements, one obtains natural weak equivalences $(\dupdown{t}{c})X\to(\dupdown{t}{c'})X$.
...
We'll modify the construction of $(\dupdown{s}{b}F)$ only for the following functors $F$: the identity $I$, the ``$s^\textup{th}$ power'' functor $(P^sX)_n:=(X_n)^s$, and $b$, the bar construction functor itself.
%
...
Again, the definition is inductive: one sets $(\caldup{0}F)=F$ and
\[(\caldup{s}F)(X):=\ker((\caldup{s-1}F)(cX)\to (\caldup{s-1}F)(QcX)).\]
There are two differences between this definition and the definition of $(\dupdown{s}{b}F)$ --- there is one fewer application of the replacement $b$, and we take strict fibers (i.e.\ kernels) instead of homotopy fibers. Fortunately, there's a zigzag of towers
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
in which the upward maps result from mapping strict fibers to homotopy fibers, and the downward maps are induced by $\epsilon:b\to I$.
\begin{prop*}
...
Now the bottom two rows are both short exact, using the fact (\ref{towerWithPowers}) that the functors $(\caldup{r}I)$ and $(\caldup{r}b)$ both preserve surjections. The top row is defined to be a homotopy fibre sequence. The induction hypothesis shows that 4 of the six verticals are weak equivalences, and two applications of the 5-lemma finish the job.
\end{proof}
...
Apply $(\caldup{t}\DASH)X$ to the diagram of functors constructed in \ref{towerWithPowers}. In light of \ref{DsDt=Dt+s}, one obtains the following diagram in $s\Comm$:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
The verticals in this diagram constitute a pointwise weak equivalence between those diagrams from which we obtain $\widetilde{X}^2:=\hofib(X\to \Tot_2\overline{X}^\bullet)$ and $(\dupdown{2}{\sigma} I)(X)$ respectively.
%%%\[\xymatrix@R=4mm{
%%%\cdots 
%%%\ar[r]
%%%&%r1c1
