...
\includeversion{AdamsSSEQ operations final}
...
\small[\textbf{I briefly believed:} It is in fact possible to iterate this construction, to naturally lift `products of products' two steps higher in filtration, even $\Sigma_4$-equivariantly. That is, there is a diagram
\[\xymatrix@R=4mm{
...
\[\mathbb{D}_r(z)=\sum_{\alpha-\beta=r}\Delta_\alpha\twist^\alpha D^\beta(z)=\begin{cases}
...
We'll need to apply the formula for $d\Delta_\alpha+\Delta_\alpha d$ to $\twist^\alpha D^\beta(z)$, for $\alpha$, $\beta$ such that $\alpha-\beta=r$. %, where $z\in (CX\otimes CX)_{-s,t}$ is the element to which we're applying the above equation.
...
We'd like to define operations $\delta_i:E^r_{-s,t}\to E^r_{-s,t+i}$ for $i$ satisfying $2\leq i<t-(r-2)$, using the chain-level formula (writing $n=t-s=|x|$):
...
At the chain level, what we're doing here is taking a representative $z\in C_{-s,t}X$ such that $\partial_\textup{sim}z=0$, and forming $\rho\Delta_{t-i}(z\otimes_\textup{v} z)$, where $z\otimes_\textup{v} z$ denotes the element of $C(X\otimes_\textup{v}X)_{-s,2t}$.
...
Suppose that $x\in E^r_{-s,t}$. The purpose of $\delta_i(x)$, when $n<i<t$, is to support a $d_{t-i+1}$-differential to $\Sq^{t-i+1}(x)$. Thus, we would not expect to be able to define $\delta_i(x)$ when $t-i+1<r$; indeed, the following result will construct $\delta_i(x)$ whenever $i<t-(r-2)$. Moreover, the Steenrod operation $\Sq^{t-i+1}(x)$ has indeterminacy vanishing by $E^{2(r-1)}$, and one might expect that whenever $t-i+1<2(r-1)$, $\delta_i(x)$ will be multivalued, but that the set of values for $\delta_i(x)$ will map onto the set of values for $\Sq^{t-i+1}(x)$ under $d_{t-i+1}$. Note that it is not correct to say in general that the indeterminacy of $\delta_i(x)$ vanishes by a certain page, but rather that one expects that the various values for $\delta_i(x)$ will all fail to be permanent cycles together.
\begin{prop}
The chain-level map $\textup{DEL}_i$ produces well defined $\delta$-operations $E^r_{-s,t}\to E^r_{-s,t+i}$ whenever $r\geq2$, $i<t$ and either $i\leq n+1$ or $i<t-2(r-2)$. When $t-2(r-2)\leq i<t-(r-2)$, the operations are multivalued.
\end{prop}
\begin{proof}
...
\end{alignat*}
\normalsize
Now by \ref{DkIsNiceToFiltration},
