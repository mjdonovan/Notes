...
\newcommand{\Ind}[2][]{\ifblank{#1}{\mathbf{I}^{\mbox{\tiny $#2$}}}{\mathbf{I}^{\mbox{\tiny $#2$}}_{#1}}}%[internal indices]{category name}
\newcommand{\forget}{\mathrm{fg}}
\newcommand{\Forget}[1]{\mathrm{fg}_{\mbox{\tiny $#1$}}}
\newcommand{\BarConst}[1]{B^{\mbox{\tiny $#1$}}}
...
%%%%You might write this as $B_\bullet(\forget,\Fr{\nontop{n}},\Forget{\nontop{}}X)\to B_\bullet(\PRLie{n},\Fr{\LL{n}},X)$.
%%%%\end{prop}
%%%%\noindent In light of this result, we write $(\derived{\Ind{\nontop{}}})X$ for either $(\derived{\Ind{\nontop{}}})X$ or $(\derived{\Ind{\nontop{}}})\forget(X)$, without ambiguity. \textbf{Discuss} the meaning of the fg and Rn in the bar construction.
%%%%\begin{proof}
...
%%%%\[\forget(\Ind{\nontop{}}(B_\bullet(\Fr{\LL{n}},\Fr{\LL{n}},X)))=\forget(B_\bullet(\Fr{\PRLie{n}},\Fr{\LL{n}},X))\in s\GR{n}.\]
%%%%\end{lem}
%%%%\begin{proof}
...
In \cite{MR0365573}, Bousfield and Kan construct, for a space (i.e.\ simplicial set) $X$, the $\F_2$-completion $\hat X$ to be the totalisation of the cosimplicial resolution of $X$ arising from the monad of the adjunction $\F_2:s\Set_*\rightleftarrows s\Vect:\forget$. By virtue of $\hat X$ having been constructed as a totalisation, there is a totalisation spectral sequence converging to $\pi_*(\hat X)$. This is known as the unstable Adams spectral sequence for $X$.
...
defined whenever $i\leq a_n$ and not all of $i,a_{n-1},\ldots,a_{2}$ are zero. The top $\Q^i$ \emph{equals} the restriction operation, so that $\Q^{i}(x)=\restn(x)$ whenever $|x|=i$ and both sides are defined%footnote:
...
For any $n\geq1$, the proposed basis of $\Fr{\LL{n}}(X)$ is seen to be linearly independent using the map $\Fr{\nontop{n}}(X)\to \Fr{\LL{n}}(X)$. To calculate $\Fr{\LL{n}}(X)$ when $n\geq2$, note that we have calculated the object $\pi_*(\Fr{\PRLie{n-1}}S(X))\in\PiAlg{\PRLie{n-1}}=\LL{n}$, and that the elements of our proposed basis of $\Fr{\LL{n}}(X)$ map to a basis of $\pi_*(\Fr{\PRLie{n-1}}S(X))$ under the canonical map $\Fr{\LL{n}}(X)\to \pi_*(\Fr{\PRLie{n-1}}S(X))$. %This proves the linear independence statement we require.
...
The functor $\Ind{\mathsf{C}^n}$ is always left adjoint to a functor $K:\GR{n}\to\mathsf{C}^n$ which sends a graded vector space $V$ to the corresponding trivial object in $\mathsf{C}^n$. That is, $KV$ has $V$ as underlying vector space, and all operations in $KV$ are zero. Note then that the right adjoint, $K$, preserves weak equivalences and fibrations, as membership of these classes is determined on the level of underlying simplicial groups. Thus, $\Ind{\mathsf{C}^n}$ is a left Quillen functor, so that we can easily define the left derived functors $\derived_*\Ind{\mathsf{C}^n}$.
...
Suppose that $X$ is a levelwise free object in $s\nontop{n}$. Then the map $I\Delta a:\Ind{\nontop{}}\Delta {\BarConst{\nontop{}}X}\to \Ind{\nontop{}}X$ is a weak equivalence in $s\GR{n}$.
\end{lem}
\begin{proof}
The map $\Ind{\nontop{}}\Delta a$ is the map on diagonal simplicial objects induced by the map $\Ind{\nontop{}}a:\Ind{\nontop{}}\BarConst{\nontop{}}X\to \Ind{\nontop{}}\Constant X$ of bisimplicial objects, which is the prolongation of the augmentation maps $\Ind{\nontop{}}a:\Ind{\nontop{}}\BarConst{\nontop{}}X_s\to \Ind{\nontop{}}\Constant X_s$ of singly simplicial objects for each $s$. For each $s$, $a:\BarConst{\nontop{}}X_s\to \Constant X_s$ is a weak equivalence of cofibrant objects in $s\nontop{n}$, so as $\Ind{\nontop{}}$ is left Quillen, $\Ind{\nontop{}}a:\Ind{\nontop{}}\BarConst{\nontop{}}X_s\to \Ind{\nontop{}}\Constant X_s$ is a weak equivalence in $s\GR{n}$. Thus, the induced map of spectral sequences (those obtained by filtering by $s$) is an isomorphism from $E^1$.
\end{proof}
...
\hspace{1.5cm}
\]
It will be important in what follows, for various $X\in\LL{n}$, to know the values of $\derived\Ind{\nontop{}}$ as an object of $\LL{n+1}$. On the other hand, the values as a graded vector space in $\GR{n+1}$ can be calculated by a Koszul complex. The plan is to map representing cycles in the Koszul complex into the large resolution, perform the $\LL{n+1}$ operations of interest at the chain level, and homotope back into the Koszul complex.
...
%\end{proof}
%\begin{lem}
%For any $X$ in $s\LL{n}$, $(\derived{\Ind{\nontop{}}})(\forget(X))$ may be calculated as the homotopy of the object 
%\[\forget(\Ind{\nontop{}}(B_\bullet(\Fr{\LL{n}},\Fr{\LL{n}},X)))=\forget(B_\bullet(\Fr{\PRLie{n}},\Fr{\LL{n}},X))\in s\GR{n}.\]
%\end{lem}
%\begin{proof}
%Although $B:=B_\bullet(\Fr{\LL{n}},\Fr{\LL{n}},X)$ is an acyclic levelwise free object in $s\nontop{n}$, with $\pi_0(B)=X$, it may fail to be `almost free'. The point here is that $B$ can be used to calculate the derived functors of $\Ind{\nontop{}}$ anyway. To see this, consider the bisimplicial object $A:=\Ind{\nontop{}}(B_\bullet(\Fr{\nontop{n}},\Fr{\nontop{n}},B))$ formed by applying $\Ind{\nontop{}}$ to the levelwise bar construction on $B$, so that
...
A basis of $\overline{B}_r$ consists of all the above monomials in which each of the $r$ distinguished subsequences of $I$ is $\Q$-admissible, as $x$ runs through a homogeneous basis for $X$. The formula for the differential is the standard one, making %under the $\ell$ grading on each $\overline{B}_r$ makes
 $\overline{B}_*$ a chain complex with increasing filtration, $F_p\overline{B}_r=\bigoplus_{\ell\leq p}\overline{B}_{r,\ell}$. 
\end{prop}
\noindent We'll say that a monomial $\BarMonomial{I}{\textbf{k}}{x}\in\overline{B}_r$ is bar-admissible if each of the $r$ distinguished subsequences of $I$ are $\Q$-admissible. Thus a basis for $\overline{B}_{r,\ell}$ consists of the bar-admissible monomials on a basis of $X$. Note that $\overline{B}_{r,\ell}=0$ for $r>\ell$, and that for any fixed element $x$ and any fixed $\ell$, the condition on $\minDim(I)$ implies that there are only finitely many monomials of the form $\BarMonomial{I}{\textbf{k}}{x}$ in $\overline{B}_{r,\ell}$.
\begin{prop}
Any cycle in $\overline{B}_{r,\ell}$, for $\ell>r$ is homologous to a cycle in $\overline{B}_{r,r}$.
\end{prop}
\begin{proof}
First, a construction. For any \emph{bar-admissible} monomial $\BarMonomial{I}{\textbf{k}}{x}\in\overline{B}_{r,\ell}$ with $\ell>r$, choose the unique integer $e$ such that $k_r=k_{r-1}=\cdots =k_{r-e+1}=1<k_{r-e}$, so that
...
That is, we've identified the length, $e$, of the longest initial segment of length one bars. Then we define
...
\end{cases}
\]
Thus $\Gamma$ either returns zero or adds a single bar to produce a bar-admissible monomial in $\overline{B}_{r+1,\ell}$. One may extend $\Gamma$ by linearity to produce linear operators $\Gamma:\overline{B}_{r,\ell}\to\overline{B}_{r+1,\ell}$ for $\ell>r$, and extend $\Gamma$ by zero on $\overline{B}_{r,r}$ to produce $\Gamma:\overline{B}_{r}\to\overline{B}_{r+1}$.


Define a total preorder on the bar-admissible monomials by declaring
...
Then, as remarked in the proof of \cite[Thm 5.3]{PriddyKoszul.pdf}, a straightforward verification shows that for any bar-admissible monomial $m\in\overline{B}_{r,\ell}$ with $\ell>r$, $\partial\Gamma(m)+\Gamma\partial(m)\equiv m$ modulo monomials strictly lesser than $m$.
...
Now $\delta:\overline{C}^{r-1,r}\to\overline{C}^{r,r}$ may be described as follows. A basis for $\overline{C}^{r-1,r}$ is given by duals of bar-admissible monomials of the form
...
%\end{itemise}
The term with $(i_{m+1},i_m)=(\alpha,\beta)$ does not vanish in this way. Moreover, for length 2 sequences $K$ and $L$, as $\Q$ and $\SqShift$ are Koszul dual, $\produces{K}{L}{\Q}$ if and only if $\produces{L}{K}{\SqShift}$. Thus, we obtain a relation:
\[\left(\left[\Q^{i_r}\middle|\cdots \middle|\Q^{i_1} \right]x\right)^*
=
...
Using this relation, we see that a basis of $H^{r,r}\overline{C}$ is represented by the cocycles $\BarMonomial{I}{\textbf{k}}{x}^*$, where $I$ and $x$ are as in the statement of this proposition, and $\textbf{k}=(1,\ldots,1)$.

As long as each of the $\Q(I)x$ are cycles, they will form the dual basis for $H_{rr}\overline{B}$. %, as in the sum defining $\Q(I)x$, the only $\SqShift$-admissible $J$ appearing is $J=I$. 
In order to check that $\Q(I)x$ is a cycle, it only remains to check that it pairs to zero with the image of $\delta$, i.e.\ with the $\SqShift$-Adem relations displayed above. But that is not hard to see. By definition of $\Q(I)x$, the functional `pair with $\Q(I)x$' on $\overline{C}^{r,r}$ \emph{equals} the functional `reduce to $\SqShift$-admissible form using $\SqShift$-Adem relations, and pair with $\left[\Q^{i_\ell}\middle|\cdots\middle|\Q^{i_1} \right]x$'. This second functional obviously returns zero on the $\SqShift$-Adem relations.
\end{proof}
Now we are in a position to describe a complex calculating $(\derived\Ind{\nontop{}})X$. For any $X\in\nontop{n}$, define the \emph{Koszul complex} $\overline{K}_*X$, for $X\in\nontop{n}$, to be the subspace of $\overline{B}_*X$ spanned by the classes $\Q(I)x$, for $I$ and $x$ as in proposition \ref{propDerivedIndTrivialUobject}.
\begin{prop}\label{KoszulComplexN>2}
%Suppose instead that $X$ is any object in $\nontop{n}$, 
The Koszul complex $\overline{K}_*X$ is a subcomplex of $\overline{B}_*X$, whose inclusion is a homology equivalence. The differential of $\overline{K}_*X$ is given by the following formula:
...
Consider the spectral sequence associated to the filtered complex $\overline{B}_*X$, indexed so that $E^0_{r,\ell}=\overline{B}_{r,\ell}(X)$ (as vector space), and $d^q:E^q_{r,\ell}\to E^q_{r-1,\ell-q}$. In particular, each $E^q_{r,\ell}$ is concentrated where $\ell\geq r$. Now the chain complex $E^0$ can actually be identified with $\overline{B}_{r,\ell}(X)$, where $KX$ is a trivial object of $\nontop{n}$ on the same underlying vector space as $X$. Thus, $E^1$ is concentrated along the line $r=\ell$, and there are natural inclusions of vector spaces $E^1_{rr}\to E^0_{rr}\to \overline{B}_r$, whose composite is a map of complexes which is a homology equivalence. One is simply left to note that this inclusion is modeled by the inclusion of the subspace of $\overline{B}_*$ spanned by the $\Q(I)x$.




%The inclusion $\overline{K}_*X\to \overline{B}_*X$ is exactly

%The spectral sequence associated to the filtration on $\overline{B}_*X$ has $E^0$ the bar construction $\overline{B}_{r,\ell}(KX)$, with $d^0$ its differential, where $KX$ is a trivial object of $\nontop{n}$ on the same underlying vector space as $X$. Precisely, $E^0_{r,\ell}=\overline{B}_{r,\ell}(KX)$. %The differential $d^0$ implements the differential on this reduced bar construction, so that
...
\end{itemize}
A basis of $\overline{B}_r$ consists of all the above monomials in which each of the $r$ distinguished subsequences of $I$ is $P$-admissible, as $x$ runs through a homogeneous basis for $X$. The formula for the differential is the standard one, making %under the $\ell$ grading on each $\overline{B}_r$ makes
 $\overline{B}_*$ a chain complex with increasing filtration, $F_p\overline{B}_r=\bigoplus_{\ell\leq p}\overline{B}_{r,\ell}$. 
\end{prop}
Note that in $\overline{B}_*$, any $I$, $\textbf{k}$ and $x$ will produce a well-defined monomial; those violating the inequality $\minDimP(I)\leq a_1$ are simply zero, rather than undefined.

\begin{prop}
Suppose that $X$ is any object in $\nontop{1}$, the Koszul complex $\overline{K}_*X$ is the subcomplex of $\overline{B}_*(\Forget{\GR{}},\Fr{\nontop{1}},X)$ with basis the classes $P(I)x$, where 
\begin{itemize}
\setlength{\parindent}{.25in}
...
There is a complex that computes $\derived\Ind{\PRLie{}}$ --- it interpolates between the Cartan-Eilenberg resolution for the homology of good Lie algebras, and May's $\overline{X}$ complex for  restricted Lie algebra homology.
...
\subsection{Constructions in the homotopy of simplicial Lie algebras} \label{SectionHtpyOpnsOnLieAlgs}
...
Fortunately, it happens that the inclusion of the Koszul complex factors through this isomorphism \emph{explicitly}. To understand this, one thinks that an element of the Koszul complex is an element of $\overline{B}_{rr}$ which survives to $E^1$ in the length filtration spectral sequence for $\overline{B}_*$. However, $d^0$ implements the sum of all the face maps but one, so that any element $z$ of $\overline{K}_r$ can be written as the image in $\ModDegeneracies{\Ind{\nontop{}}(\BarConst{\nontop{}}X)}_r$ of a unique expression in $\Ind{\nontop{}}(\BarConst{\nontop{}}X)_r$ of the form
\[z=\sum_{j}\left[\Q^{i_{j,r}} \middle|\cdots\middle|\Q^{i_{j,1}} \right]x_j\]
such that 
\[\sum_{k=1}^{r-1}d_kz:=\sum_{k=1}^{r-1}\sum_{j}\left[\Q^{i_{j,r}} \middle|\cdots\middle|\Q^{i_{j,k+1}}\Q^{i_{j,k}}\middle|\cdots\middle|\Q^{i_{j,1}} \right]x_j=0\textup{ in }\overline{B}_{r-1,r}.\]
...
%\[\overline{K}_*X\to N(\Ind{\nontop{}}(\BarConst{\nontop{}}(\Forget{\nontop{}}X)))\overset{N(\Ind{\nontop{}}(i))}{\to} N(\Ind{\nontop{}}(\Forget{\nontop{}}\BarConst{\LL{n}}(X)))\]
\begin{alignat*}{2}
\overline{K}_*(\Forget{\nontop{}}X)
&\xrightarrow{\phantom{N(\Ind{\nontop{}}(i))}}
N(\Ind{\nontop{}}(\BarConst{\nontop{}}(\Forget{\nontop{}}X)))%
\\
&\xrightarrow{N(\Ind{\nontop{}}(i))}
N(\Ind{\nontop{}}(\Forget{\nontop{}}\BarConst{\LL{}}(X)))\\
% Left hand side
% Relation
&\phantom{\xrightarrow{{N(\Ind{\nontop{}}(i))}}}\ =
% Right hand side
\Forget{\GR{}}N(\Ind{\nontop{}}(\BarConst{\LL{}}(X)))% Comment
\end{alignat*}
\newpage
These two maps are inclusions, so that $\overline{K}_*(\Forget{\nontop{}}X)$ is a homology-equivalent subcomplex of $N(\Ind{\nontop{}}(\BarConst{\LL{}}(X)))$, the differential graded partially restricted Lie algebra which calculates the $\PiAlgebra{\PRLie{n}}$ $(\derived\Ind{\nontop{}})X$. Suppressing the forgetful functors, let 
\[\jmath:\overline{K}_*(X)\to N(\Ind{\nontop{}}(\BarConst{\LL{}}(X)))\]
be this composite inclusion. In order to calculate with this larger complex, we'll need to be explicit about the bar construction $\Ind{\nontop{}}(\BarConst{\LL{}}(X))$. As $\Ind{\nontop{}}(\Fr{\LL{n}})W=\Fr{\PRLie{n}}W$ for $W\in\GR{n}$, we can write:
\[B^\textup{L}_rX:=\Ind{\nontop{}}(\BarConst{\LL{}}(X))_r=\Fr[out]{\PRLie{n}}\Fr[r]{\LL{n}}\cdots\Fr[1]{\LL{n}} X.\]
Here, we've introduced the notation ``$B^\textup{L}_\bullet$'' for this \emph{large} bar construction, and added subscripts to the various free constructions involved, in order to disambiguate them in what follows. The structure maps in this simplicial object are defined by the following natural transformations:
\begin{itemize}\squishlist
\setlength{\parindent}{.25in}
\item $d_0:B^\textup{L}_r\to B^\textup{L}_{r-1}$ is induced by $\Fr[1]{\LL{n}}X\to X$;
\item $d_i:B^\textup{L}_r\to B^\textup{L}_{r-1}$ is induced by $\Fr[i+1]{\LL{n}}\Fr[i]{\LL{n}}\to \Fr[i]{\LL{n}}$ for $0<i<r$;
\item $d_r:B^\textup{L}_r\to B^\textup{L}_{r-1}$ is induced by $\Fr[out]{\PRLie{n}}\Fr[r]{\LL{n}}\to \Fr[out]{\PRLie{n}}$ (which, for $n\geq2$, kills nontop $\Q^i$ terms and identifies the top $\Q$ with $\restn{}$, and when $n=1$, kills all $P$ operations);
\item $s_i:B^\textup{L}_r\to B^\textup{L}_{r+1}$ is induced by the unit map, after inserting another $\Fr{\LL{n}}$ between $\Fr[i+1]{\LL{n}}$ and $\Fr[i]{\LL{n}}$ , for $0\leq i\leq r$.
\end{itemize}
The bar notation used until now will not suffice to describe elements of $B^\textup{L}_\bullet$. Instead, we add subscripts corresponding to those added to the free constructions above. For example, here are two elements of $B^\textup{L}_3$ (when $n\geq2$), if $x,y\in X$ are of sufficiently large degrees:
...
We are now in a position to calculate the $\LL{n+1}$ structure on $(\derived\Ind{\nontop{}})X$ in certain cases, since we can write down chain-level formulae. For $z\in Z\overline{K}_p(X)$ and $w\in Z\overline{K}_q(X)$, representing classes $\overline{z},\overline{w}\in(\derived\Ind{\nontop{}})X$:
\begin{itemise}
\setlength{\parindent}{.25in}
\item $[\overline{z},\overline{w}]$ is represented by $\sum_{(\alpha,\beta)\in\Shuffles{p}{q}}[s_\beta(\jmath z), s_\alpha(\jmath w)]_{out}\in B^\textup{L}_{p+q}X$;
\item $Q^i\overline{z}$ is represented by $\sum_{(\alpha,\beta)\in\HalfShuffles{i}{i}}[s_\beta(\jmath z), s_\alpha(\jmath z)]_{out}\in B^\textup{L}_{p+i}X$ for $0<i\leq p$; and
\item $Q^0\overline{z}$ is represented by $\restn[out]{(z)}\in B^\textup{L}_{p}X$ as long as $Q^i\overline{z}$ is defined, (i.e.\ when not all of $a_n,\ldots,a_2$ are zero, where $\overline{z}\in((\derived_{p}\Ind{\nontop{}})X)_{a_n,\ldots,a_1}$).
\end{itemise}
%%%%%%%
...
using the notation from the description of \textbf{Priddy's algorithm}, and writing $\textbf{1}:=(1,\ldots,1)$. Thus
...
in the bar construction $N{B_*}(\PRLie{n},\LL{n},X)$. This cycle is the image of a cycle in $NB_*(\Forget{\GR{}},\nontop{n},X)$ which, after mapping down to the quotient by degenerate simplices, we dare write as:
\[\sum_{j}\left[\Q^{a_n}\Q^{i^{{(j)}}_p}\middle|\cdots \middle|\Q^{i^{{(j)}}_1}\right]x^{(j)}\in\overline{B}_{p,p+1}.\]
Now in order to compress this into the Koszul complex, we must apply \textbf{Priddy's algorithm}. But one observes readily from the $\Q$-Adem relation that if $\produces{(i,j)}{(i',j')}{\Q}$, then $j'>j$, so that as we perform \textbf{Priddy's algorithm}, we only ever decrease the index of the final $\Q$ appearing in each bar. Then the hypothesis on the $x^{{j}}$ guarantees that as we run the algorithm, every term that ends up in $\overline{B}_{p,p}$ willl vanish. Thus \textbf{Priddy's algorithm} constructs a null-homotopy.
\end{proof}
