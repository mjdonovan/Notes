...
\includeversion{Adams sseq operations}
...
\item the restriction of $D^k$ to $C_{-i}R\otimes C^{-j}S$ is zero unless $i\geq k$ and $j\geq k$; and
\item $D^k$ maps $C_{-k}R\otimes C_{-k}S$ identically onto $C_{-k}(R\otimes S)$.
\end{itemise}
%It is a useful convention to define $D^i=0$ for $i<0$.
...
\end{align*}
Actually, let's define these on the entire double complex by just defining them to be zero in dimensions they aren't supposed to be defined.
...
\[S^{i,s}_\textup{alg}:x\mapsto \rho\Delta (D^{s-i}(x\otimes x)+D^{s-i+1}(x\otimes dx)).\]
The usual trick, along with the fact that $\Delta$ is a symmetric chain map, shows that $d(S^{i,s}_\textup{alg}(x))=\rho\Delta D^{s-i+1}(dx\otimes dx)$, so that if $x\in E^r_{-s,t}$, $S^{i,s}_\textup{alg}(x)\in E^{2r-1}_{-s-i,2t}$. To examine the indeterminacy, $x\in E^r_{-s,t}$ is determined up to boundaries of $y\in E^{r-1}_{-(s-r+1),t+r+2}$, and the following equation%footnote:
\footnote{Here's how, using analogues of \cite[(1.111),(1.112)]{MR2245560}:
\begin{alignat*}{2}
\textup{RHS}
&=
...
\[S^{i,s}_\textup{alg}(x)-S^{i,s}_\textup{alg}(x+dy)=d\rho\Delta [D^{s-i-1}(y\otimes y)+D^{s-i}(y\otimes dy)+D^{s-i+1}(dy\otimes x)].\]
...
\newcommand{\twist}{\sigma}
Write $\twist F$ as somewhat ambiguous shorthand for the function $TFT$. 
Now write $\mathbb{D}_r$ for the sum (for $r\geq0$):
\[\mathbb{D}_r=\sum_{\beta-\alpha=r}\Delta_\alpha\twist^\alpha D^\beta=\sum_{j\geq0}\Delta_{j}\twist^{j}D^{j+r}.\]
Now suppose that $x\in E^r_{-s,t}$. Let's try to define $\delta_i(x)$ for $n<i<t$, where $n=t-s=|x|$. I believe that the correct formula may be
\[x\mapsto \textup{DEL}_{i}^\textup{alg}(x):=\mathbb{D}_{i-n}(x\otimes x)+\mathbb{D}_{i-n+1}(dx\otimes x).\]
\subsubsection*{Exhibit A:}
Let's calculate the leading term of $\textup{DEL}_{i}^\textup{alg}(x)$. Write $x=x_s+x_{>s}$, where $x_s\in X^s_t$, and $x_{>s}$ lies in filtration $s+1$. The the leading term will be $\rho\Delta_{t-i}\twist^{t-i}D^s (x_s\otimes x_s)$. Moreover, as $D$ is \emph{special}, this equals $\rho\overline{\Delta}_{t-i}(x_s\otimes x_s)\in C(X\otimes_{\Sigma_2} X)_{-s,t+i}$ (where we think of $(x_s\otimes x_s)$ as an element of $C(X\otimes_\textup{v}X)_{-s,2t}$), so that `$x_s\mapsto D^s(x_s\otimes x_s)$' implements the assignment `$z\mapsto z\otimes z$' at the $E^1$-level, and we're getting a representative for the obvious $E^2$-level $\delta_i$ operation.
...
Let's calculate $d\mathbb{D}_{r}+\mathbb{D}_{r}d$ (when $r\geq1$).
\begin{alignat*}{2}
d\mathbb{D}_r
&+
...
% Comment
\end{alignat*}
using the observation that $(1+\twist)F\twist G+F(1+\twist)G=(1+\twist)(FG)$. Then use the fact that $(1+\twist)\Delta_0=\Delta+\phi_0$.} that for $r\geq1$:
\[d\mathbb{D}_r+\mathbb{D}_rd=(1+\twist)\mathbb{D}_{r-1}+\Delta\sigma D^{r-1}+ \sum_{j\geq0} \phi_{j}\sigma^{j-1}D^{j+r-1},\]
from which it's a formality to derive that
\[d(\textup{DEL}^\textup{alg}_i(x))= \textup{DEL}^\textup{alg}_i(dx)+S_\textup{alg}^{t-i+1,s}(x)+\textup{error}\]
where the error term is
\[\sum_{j\geq0}\phi_j\sigma^{j-1}D^{j+i-n-1}(x\otimes x)+\sum_{j\geq0}\phi_j\sigma^{j-1}D^{j+i-n}(dx\otimes x)).\]
But each error term is in fact zero. Let's show that the summand $\phi_j\sigma^{j-1}D^{j+i-n-1}(x\otimes x)$ is zero. Write $x=\sum x_{j}^{j-n}$ as the sum of its various homogeneous parts. Then $\sum\phi_j\sigma^{j-1}D^{j+i-n-1}(x\otimes x)=\sum\phi_j\sigma^{j-1}D^{j+i-n-1}(x_j^{j-n}\otimes x_j^{j-n})=0$, as $D^{j+i-n-1}(x_j^{j-n}\otimes x_j^{j-n})=0$ for dimension reasons.
