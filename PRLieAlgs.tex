...
\cancel{(1{+}\twist)\mathbb{D}_{n-i+2}(y\cdot y)}&&+
\Delta_{n-i+2}(1{+}\sigma)D^0(y\cdot y)\\
d\Delta_{n-i-1}\twist^{n-i-1} D^0(x\cdot dy)&=
\cancel{\Delta_{n-i-1}\twist^{n-i-1} D^0(dx\cdot dy)}&&+
(1{+}\twist)\Delta_{n-i}\twist^{n-i-1} D^0(x\cdot dy)&&+
\cancel{\phi_{n-i}\twist^{n-i-1} D^0(x\cdot dy)}\\
d\Delta_{n-i}\twist^{n-i}D^0(dy\cdot y)&=
\Delta_{n-i}\twist^{n-i}D^0(dy\cdot dy)&&+
(1{+}\twist)\Delta_{n-i+1}\twist^{n-i}D^0(dy\cdot y)&&+
\cancel{\phi_{n-i+1}\twist^{n-i}D^0(dy\cdot y)}\\
d\Delta_{n-i}\twist^{n-i-1} D^0(y\cdot y)&=
\Delta_{n-i}\twist^{n-i-1} D^0d(y\cdot y)&&+
(1{+}\twist)\Delta_{n-i+2}\twist^{n-i-1} D^0(y\cdot y)&&+
\cancel{\phi_{n-i+2}\twist^{n-i-1} D^0(y\cdot y)}
\end{alignat*}}
\[\textup{A2}+\textup{B1}=\mathbb{D}_{n-i}((x+dy)\cdot(x+dy)-x\cdot x)\]
\[\textup{A5}+\textup{B4}+\textup{C1}=\Delta_{n-i}\twist^{n-i}D^0((x+dy)\cdot(x+dy)-x\cdot x)\]
\[\textup{A3}+\textup{B2}=\textup{A6}+\textup{B5}+\textup{C2}= \textup{B6}+\textup{C3}=0\]
\end{proof}

\subsection{Thought}
Could it be that we can define the operations differently on the image of cycles in homotopy, and the operations I'm defining all over the place really do converge to zero? Then the way to recover the homotopy of the sphere would be to do it in the sseq of an iterated symmetric square, then finally to map out to the underlying sseq with an iterated shift.
