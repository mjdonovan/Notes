...
For each of the above categories, there is a functor $\Ind{\mathsf{C^n}}:\mathsf{C^n}\to\GR{n}$ which sends $X\in\mathsf{C}^n$ to the quotient of the underlying vector space of $X$ by the subspace generated by the images of all the operations defining $X$.
...
We are interested in the derived functors $(\derived\Ind{\LL{n}})X\in\GR{n+1}$ for $X\in\LL{n}$, most of all when $n=1$. It is convenient for us to omit the customary asterisk in $\derived_*$, and to think of the derived functors as a single object with a one more grading than $X$ has. We always write the new grading to the left of the previous gradings, as is customary. To study $(\derived\Ind{\LL{n}})(X)$ we use the Grothendieck spectral sequence derived in \cite{Blanc_Stover-Groth_SS.pdf}, which takes the form:
...
\begin{shaded}
\textbf{The following observation explains why there's little danger in having two functors named $\Ind{\nontop{n}}$.}
\begin{prop}
For $n\geq1$, and any $X\in\LL{n}$, there's an isomorphism of vector spaces
\[(\derived{\Ind{\nontop{n}}})(\forget_{\nontop{}}X)\cong\forget_{\GR{}}(\derived{\Psi})X,\]
induced by the evident inclusion 
