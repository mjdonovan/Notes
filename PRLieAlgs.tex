...
\begin{convergence}
Following \cite{BousKanSSeq.pdf}, for any functor $F:s\Comm\to s\Comm$, we'll define $r^\textup{th}$ derived functor $(D^sF)$ of $F$ with respect to Andr\'e-Quillen homology. The definition is inductive: we set $(D^0F)=F$, and
\[(D^sF)(X):=\ker((D^{s-1}F)(cY)\to (D^{s-1}F)(QcY)).\]
...
Our purpose will be to prove the following fact about the resulting tower:
\begin{prop}\label{convergenceProp}
For a connected simplicial algebra $X\in s\Comm$, consider the tower:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
For any $t\geq0$, and any $q\geq0$, there is an integer $s\geq0$ such that the map $\pi_q(D^{s+t}I)X\to\pi_q(D^{t}I)X$.
\end{prop}
...
\begin{itemise}
\setlength{\parindent}{.25in}
\item $(D^r_nI)Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consiting of those megamonomials which have a doubling in each of the $\calT_1,\ldots,\calT_r$ functors.
\item $(D^r_nP^{s+1})Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consiting of those megamonomials which have a doubling in each of the $\calT_2,\ldots,\calT_r$ functors, and a power of $s+1$ in the $\calT_1$ functor.
\end{itemise}
...
\end{alignat*}
The proof for derived functors of $P^{s+1}$ is identical.
\end{proof}
\begin{cor}\label{towerWithPowers}
There is a commuting diagram of natural transformations:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
\end{cor}
\begin{cor}
When calculating $(D^rI)$ or $(D^rP^{s+1})$, all the kernels taken are of surjective maps.
\end{cor}
...
\begin{lem*}
$D^s_n(Y)\subset (\calT_1T^n_1)\cdots (\calT_sT^n_s)Y$ is the subset consiting of those megamonomials which have a doubling in each of the $\calT_1,\ldots,\calT_s$ functors.
\end{lem*}
\begin{proof}
...
\begin{prop*}
For $s,t\geq0$, $D^tP^{s+1}(X)_n\subset (\calT_1T^n_1)\cdots (\calT_tT^n_t)X_n$ is the subset consiting of those megamonomials which have a 2-fold product in each of the $\calT_2,\ldots,\calT_t$ functors, and which have a $\min\{s,2\}$-fold product in $\calT^1$.
\end{prop*}
\begin{cor*}
