...
there are natural homology equivalences of chain complexes
\[NA_*\to CA_*\to \ModDegeneracies{A}_*\]
of which the first is an inclusion, the second is a surjection, and the composite is an isomorphism. Each has homology the homotopy of $A$. If $A=\frakg$ is a simplicial Lie algebra over $\F_2$, then there are readily defined operations in $N\frakg_*$ which on homology give $\pi_*\frakg$ its algebraic structure. %Giving the inverse isomorphism could cause difficulty: for any $z\in A_n$, there is a unique degenerate element $b$ such that $z+b\in NA_n$, but one must find this `$b$'.
