...
\includeversion{prereqs for Thoughts III}
\includeversion{Thoughts on Adams Multiplicativity III}
...
It seems that to use the construction of Bousfield-Kan \cite{BK_pairings_products.pdf,BK_pairings.pdf}, we'll need to understand the construction of $D^1X$, for cosimplicial objects $X$:
\[\xymatrix@R=4mm{
D^1X\ar[r]
\ar[d]^-{\delta}
&%r1c1
...
\section{Prerequisites for Adams multiplicativity}
%\section{Adams multiplicativity}
Let $\widetilde{A}^n:=c(K\overline{Q})^{n+1}A$, for $A$ a simplicial algebra. Then $\widetilde{A}^\bullet$ is a levelwise cofibrant cosimplicial object, with a Hurewicz morphism to $Q\widetilde{A}^\bullet$. However, $Q\widetilde{A}^\bullet$ is contractible, and the cosimplicial homology coalgebra $\pi_*(Q\widetilde{A}^\bullet)$ is thus a cofree resolution of $H_*(A)$. The Hurewicz morphism is in each level an isomorphism onto the primitives:
\[\pi_*(\widetilde{A}^\bullet)\overset{\simeq}{\to}P\pi_*(Q\widetilde{A}^\bullet)\overset{}{\to}\pi_*(Q\widetilde{A}^\bullet)\]
so that the $E^2$ term is derived primitives on $H_*(A)$. As such, it should admit a product. In fact, the product should shift both gradings by 1, the internal grading $*$ and the filtration grading $\bullet$.

\subsection{Bousfield-Kan $D^1$-construction}
It seems that to use the construction of Bousfield-Kan \cite{BK_pairings_products.pdf,BK_pairings.pdf}, we'll need to understand the construction of $D^1X$, for cosimplicial objects $X$:
\[\xymatrix@R=4mm{
D^1X\ar[r]
\ar[d]^-{\delta}
...
The game will be to create a factorization
\[\xymatrix@R=4mm{
\widetilde{A}\otimes \widetilde{A}\ar[r]\ar@{~>}[d]
&%r1c1
\widetilde{A}\\%r1c2
...
are maps $\widetilde{(\alpha,0)}:S^t\to c_1(X\times Y)$ and $\widetilde{(0,\beta)}:S^{t'}\to c_1(X\times Y)$, and there is a commuting diagram:
...
The square at the bottom of the diagram commutes as the horizontals are maps of homology Lie coalgebras, and we can see that the triangle commutes because we understand the coLie algebra structure of $H_*(C_{t,t'})$. As all of the maps in the six-arrow commuting diagram  are natural, we may check it on the universal example: $(\imath_t,\imath_{t'})\in\hom_{s\Comm}(S^t,S^t)\times\hom_{s\Comm}(S^{t'},S^{t'})$. That is, it's enough to check that the following diagram commutes:
\[\mathclap{\xymatrix@R=4mm{
...
\section{The crucial commuting diagram}
...
Here, $\overline{D}^1X$ is the pullback of the first column in the following diagram:
\[\xymatrix@R=4mm{
&%r1c1
0\ar[d]
\ar[r]
...
This diagram constructs both a map $\overline{\mu}:X^\fraks\otimes X^\fraks\to(\overline{D}^1X)^\fraks$ and a zig-zag of weak equivalences from $\overline{D}^1X$ to $D^1X$. This also shows that the small square in the top right corner of the diagram of interest commutes.

At this moment, it is opportune to extend the diagram of interest a little. We change the source to $\hom_{s\Comm}(S^t,X^\fraks)\times\hom_{s\Comm}(S^{t'},X^\fraks)$, removing the symbols `$\hom_{s\Comm}$' for abbreviation. We'll decorate some of the arrows for emphasis, alhough all of the arrows in this diagram have already been defined:
%\[\xymatrix@R=4mm{
...
We must be getting close. Indeed, write `$\textup{wavy}$' for the composite of the wavy maps, and write $g\in QC_{t,t'}$ for the a representative of $g\in\pi_{\frakt+1}(QC_{t,t'})$. Choosing $(\alpha,\beta)\in (S^t,X^\fraks)\times (S^{t'},X^\fraks)$, a representative for $\textup{wavy}(\alpha,\beta)$ is given by the image of $g$ under the composite
...
%r2c2
&A}\]
The point then is that $g\in CS^\frakt$ is a chain with the property that $\partial(g)$ is the image of the fundamental class in $S^\frakt$. In $C_{t,t'}$, $\partial(g)$ is identified with $EZ^\mu(\imath_t,\imath_{t'})$, which is decomposable, so that $g$ becomes a cycle in $QC_{t,t'}$, representing the $(t+t'+1)$-dimensional homology class. As this $g$ is in the image of the map $CS^\frakt\to C_{t,t'}$, it maps to $0$ in $A$, and on homology, $g$ is annihilated by the top (and bottom) composite.
\begin{lem}
The composite 
...
\ar@/^1em/[rrrd]^-{\widetilde{\epsilon\alpha}\sqcup\widetilde{\epsilon\beta}}
&%r1c1
&%r1c2
&%r1c3
\\%r1c4
...
\begin{thm}
The monster diagram commutes.
\end{thm}
\begin{proof}
We only need to show that the diagram commutes on a single $(\alpha,\beta)$ for each pair of homotopy classes that $\alpha,\beta$ represent. In particular, one can choose $\alpha:S^t\to X^\fraks=cX^\fraks_\textup{fl}$ in such a way that $\alpha=\widetilde{\epsilon\alpha}$ (by choosing $\alpha$ to be the canonical lift of whatever homotopy class $S^t\to X^\fraks_\textup{fl}$ you're interested in). The same applies to $\beta$. In summary, suppose given $\alpha$ and $\beta$ such that $\alpha=\widetilde{\epsilon\alpha}$ and $\beta=\widetilde{\epsilon\beta}$. Then we must show that $\partial_\textup{conn}(\textup{dotted}(\alpha,\beta))=\overline{\mu}_*(\textup{sEZ}(\alpha,\beta))$.

All of the previous discussion together has shown $\textup{dotted}(\alpha,\beta)$ is the image in $QX^{\fraks+1}_\textup{fl}$ of the cycle $g\in Qc_{t,t'}$ under the top row of the following commuting diagram:
\[\xymatrix@R=4mm{
...
c(X^\fraks)^2\ar[r]^-{h}
&%r1c3
X^\fraks\makebox[0cm][l]{$\,\ni\partial_\textup{conn}(\textup{dotted}(\alpha,\beta))$}
&%r1c4
\\%r3c5
...
\[0\to\overline{D}^1X^\fraks\to X^\fraks\overset{\epsilon d^0}{\to} X^{\fraks+1}_\textup{fl}\to 0,\]
 confirming that $\partial_\textup{conn}(\textup{dotted}(\alpha,\beta))$ is the image under $\alpha\sqcup\beta$ of $\textup{EZ}^\mu(\imath_t,\imath_t')$, completing the proof.
\end{proof}
