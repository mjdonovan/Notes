...
Let's list the various maps we'll need.
...
\[N^s\pi_tX\overset{\Gamma^i}{\to}N^{s+i}((\pi_tX)^{\otimes2})\overset{N^{s+i}(\Delta)}{\to}N^{s+i}\pi_{2t}(X\wedge X)\to N^{s+i+1}\pi_{2t+1}(X)\]
...
\[x\mapsto D^{t-i}(x\otimes x)+D^{t-i+1}(x\otimes dx)\]
...
\textbf{See that $\Sq^0=0$ as we're looking at Lie coalgebra homology.}
...
\tau:F_{p}CX_n&\to F_{2p}(CX\otimes CX)_n
\end{align*}
...
\end{proof}
\subsection{Attempted homotopy of $\rho\Delta D^{t-i}\tau$}
Since we want the steenrod operations to be the target of the differential, and the not-so-unstable $\delta$ operations to be the source, we really want something like $\textup{`steen'}(x)\equiv \Sigma dx+d\Sigma x$ modulo high filtration for some cycle function $\Sigma$. Here's a clumsy and thoughtless attempt to homotope $\rho\Delta D^{t-i}\tau y$. The equations
...
\Delta D^{t-i}
&=
(\Delta_0(1+T)+\phi_0)D^{t-i}%
\\
&\sim
\Delta_1(1+T)D^{t-i+1}+\phi_1D^{t-i+1}+\phi_0D^{t-i}%
\\
% Left hand side
% Relation
&\sim
% Right hand side
\Delta_r(1+T)D^{t-i+r}+\phi_rD^{t-i+r}+\cdots+\phi_0D^{t-i}%
% Comment
\end{alignat*}
where the homotopy used was
\[\Delta_0D^{t-i+1}+\cdots +\Delta_{r-1}D^{t-i+r}\]
I guess that if we push this all the way to $r=t-i$, you've used a homotopy with leading term $\Delta_{t-i-1}D^0$, which recalls the formula for $\delta_{i+1}$ on an element of $E^2_{t,s}$, so that $\delta_{i+i}$ has a differential to whatever Steenrod operation we were thinking about. Who knows?
