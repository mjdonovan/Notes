...
\end{prop}
\begin{proof}
First, a construction. For any \emph{bar-admissible} monomial $\BarMonomial{I}{\textbf{k}}{x}\in\Boverline_{r,\ell}$ with $\ell>r$, choose the unique integer $e$ such that $k_r=k_{r-1}=\cdots =k_{r-e+1}=1<k_{r-e}$, so that
...
Then, as remarked in the proof of \cite[Thm 5.3]{PriddyKoszul.pdf}, a straightforward verification shows that for any bar-admissible monomial $m\in\Boverline_{r,\ell}$ with $\ell>r$, $\partial\Gamma(m)+\Gamma\partial(m)\equiv m$ modulo monomials strictly lesser than $m$.
...
As long as each of the $\Q(I)x$ are cycles, they will form the dual basis for $H_{rr}\Boverline$. %, as in the sum defining $\Q(I)x$, the only $\SqShift$-admissible $J$ appearing is $J=I$. 
...
\begin{prop}
Suppose that $X$ is any object in $\nontop{1}$, the Koszul complex $\Koverline_*X$ is the subcomplex of $\Boverline_*(\forget{\GR{}},\Fr{\nontop{1}},X)$ with basis the classes $P(I)x$, where 
\begin{itemize}
\setlength{\parindent}{.25in}
...
The inclusion of $\Koverline_*X$ into $\Boverline_*(\forget{\GR{}},\Fr{\nontop{1}},X)$ is a homology equivalence, the restriction of the differential to $\Koverline_*X$ is given by the following formula:
...
Now for $X\in\nontop{n}$, with $n\geq2$, the Koszul complex $\Koverline_*(X)$ was defined as a subcomplex of $\Boverline_*(X):=\ModDegeneracies{\Ind{\nontop{}}\BarConst{\nontop{}}(X)}_*$. We'll explain this a little further for clarity, in the $n\geq2$ case%footnote:
...
is spanned by the monomials of proposition \ref{PropDescnOfReducedBarConstruction}, except that here we \emph{allow} empty bars. The restriction that no bar should be empty is the result of taking the quotient by degenerate simplices to form $\Boverline_*(X)$. The face maps are defined by
\begin{alignat*}{2}
d_0([e_r|\cdots |e_1]x)
&=
[e_r|\cdots |e_2]e_1x%
...
% Right hand side
0% Comment
&\qquad&\text{(unless $e_r$ is an empty bar)}
\end{alignat*}
...
This condition implies that $d_kz$ is zero in ${\Ind{\nontop{}}(\BarConst{\nontop{}}X)}_{r-1}$ for each $k>0$, not just zero in the quotient. Thus $z$ lies in the subcomplex $NB_r$.
...
\end{alignat*}
Here we've introduced the notation $\Boverline_*^\textup{L}(X)$ for $N(\Ind{\nontop{}}(\BarConst{\LL{}}(X)))_*$, on which we'll define operations inducing the $\PiAlgebra{\PRLie{n}}$ structure on $(\derived\Ind{\nontop{}})X$.
...
\item $d_r:(\Ind{\nontop{}}\BarConst{\LL{}}(X))_r\to (\Ind{\nontop{}}\BarConst{\LL{}}(X))_{r-1}$ is induced by the natural transformation $\Fr[out]{\PRLie{n}}\Fr[r]{\LL{n}}\to \Fr[out]{\PRLie{n}}$ which kills nontop $\Q^i$ terms and identifies the top $\Q$ with $\restn{}$ (and when $n=1$, kills all $P$ operations);
...
\end{Omitted}
Before we work with these definitions, we include a handy technical lemma, which is immediate from the simplicial relations:
\begin{lem}
...
\end{Omitted}
\begin{prop}
Suppose that $z\in Z\Koverline_p(X)_{a_n,\ldots,a_1}$. Then:
\begin{itemize}\squishlist
\setlength{\parindent}{.25in}
\item  if $n\geq2$, $\Q^k\overline{z}=0$ for $1\leq k\leq p$;
\item  if $n=1$, $\Q^k\overline{z}=0$ for $2\leq k\leq p$; and
\item if $n=1$, write $z\in Z\Koverline_p(X)_{a_1}$ as a sum 
\[z=\sum_{j}P(I^{(j)})x^{(j)}\]
...
The same argument shows that $d_{p+k+1}a$ is the above representative for $\Q^k\overline{z}$, and that $d_{p+k}a=0$. We are left to calculate $d_ia$ for $0\leq i<p+k$, where 
...
\begin{lem}
Suppose that $z\in Z\Koverline_p(X)_{a_n,\ldots,a_1}$, where not all of $a_n,\ldots,a_2$ are zero. 
\begin{itemise}
\setlength{\parindent}{.25in}
...
\[z=\sum_{j}Q(I^{(j)})x^{(j)}\]
is a cycle in $\Koverline_p(X)$, and that for all $j$, $\Q^ix^{(j)}=0$ whenever $i\geq i^{{(j)}}_1$. Then $\Q^0\overline{z}=0$.
\end{lem}
\begin{proof}
