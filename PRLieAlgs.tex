...
\end{cases}\]\textbf{Check that it still works with the primes.}
\end{cor}
\begin{proof}
...
\end{gather*}
\end{proof}
\subsection{Some observations on $\textup{DpD}'$}
\begin{alignat*}{2}
\textup{DpD}'(x)
&=
\rho(\mathbb{D}_{n-i}(x\otimes x)+\mathbb{D}_{n-i-1}(dx\otimes x))\\
&\ \quad {}+\rho(\Delta_{n-i}\twist^{n-i}D^0(x\otimes x)+\Delta_{n-i-1}\twist^{n-i-1}D^0(x\otimes dx))%
\\
&=
\sum_{j\geq1}\Delta_{n-i+j}\sigma^{n-i+j}D^j(x\otimes x)+\sum_{j\geq1}\Delta_{n-i-1+j}\sigma^{n-i-1+j}D^j(dx\otimes x)\\
&\ \quad {}+\Delta_{n-i-1}\twist^{n-i-1}D^0d(x\otimes x)
\end{alignat*}
This calculation shows that if $x^0\in X^{0}_n$ is a cycle, (i.e.\ if $d^h(x^0)=0$ and $d^v(x^0)=0$), then $\textup{DpD}'(x^0)=0$.

Suppose now that $x$ is a permanent cycle, and $i<n$. Then since the spectral sequence is convergent, one can write $x=x^0+dy$, for $x^0\in X^0_n$ a cycle, and some $y\in CX_{n+1}$. Now the previous calculations show that $0=\textup{DpD}'_i(x^0)$ and $\textup{DpD}'_i(x^0+dy)$ differ by a boundary, namely the sum of the boundaries of the following three quantities:
\[(\mathbb{D}_{n-i-1}+\Delta_{n-i-1}\twist^{n-i-1}D^0)(x^0\cdot dy)=0;\]
\[(\mathbb{D}_{(n+1-i)-1}+\Delta_{(n+1-i)-1}\twist^{(n+1-i)-1}D^0)(dy\cdot y);\]
\[(\mathbb{D}_{n+1-i}+\Delta_{n+1-i}\twist^{n+1-i}D^0)(y\cdot y).\]
The first quantity equals zero since $\{D^k\}$ is special, and the sum of the other two quantities is \textbf{similar to} $\textup{DpD}'_i(y)$, lying in filtration at least 1. Thus $\textup{DpD}'_i(x)$ is hit by some differential.

On the other hand, if $d_rx\neq0$ and $i\leq n$, then \ref{dvsDEL} shows that $d_r\delta_ix=\delta_id_rx$. In case $i>n$, something similar happens. Basically all the $\delta$ and $\Sq$ operations are doomed! They're all correct at $E^2$ except on the 0-line, where they're all messed up.
%\subsection{Thought}
