...
\end{cases}
\]
Now suppose that $x\in E^r_{-s,t}$. One may try to define $\delta_i(x)\in E^?_{-s,t+i}$ for $2\leq i<t$, writing $n=t-s=|x|$. I believe that the correct formula may be
\[x\mapsto \textup{DEL}_{i}(x):=\rho(\mathbb{D}_{n-i}(x\otimes x)+\mathbb{D}_{n-i-1}(dx\otimes x)).\]
\subsubsection*{Correctness at $E^2$}
As $E^1$ is obtained from $E^0$ by taking homology in the simplicial direction, there are $\delta$-operations on $E^1$. They are unstable with respect to internal degree $t$, not with respect to total degree $n$. All of these operations are linear except for the top operation, and commute with the differentials in the cosimplicial direction. Thus, we obtain operations $\delta_i^\textup{alg}:E^2_{-s,t}(X)\to E^2_{-s,t+i}(X\otimes_{\Sigma_2}X)$ for $2\leq i<t$.

At the chain level, what we're doing here is taking a `bidirectional cycle', i.e.\ $z\in C_{-s,t}X$ such that $\partial_\textup{sim}z=0$ and $d_\textup{cos}z=0$, and forming $\rho\Delta_{t-i}(z\otimes z)$ after viewing $z\otimes z$ as lying in $C(X\otimes_\textup{v}X)_{-s,2t}$.
