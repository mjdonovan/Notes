...
\ar[r]
&%r1c2
\Lambda V'X\ar[d]^-{\lambda}
&%r1c3
\Lambda V'X\ar[l]^-{\Lambda(\epsilon)}
\ar[d]^-{\lambda}
\\%r1c4
X\otimes X\ar[ur]^-{0}
\ar[dr]_-{\mu}
...
\pi_{\frakt+1}(V'X)\ar[l]^-{\cong\textup{-ish}}_-{\partial_\textup{conn}}
\\%r1c4
...
We'll here construct the algebraically defined map $\xi_\textup{alg}$ at the chain level. Note that this map is \emph{not} a cosimplicial map $(HX)^{\times2}\to HVX$, but instead, induces a chain map on the normalised complexes of primitives.
...
%\ar[ddd]
&%r4c3
c(V'X^\fraks\Times V'X^\fraks)\ar[r]_-{h}
%\ar[dd]^-{\beta}
&%r4c4
VX^\fraks\\\\%r4c5
...
\[h:\left[c((X^\fraks)^2)\overset{c(\epsilon^2)}{\to}c((V'X^{\fraks-1})^2)\overset{c(\textup{add})}{\to}c(V'X^{\fraks-1})=X^\fraks\right]\]
\[h':\left[(VX^\fraks)^2\overset{\epsilon^2}{\to}(V'X^\fraks)^2\overset{\textup{add}}{\to}V'X^\fraks\right]\]
\begin{lem}
...
&%r1c2
\pi_{\frakt}(\overline{D}^1X^\fraks)&%r1c3
\pi_{\frakt+1}(V'X^{\fraks})\ar[l]_-{\partial_\textup{conn}}
\ar@{~}[dd]^-{\textup{zig-zag}}_-{\cong}
\\%r1c4
&%r2c1
&%r2c2
\pi_{\frakt+1}(QV'X^{\fraks})\ar@{..>}[ur]_-{=}&%r2c3
%\pi_{\frakt+1}(X^{\fraks+1})\ar[u]_-{\cong}
%\ar[d]^-{\cong}
\\%r2c4
%\textup{Pr}_t(HX^{\fraks})\otimes \textup{Pr}_{t'}(HX^{\fraks})\ar[r]^-{j}
...
\[Qc(X^\fraks\times X^\fraks)\overset{Q\overline{\xi}_\textup{res}}{\to}QVX^{\fraks}\overset{Q\epsilon}{\to}QV'X^{\fraks}=V'X^{\fraks}.\]
...
&%r2c2
c(V'X^{\fraks-1}\times V'X^{\fraks-1})\ar[r]_-{c(\textup{add})}
&%r2c3
c(V'X^{\fraks-1})\makebox[0cm][l]{\,$=X^\fraks$}%r2c4
}\qedhere\]
\end{proof}
...
\[0\to\overline{D}^1X^\fraks\to X^\fraks\overset{\epsilon v}{\to} V'X^\fraks\to 0,\]
