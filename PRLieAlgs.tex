...
\newcommand{\TruncAugSimplex}[1]{{\Delta^{+}_{\geq#1}}}
...
Here, $\hotfib(X|_{\FullCube{n}})$ is defined to be the homotopy fiber of the map $X^{-1}\to \holim(X|_{\PuncCube{n}})$. This is the first definition of the homotopy fibre of a cube that Goodwillie gives. This definition is is equivalent, however, to an iterative definition which we'll give now. Suppose that one has defined the notion of total fibre for diagrams indexed by $\FullCube{n-1}$. Then for $C:\FullCube{n}\to s\algCat$, one defines
...
\textbf{Put $C=X|_?$ and then comment that the tower maps become the inclusions of the hofib in this definition.}
