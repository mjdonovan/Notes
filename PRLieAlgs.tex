...
\usepackage[paperheight=200mm]{geometry}%remove me after editing
\topmargin=-80pt%remove me after editing
\textheight=500pt
...
\Ixcludeversion{Adams Muliplicativity}
...
\excludeversion{Lie Steenrods satisfy the Adem relation}
...
\pagebreak
\section{\st{Categories of interest}}\label{ChapterCategoriesOfInterest}
...
\pagebreak
\section{\st{Grothendieck spectral sequences}}
...
\pagebreak
\section{\st{Multiplicativity of the Grothendieck spectral sequence}}
...
\pagebreak
\section{\st{Kos action on $\mathbb{R}\textup{Pr}$ $=$ $E_2$-id of horizontal operations?}}
...
\pagebreak
\section{\st{Multiplicativity of the Adams sseq}}
%\section{Adams multiplicativity}
...
\pagebreak
\section{\st{Operations in the Adams spectral sequence}}
\begin{thm}
...
\subsection{\st{$\Sq$-Adem relations for lifted classes}}
...
\pagebreak
\section{\st{Unstable Koszul complexes for $\derived\Ind{\nontop{}}$} \label{sectionOnUnstableKoszulComplexes}}
...
\pagebreak
\section{\st{Lie algebra homology and low dimensional calculations}}
...
\pagebreak
\section{\st{Lie Steenrod operations via a Goerss map}}
...
Fix a cocycle $\alpha\in (N_{n}\textup{Ab}(\frakg_\bullet))^*$. %We may assume without loss of generality (or are we \textbf{forced} to assume) that  $\alpha$ evaluates to zero... wait... wtf am I talking about?
...
The point now is that there is a commuting diagram%footnote:
...
\ar[d]^-{r}&%r1c1
N_{n+k-1}^\textup{h}N_2^\textup{v}\textbf{B} \ar[r]^-{D^\textup{v}_0\circ\psi}&%r1c2
N_{n+k-1}^\textup{h}(N_1^\textup{v}\textbf{B}\otimes N_1^\textup{v}\textbf{B})\ar[d]^-{r\otimes r}
\\%r1c3
N_{n+k}^\textup{h}\textup{Ab}(\frakg) \ar[r]^-{\textup{Ab}(\xi)}&%r1c1
N_{n+k-1}^\textup{h}\textup{Ab}(\frakg\wedge \frakg) \ar[r]^-{j}&%r1c2
...
N_{n+k}^\textup{h}N_1^\textup{v}\textbf{B} \ar[d]^-{r}
 \ar[r]^-{D^\textup{v}_0\psi (d^\textup{h}_0h+hd^\textup{h}_0)}&%r1c2
...
\\%r1c3
N_{n+k}^\textup{h}\textup{Ab}(\frakg) \ar[r]^-{j\circ\textup{Ab}(\xi)}&%r1c2
N_{n+k-1}^\textup{h}(\textup{Ab}(\frakg)\otimes \textup{Ab}(\frakg))
...
%\[\bigoplus_{n_1+n_2=2n}
%N^\textup{h}_{n_1}N^\textup{v}_1\textbf{B}\otimes N^\textup{h}_{n_2}N^\textup{v}_1\textbf{B}
%\to
%N^\textup{h}_nN^\textup{v}_1\textbf{B}\otimes N^\textup{h}_nN^\textup{v}_1\textbf{B}\]
...
\pagebreak
\section{\st{Structure on the derived functors $(\derived\Ind{\nontop{}})X$} }
