...
\includeversion{Adams Muliplicativity}
\excludeversion{letter to Dwyer}
...
[\textbf{I briefly believed:} It is in fact possible to iterate this construction, to naturally lift `products of products' two steps higher in filtration, even $\Sigma_4$-equivariantly. That is, there is a diagram
\[\xymatrix@R=4mm{
X^{\otimes4}_{\Sigma_4}\ar@{..>}[rr]
&%r1c1
&%r1c2
D^1\overline{D}^1X\ar[d]^-{\delta}
\\%r1c3
&%r2c1
\overline{D}^1X\otimes \overline{D}^1X\ar[d]^-{\delta\otimes\delta}
\ar@{-->}[ru]
\ar[dru]_{\ \ \mu}
&%r2c2
\overline{D}^1X\ar[d]^-{\delta}
\\%r2c3
X^{\otimes2}\otimes X^{\otimes2}\ar[ru]
\ar[udr]_-{\mu\otimes\mu}
\ar@{->>}[uu]
&%r3c1
X\otimes X\ar[dru]^{\ \ \mu}
&%r3c2
X%r3c3
}\]
The dashed arrow is the map to be constructed, while the dotted arrow is to demonstrate that the composite $X^{\otimes4}\to D^1\overline{D}^1X$ is $\Sigma_4$-equivariant. In order to construct the dashed arrow, one starts with the commuting diagram
\[\xymatrix@R=10mm@C=20mm@!0{
&%r1c1
(\Lambda\overline{V}X)^{\otimes2}\ar[rrrd]|-(.7){0}
\ar[d]_-{\lambda^{\otimes2}}
\ar[dddr]|-(.8){\mu}
&%r1c2
&%r1c3
&%r1c4
\\%r1c5
X^{\otimes2}\ar[rrrd]|-(.8){?}
\ar[r]^-{\overline{v}^{\otimes2}}
\ar[dddr]|-(.8){\mu}
&%r2c1
(\overline{V}X)^{\otimes2}\ar[rrrd]|-(.7){0}
\ar[dddr]|-(.8){\mu}
&%r2c2
&%r2c3
&%r2c4
\Lambda V\Lambda\overline{V}X\ar[d]_-{\Lambda V(\lambda)}
\ar[dddr]|-(.8){\lambda}
\\%r2c5
&%r3c1
&%r3c2
&%r3c3
\Lambda VX\ar[r]_-{\Lambda V(\overline{v})}
\ar[dddr]|-(.8){\lambda}
&%r3c4
\Lambda V\overline{V}X
\ar[dddr]|-(.8){\lambda}
\\%r3c5
&
&%r1c1
\Lambda\overline{V}X\ar[rrrd]|-(.8){v}
\ar[d]^-{\lambda}
&%r1c2
&%r1c3
&%r1c4
...
&
&%r3c2
VX\ar[r]_-{V(\overline{v})}
&%r3c3
V\overline{V}X
}\]
\textbf{end of past belief.}]

\[\xymatrix@R=9mm@C=14mm@!0{
X^{\otimes 4}\ar[d]^-{\mu\otimes\mu}
\ar[drrrr]^-{0}
&%r1c1
&%r1c2
&%r1c3
...
X^{\otimes2}\ar[dr]^-{0}
\ar[dddr]_-{\mu}
\ar[rr]^-{\overline{v}^{\otimes2}}
&%r2c1
&%r2c2
(\overline{V}X)^{\otimes2}\ar[dr]^-{0}
\ar[dddr]_-{0}
&%r2c3
&%r2c4
(\Lambda\overline{V}X)^{\otimes2}\ar[dr]^-{0}\ar[ll]^-{\lambda^{\otimes2}}
\ar[dddr]_-{0}
&%r2c5
\\%r2c6
&%r3c1
...
&%r3c2
&%r3c3
\Lambda V\overline{V}X\ar[d]^-{\lambda}
&%r3c4
&%r3c5
\Lambda V\Lambda\overline{V}X\ar[ll]_-{\Lambda (V(\lambda))}
\ar[d]^-{\lambda ? \Lambda(\lambda)}
&\textup{(r1)}
\\%r3c6
&%r3c1
\overline{V}X
...
&%r3c4
&%r3c5
V\Lambda\overline{V}X\ar[ll]_-{V(\lambda)}
&\textup{(r2)}
\\%r3c6
&%r3c1
X\ar[u]_-{\overline{v}}
...
&%r3c2
&%r3c3
\overline{V}X\ar[u]_-{v}
&%r3c4
&%r3c5
\Lambda\overline{V}X\ar[ll]_-{\lambda}
\ar[u]_-{v}
&\textup{(r3)}
\\%r3c6
&%r3c1
...
&%r3c4
&%r3c5
\Lambda VX\ar[ll]_-{\lambda}
\ar[u]_-{\Lambda(\epsilon)}
&\textup{(r4)}\\
&\textup{(A)}&&\textup{(B)}&&\textup{(C)}
}\]
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item A124 calculates $\overline{D}^1X$
\item B124 calculates $V\overline{D}^1X$
\item C124 calculates $\Lambda V\overline{D}^1X$.. if we mean $\Lambda(\lambda)$ near (r1) and (r2)!
\item ABC1 calculates $\Lambda D^1(\overline{V}X)$
\end{enumerate}
...
&%r3c4
&%r3c5
\Lambda V\Lambda\overline{V}X\ar[ll]^-{\Lambda (V(\lambda))}
\ar[d]^-{\Lambda(\lambda)}
&\textup{(r1)}
\\%r3c6
&%r3c1
...
&%r3c4
&%r3c5
V\Lambda\overline{V}X\ar[ll]^-{V(\lambda)}
&\textup{(r2)}
\\%r3c6
&%r3c1
X\ar[u]_-{\overline{v}}
...
\setlength{\parindent}{.25in}
\item A124 calculates $\overline{D}^1X$
\item B124 calculates $V\overline{D}^1X$... maybe!
\item C124 calculates $\Lambda V\overline{D}^1X$.. if the last one works!
%\item 
%\item A123 calculates $\overline{D}^1X$
%\item B123 calculates $D^1\overline{V}X$
%\item C123 calculates $\Lambda V\overline{D}^1X$.. maybe!
%\item ABC1 calculates $\Lambda D^1(\overline{V}X)$
\end{enumerate}
So the idea is
\[X^{\otimes 4}\to (\overline{D}^1X)^{\otimes2}\to \textup{pb}(\textup{r0})\to \lim\textup{ABC123}\overset{}{\from}D^1\overline{D}^1X.\]
...
\end{prop}
\begin{proof}
Since the spectral sequence converges, $x$ can be written as $x_0+dy$ for $x_0\in NX_{0,n}$ such that $dx_0=0$ and $y\in NX_{n+1}$. Now $x\times x$ is represented by
\[\rho\Delta D^0(x_0\otimes x_0)+\rho\Delta D^0(dy\otimes x_0+x_0\otimes dy+dy\otimes dy).\]
Now the first term here vanishes, as
\[\rho\Delta D^0(x_0\otimes x_0)=\rho(\phi_0 D^0(x_0\otimes x_0)+\Delta_0 D^0(x_0\otimes x_0)+\Delta_0 TD^0(x_0\otimes x_0)),\]
and $\phi_0D^0(x_0\otimes x_0)=0$ as $n>0$, and $TD^0(x_0\otimes x_0)=D^0(x_0\otimes x_0)$ since $\{D^k\}$ is special. The remaining term may be expressed as the boundary
...
\subsection{Chain level operations doubling filtration}
For $x\in CX_{n}$, and any $i\geq2$, define
\[\textup{DUB}_i(x):=\rho(\Delta_{n-i}D^0(x\otimes x)+\Delta_{n-i-1}D^0(x\otimes dx)).\]
%and (setting $i=n$) define 
%\[\textup{DUB}_n(x):=\rho\Delta_{0}D^0(x\otimes x).\]
These chain level operations vanish whenever $i>n$. %may or may not give operations on the total complex which are compatible with the $\delta$-operations on the target... These double filtration, and are \textbf{presumably} zero on $E^2_{-s,t}$ when $s>0$.
...
In the case $i=n$, the second line in the expression for $d(\textup{DUB}_i(x))$ does not appear. The first missing term was identified in the previous case as ``$\textup{DUB}_i(dx)$'', which no longer appears on the LHS when $i=n$. The second missing term
...
\end{lem}
\begin{proof}We'll need to apply the formula for $d\Delta_\alpha+\Delta_\alpha d$ to $\twist^\alpha D^\beta(z)$, for $\alpha$, $\beta$ such that $\alpha-\beta=r$. %, where $z\in (CX\otimes CX)_{-s,t}$ is the element to which we're applying the above equation.
...
\\
&=
\twolinesum{\alpha-\beta=r}{\alpha\geq0,\,\beta\geq0}\left(((1+\twist)\Delta_{\alpha+1}+\phi_{\alpha+1})\twist^\alpha D^\beta+\Delta_\alpha\twist^\alpha(1+\twist) D^{\beta-1}\right)(z)%
\\
&=
...
\\
&\ \ \ \ \ \ \ \ \ \ \ +
((1+\twist)\Delta_{0}+\phi_{0})\twist D^{-r-1}(z)+\Delta_{r+1}(1+\twist)\twist^\alpha D^{0}(z)%
\end{alignat*}
...
\[d(\textup{DEL}_i(x))+ \textup{DEL}_i(dx)=\begin{cases}
\textup{SQ}^{t-i+1,s}(x),&\textup{if }n+2\leq i \leq t-1;\\
\rho\Delta D^1(x\otimes dx),&\textup{if }i=n+1;\\
d(\textup{DUB}_i(x)),&\textup{if }i=n;\\
d(\textup{DUB}_i(x))+\textup{DUB}_i(dx),&\textup{if }2\leq i\leq n-1.
\end{cases}\]
If, moreover, $x\in Z^r_{-s,t}$:
\[d(\underbrace{\textup{DpD}_i(x)}_{\in F_s})+ \underbrace{\textup{DpD}_i(dx)}_{\in F_{s+r}}=\begin{cases}
\textup{SQ}^{t-i+1,s}(x)\in F_{s+(t-i+1)},&\textup{if }n+2\leq i \leq t-1;\\
\rho\Delta D^1(x\otimes dx)\in F_{s+(s+r-1)},&\textup{if }i=n+1;\\
...
\end{cases}
\end{alignat*}
Finally, when $i=n+1$, summand B equals 
\[\rho\Delta_0(1+T)D^0(x\otimes x)=\rho(1+\twist)\Delta_0D^0(x\otimes x)=\rho\Delta D^0(x\otimes x),\]
since $\rho\phi_0D^0(x\otimes x)$ vanishes as $t>i\geq2$. This cancels with the first summand in the definition of $\textup{SQ}^{t-i+1,s}(x)=\textup{SQ}^{s,s}(x)$, and the result follows.
%\begin{alignat*}{2}
%&
%\rho\Delta(D^{i-n-1}(x\otimes x)+D^{i-n}(x\otimes dx))+\\
%&\ \ +\rho\Delta_{n-i+1}(1+\twist)D^0(x\otimes x)+\rho\Delta_{n-i}(1+\twist)D^0(dx\otimes x)+\\
...
\subsection{Rewriting $d\textup{DEL}_i+\textup{DEL}_id$}
Let's examine the quantity $\rho\Delta_{n-i+1}(1+\twist)D^0(x\otimes x)+\rho\Delta_{n-i}(1+\twist)D^0(dx\otimes x)$
\begin{alignat*}{2}
&
\rho\Delta_{n-i+1}(1{+}\twist)D^0(x\otimes x)+\rho\Delta_{n-i}(1{+}\twist)D^0(dx\otimes x)\\
=&\rho(1{+}\twist)\Delta_{n-i+1}D^0(x\otimes x)+\rho\Delta_{n-i}(1{+}\twist)D^0(dx\otimes x)\\
=&\rho(\Delta_{n-i}d+d\Delta_{n-i}+\psi_{n-i+1})D^0(x\otimes x)+\rho\Delta_{n-i}(1{+}\twist)D^0(dx\otimes x)\\
=&\rho\Delta_{n-i}D^0(dx\otimes x+x\otimes dx) + d\Delta_{n-i}D^0(x\otimes x)+\Delta_{n-i}D^0(dx\otimes x)+\Delta_{n-i}\twist D^0(dx\otimes x)\\
=&\rho\Delta_{n-i}D^0(x\otimes dx) + d\Delta_{n-i}D^0(x\otimes x)+\Delta_{n-i}\twist D^0(dx\otimes x)\\
=&d\Delta_{n-i}D^0(x\otimes x)+\rho(1{+}\twist)\Delta_{n-i}D^0(x\otimes dx)  \\
=&d\Delta_{n-i}D^0(x\otimes x)+\rho d\Delta_{n-i-1}D^0(x\otimes dx)+\Delta_{n-i-1}dD^0(x\otimes dx)\\
=&d(\Delta_{n-i}D^0(x\otimes x)+\Delta_{n-i-1}D^0(x\otimes dx))+\Delta_{n-i-1}D^0(dx\otimes dx)
\end{alignat*}
which is exactly what we expected. This was just a tedious back-verification.


\subsection{$\textup{DEL}+\textup{DUB}$ preserves homologies of cycles}
Suppose that $x$ is a permanent cycle of dimension $n$, and $y$ has dimension $n+1$.
\begin{prop}
$\textup{DpD}_i(x)\sim \textup{DpD}_i(x+dy)$ for $i<n$.
\end{prop}
\begin{proof}Using the symbol `$\cdot$' to abbreviate `$\otimes$':
\small
\begin{alignat*}{4}
...
d\Delta_{n-i+1}\twist D^0(y\cdot y)&=
\Delta_{n-i+1}\twist D^0d(y\cdot y)&&+
(1{+}\twist)\Delta_{n-i+2}\twist D^0(y\cdot y)&&+
\cancel{\phi_{n-i+2}\twist D^0(y\cdot y)}
\end{alignat*}
\normalsize
Now we'll use coordinates to denote the image under $\rho$ of an entry in the above $6\times3$ grid, writing, for example, B4 for the quantity $\rho((1{+}\twist)\Delta_{n-i}\twist D^0(x\cdot dy))$. Then the result follows from the following observations:
\begin{gather*}
\textup{A2}+\textup{B1}=\rho\mathbb{D}_{n-i}((x+dy)\cdot(x+dy)-x\cdot x);\\
\textup{A5}+\textup{B4}+\textup{C1}=\rho \Delta_{n-i}D^0((x+dy)\cdot(x+dy)-x\cdot x);\\
\textup{A3}+\textup{B2}=\textup{A6}+\textup{B5}+\textup{C2}= \textup{B6}+\textup{C3}=0.\qedhere
\end{gather*}
\end{proof}
...
\begin{proof}Using the symbol `$\cdot$' to abbreviate `$\otimes$', and $\widetilde{\twist}$ for $\twist^{n-i}$:
\small
\begin{alignat*}{4}
d\mathbb{D}_{n-i-1}(x\cdot dy)&=
\cancel{\mathbb{D}_{n-i-1}(dx\cdot dy)}&&+
...
\cancel{(1{+}\twist)\mathbb{D}_{n-i+2}(y\cdot y)}&&+
\Delta_{n-i+2}(1{+}\sigma)D^0(y\cdot y)\\
d\Delta_{n-i-1}\twist \widetilde{\twist}D^0(x\cdot dy)&=
\cancel{\Delta_{n-i-1}\twist \widetilde{\twist}D^0(dx\cdot dy)}&&+
(1{+}\twist)\Delta_{n-i}\twist \widetilde{\twist}D^0(x\cdot dy)&&+
\cancel{\phi_{n-i}\twist \widetilde{\twist}D^0(x\cdot dy)}\\
d\Delta_{n-i}\widetilde{\twist}D^0(dy\cdot y)&=
\Delta_{n-i}\widetilde{\twist}D^0(dy\cdot dy)&&+
(1{+}\twist)\Delta_{n-i+1}\widetilde{\twist}D^0(dy\cdot y)&&+
\cancel{\phi_{n-i+1}\widetilde{\twist}D^0(dy\cdot y)}\\
d\Delta_{n-i+1}\twist \widetilde{\twist}D^0(y\cdot y)&=
\Delta_{n-i+1}\twist \widetilde{\twist}D^0d(y\cdot y)&&+
(1{+}\twist)\Delta_{n-i+2}\twist \widetilde{\twist}D^0(y\cdot y)&&+
\cancel{\phi_{n-i+2}\twist \widetilde{\twist}D^0(y\cdot y)}
\end{alignat*}
\normalsize Now we'll use coordinates to denote the image under $\rho$ of an entry in the above $6\times3$ grid, writing, for example, B4 for the quantity $\rho((1{+}\twist)\Delta_{n-i}\twist \widetilde{\twist}D^0(x\cdot dy))$. Then the result follows from the following observations:
\begin{gather*}
\textup{A2}+\textup{B1}=\rho\mathbb{D}_{n-i}((x+dy)\cdot(x+dy)-x\cdot x);\\
\textup{A5}+\textup{B4}+\textup{C1}=\rho \Delta_{n-i}\widetilde{\twist}D^0((x+dy)\cdot(x+dy)-x\cdot x);\\
\textup{A3}+\textup{B2}=\textup{A6}+\textup{B5}+\textup{C2}= \textup{B6}+\textup{C3}=0.\qedhere
\end{gather*}
\end{proof}

\subsection{Thought}
Could it be that we can define the operations differently on the image of cycles in homotopy, and the operations I'm defining all over the place really do converge to zero? Then the way to recover the homotopy of the sphere would be to do the nice, unstable $\delta$ operations in the sseq of an iterated symmetric square, then finally to map them out to the underlying sseq with an iterated shift. I'm not sure if the final step could work.
