...
\begin{prop}
The chain-level map $\textup{DEL}_i$ produces well defined operations $\delta_i:E^r_{-s,t}\to E^r_{-s,t+i}$ whenever $r\geq1$, $i<t-(r-2)$ and either $i\leq n+1$ or $i<t-2(r-2)$. If $i\geq t-2(r-2)$, the operations may still be defined, but are multivalued.
\end{prop}
\begin{proof}
