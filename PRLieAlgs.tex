...
\includeversion{adams redo for notation}
...
\\%r2c4
&%r3c1
X\ar@{->>}[u]_-{\epsilon\circ v}
\ar@{=}[r]
&%r3c2
X\ar@{->>}[u]_-{\epsilon\circ v}&%r3c3
X\ar[u]_-{v}\ar@{=}[l]%r3c4
}\]
This diagram constructs both a map $\overline{\mu}:X\otimes X \to \overline{D}^1X$ and a zig-zag of levelwise (write $E^2$?) weak equivalences from $\overline{D}^1X$ to $D^1X$. Of course, the resulting connecting homomorphisms commute:
...
%}}\]
Let's abbreviate a little for the sake of compactness. Write $X$ for $X^\fraks$, $V$ for $VX^\fraks$, $\overline{V}$ for $\overline{V}X^\fraks$, dots for categorical products, and powers for categorical self-products.
%\[\newcommand{\Times}{{\cdot}}
...
\end{lem}
\begin{proof}
It's enough to check that the composite $c((X^\fraks)^{\sqcup2})\to  c(VX^\fraks)$ is null. However, a map into the codomain is null iff it is zero on homology, as the codomain is a GEM. Fortunately, we have already described the relevant maps on homology algebraically, the point being that their composite is zero.
\end{proof}
...
Throughout, I'll write $\frakt=t+t'$ and $\fraks=s+s'$ in order to save a bit of space. We're going to need the following commutative diagram of vector spaces:
\[\mathclap{\xymatrix@R=4mm{
...
\pi_{\frakt+1}(\overline{V}X^\fraks)
\ar[l]^-{\cong\textup{-ish}}_-{\partial_\textup{conn}}
\\%r1c4
&%r2c1
&%r2c2
...
&%r2c3
\pi_{\frakt+1}(VX^\fraks)\ar[u]_-{\cong}
\ar[l]^-{\cong\textup{-ish}}_-{\partial_\textup{conn}}
\ar[d]^-{\cong}
\\%r2c4
...
&%r3c2
&%r3c3
\textup{Pr}_{\frakt+1}(HVX^\fraks)%r3c4
}}\]
It helps to modify this diagram a little. Indeed, for each $(\alpha,\beta)\in \hom_{s\Comm}(S^t,X^\fraks)\times\hom_{s\Comm}(S^{t'},X^\fraks)$, there is a diagram starting with the singleton set $\{(\alpha,\beta)\}$:% such that $\alpha=\widetilde{\epsilon\alpha}$ and $\alpha=\widetilde{\epsilon\alpha}$
%\[\xymatrix@R=4mm{
...
&%r1c2
\pi_{\frakt}(\overline{D}^1X^\fraks)&%r1c3
\pi_{\frakt+1}(\overline{V}X^{\fraks})\ar[l]_-{\partial_\textup{conn}}
\ar@{~}[dd]^-{\textup{zig-zag}}_-{\cong}
\\%r1c4
&%r2c1
&%r2c2
\pi_{\frakt+1}(Q\overline{V}X^{\fraks})\ar@{..>}[ur]_-{=}&%r2c3
%\pi_{\frakt+1}(X^{\fraks+1})\ar[u]_-{\cong}
%\ar[d]^-{\cong}
\\%r2c4
%\textup{Pr}_t(HX^{\fraks})\otimes \textup{Pr}_{t'}(HX^{\fraks})\ar[r]^-{j}
...
&%r3c2
&%r3c3
\textup{Pr}_{\frakt+1}(HVX^{\fraks})\ar@{ >->}[d]
\\%r3c4
(HX^\fraks\wedge HX^\fraks)_{\frakt+1}\ar@{=}[r]
&\pi_{\frakt+1}Q(\textup{cof})
\ar[r]^-{\pi_*(Q\xi_\textup{res})}
&
\pi_{\frakt+1}QVX^{\fraks}\ar@{=}[r]
...
this composite being the sum of the three maps $\overline{v}\circ c(\textup{add})\circ c(\epsilon^2)\circ\overline{F}(\alpha,\beta)$ and $\overline{v}\circ\pi_i\circ\epsilon\circ\overline{F}(\alpha,\beta)$ for $i=1$ and $2$.
...
&%r2c2
c(\overline{V}X^{\fraks-1}\times \overline{V}X^{\fraks-1})\ar[r]_-{c(\textup{add})}
&%r2c3
c(\overline{V}X^{\fraks-1})\makebox[0cm][l]{\,$=X^\fraks$}%r2c4
}\]
Starting with the straight arrows, we need to justify the rightmost curved arrow. The naturality of $\overline{F}$ allows us to see the bottom curved arrow, then the definition of $\overline{F}$ gives the next curved arrow, and naturality of the standard lift gives the third curved arrow.
\end{proof}
\begin{thm}
...
&%r1c3
QX^\fraks\ar[r]^-{Q(\epsilon v)}
&%r1c4
Q\overline{V}X^\fraks\\%r1c5
\makebox[0cm][r]{$g\in\,$}
C_{t,t'}\ar[r]^-{\overline{F}(\alpha,\beta)}
\ar@{->>}[u]
\ar[d]_-{d_0}
...
\ar[d]_-{d_0}
&%r1c3
X^\fraks\ar@{->>}[r]^-{\epsilon v}
\ar@{->>}[u]
\ar[d]_-{d_0}
&%r1c4
\overline{V}X^\fraks\makebox[0cm][l]{$\,\ni\textup{dot}(\alpha,\beta)$}\ar@{=}[u]
\\%r2c5
...
\[0\to\overline{D}^1X^\fraks\to X^\fraks\overset{\epsilon v}{\to} \overline{V}X^\fraks\to 0,\]
