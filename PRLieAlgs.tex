...
For each of the categories of \S\ref{ChapterCategoriesOfInterest}, there is a functor $\Ind{\mathsf{C^n}}:\mathsf{C^n}\to\GR{n}$ which sends $X\in\mathsf{C}^n$ to the quotient of the underlying vector space of $X$ by the subspace generated by the images of all the operations defining $X$.
...
To check that the triangle commutes is the note that we didn't fail to kill any of the $\LL{n}$-operations over the course of applying the two functors $\Psi$ and $\Ind{\PRLie{n}}$. This is easy both in case $n=1$ and in case $n\geq2$. Finally, we'll check that $\Psi(\Fr{\LL{n}}(X))\cong\Fr{\PRLie{n}}(X)$ in either case.
...
Proposition \ref{PropOnTheCommutingDiagramOfIndFunctors} included the fact that ${\Ind{\nontop{n}}}:\LL{n}\to\PRLie{n}$ preserves free objects. Thus, for $X\in\LL{n}$, we may study $(\derived\Ind{\LL{n}})(X)$ using the Grothendieck spectral sequence derived in \cite{Blanc_Stover-Groth_SS.pdf}, which takes the form:
...
Here, $(\derived{\Ind{\nontop{n}}})X\in\PiAlg{\PRLie{n}}$, and $\overline{\Ind{\PRLie{n}}}$ is the functor $\PiAlg{\PRLie{n}}\to\GR{n+1}$ induced by $\Ind{\PRLie{n}}$. The $t$ subscript indicates the value of the second grading from the left, which arose as the homological degree in $(\derived{\Ind{\nontop{n}}})X$. It is simpler to write:
\[E^2=(\derived\overline{\Ind{\PRLie{n}}})(\derived{\Ind{\nontop{n}}})X\implies(\derived\Ind{\LL{n}})X\textup{ with }\deg(d^r)=(-r,r-1,0,\ldots,0).\]
Here, each $E^r$ is $(n+2)$-graded, while $(\derived\Ind{\LL{n}})X$ is $(n+1)$-graded. Each page $E^r$ has two homological degrees in addition to the $n$ internal degrees of $X$, and the leftmost two degrees on $E^\infty$ sum to the leftmost degree in $(\derived\Ind{\LL{n}})X$. It isn't difficult to identify $\overline{\Ind{\PRLie{n}}}$:
\begin{prop}
Under the identifiction of $\PiAlg{\PRLie{n}}$ with $\LL{n+1}$, the functor $\overline{\Ind{\PRLie{n}}}:\PiAlg{\PRLie{n}}\to\GR{n+1}$ coincides with ${\Ind{\LL{n+1}}}:\LL{n+1}\to\GR{n+1}$.
...
\end{prop}
\begin{proof}
As $\overline{\Ind{\PRLie{n}}}$ is a $0^\textup{th}$ left derived functor, and $\Ind{\LL{n+1}}$ preserves colimits, it is enough to check this claim on free objects in $\PiAlg{\PRLie{n}}$. All free objects are isomorphic to $\pi_*(\Fr{\PRLie{n}}SX)$ for some $X\in\GS{n+1}$, and
...
For emphasis, each page $E^r$, for $r\leq\infty$ is (at least) an element of  $\GR{n+2}$, with $d^r$ of degree $(-r,r-1,0,\ldots,0)$. The target, $(\derived\Ind{\LL{n}})X$, is (at least) an object of $\GR{n+1}$. The spectral sequence is convergent, which is to say that there is a positive, increasing, exhaustive filtration $F$ on $(\derived\Ind{\LL{n}})X$ such that
...
\begin{prop}
For $n\geq1$, and any $X\in\LL{n}$, there's an isomorphism of vector spaces
\[(\derived\Psi)(\forget_{\nontop{}}X)\cong\forget_{\GR{}}(\derived{{\Ind{\nontop{n}}}})X,\]
induced by the evident inclusion 
\[\Ind{\nontop{n}}B_\bullet(\Fr{\nontop{n}},\Fr{\nontop{n}},\forget_{\nontop{}}X)\to \forget_{\GR{}}\Psi B_\bullet(\Fr{\LL{n}},\Fr{\LL{n}},X).\]
You might write this as $B_\bullet(\forget,\Fr{\nontop{n}},\forget_{\nontop{}}X)\to B_\bullet(\PRLie{n},\Fr{\LL{n}},X)$.
\end{prop}
...
%%%%\end{shaded}
For any object $X\in\nontop{n}$, write $\overline{B}_*:= N(\Ind{\nontop{n}}(\BarConst{\nontop{n}}X))$. This is the normalized complex, obtained by taking the quotient by degenerate simplices, of the simplicial bar construction calculating $(\derived\Ind{\nontop{n}})X$.
\begin{prop}
...
%Thus, $E^1_{r,\ell}=(\derived_{r,\ell}\Ind{\nontop{n}})(KX)$ is  zero except when $r=\ell$, so that $d^1$ is the last nonzero differential, after which the spectral sequence collapses.
