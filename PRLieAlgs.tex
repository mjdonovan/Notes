...
\subsection{Steenrod operations algebraically defined on $E^2$}
In this case, the `surprising' operations are easy to define. That is, define Steenrod operations $\Sq^i_\textup{alg}:E^2_{s,t}(X)\to E^2_{s+i,2t}(X\otimes_{\Sigma_2}X)$ for all $i\geq 0$ using the chain-level formula:
\[S^i_\textup{alg}:x\mapsto \rho\Delta (D^{s-i}(x\otimes x)+D^{s-i+1}(x\otimes dx)).\]
The usual trick, along with the fact that $\Delta$ is a symmetric chain map, shows that $d(S^i_\textup{alg}(x))=\rho\Delta D^{s-i+1}(dx\otimes dx)$, so that if $x\in E^r_{s,t}$, $S^i_\textup{alg}(x)\in E^{2r-1}_{s+i,2t}$. To examine the indeterminacy, $x\in E^r_{s,t}$ is determined up to boundaries of $y\in E^{r-1}_{s-r+1,t+r+2}$, and the following equation%footnote:
\footnote{Here's how:
\begin{alignat*}{2}
\textup{RHS}
&=
...
\end{alignat*}} shows how this affects the value of $S^i_\textup{alg}$:
\[S^i_\textup{alg}(x)-S^i_\textup{alg}(x+dy)=d\rho\Delta [D^{s-i-1}(y\otimes y)+D^{s-i}(y\otimes dy)+D^{s-i+1}(dy\otimes x)].\]
The filtration of this bounding cycle is at least $2(s-r+1)-(s-i-1)=(s+i)-(2r-3)$, so that $\Sq_\textup{alg}^i$ has indeterminacy $2r-2$ (phew).
 ...
 and the analogues of \cite[(1.111),(1.112)]{MR2245560} still hold, which is to say that 
...
\[J_r=\sum_{\beta-\alpha=r}\Delta_\alpha\twist^\alpha D^\beta.\]
Let's calculate $dJ_{r}+J_{r}d$ (when $r\geq0\textbf{?}1$). Write
\[J_{r}=\sum_{j\geq0}\Delta_{j}\twist^{j}D^{j+r}\]
\begin{alignat*}{2}
d\mathbb{D}_r
&+
