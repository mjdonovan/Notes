...
As the top row is a coequaliser diagram of vectorspaces, $j''$ induces a map $j'$ as long as $j''$ vanishes on $\textup{im}((\alpha_{\sqcup}-\beta_{\sqcup})\circ \textup{in}_{\mathsf{C}\mathsf{C}})$. Then $j'$ will induce a map $j$ as long as $j'$ vanishes on nontrivial operations in $X\wedge Y$.

Here $\mathsf{C}(X\oplus Y)$ is spanned by formal expressions $e^0(x_i,y_k)$, which guarantees the unicity of the map $j$. The vector space $\mathsf{C}(\mathsf{C}(X)\oplus\mathsf{C}(Y))$ is spanned by doubly-formal expressions $e^0(e^1_{i_1}(x_{i_1i_2}),e^1_{k_1}(y_{k_1k_2}))$, and its subspace $\mathsf{C}\mathsf{C}X\wedge \mathsf{C}\mathsf{C}Y$ is spanned by those in which $e^0$ only contains terms which involve some element of both $X$ and $Y$. The image of this expression under the map $(\mathsf{C}(\rho\oplus\rho)-\beta)$ is
\[e^0(e_{i_1}(x_{i_1i_2}),e_{k_1}(y_{k_1k_2}))-e^0(e^0_{i_1}(x_{i_1i_2}),e^0_{k_1}(y_{k_1k_2}))\]
