...
%%%%%\[
%%%%%\{\Q^Ib\,:\,b\in B_{a_{n+1},\ldots,a_1},\,I\in\admis{\Q},\ i_1\leq|b|,\
%%%%%\textup{if }a_{n}=\cdots=a_2=0\textup{ then $I$ doesn't contain zero}\}
%%%%%\]
...
%\textbf{NOTE that we can change the $a_{n+1}$ to an $a_n$, as if $a_{n+1}$ is }
\end{enumerate}
We always allow $I$ to be the empty sequence in the above bases, in which
case the condition on $i_{1}$ is to be ignored. If $I$ is nonempty, we mean
that $i_{1}$ is the rightmost entry of $I=(i_\ell,\ldots,i_1)$.
\end{prop*}
...
%We can add a commuting square to the resulting diagram on homotopy groups:
%\[\xymatrix@R=0mm{
%\pi_*(\Fr{\PRLie{n}}S(X))
%\ar[r]^{\alpha_*}
%&%r1c2
%\pi_*(\Fr{\RestLie{n}}S(X))
%\ar[dr]^-(.4){\rho\gamma_*}&%r1c3
%%\Fr{\RestLie{n}}S(X)/\Fr{\PRLie{n}}S(X)
%%\ar[r]
%%\ar[d]^-{\cong}
%%&%r1c4
...
%\ar[ur]
%\ar[dr]^-{\psi}
%&&\pi_*\left(\frac{\displaystyle \Fr{\RestLie{n}}S(X^\textup{z})}{\displaystyle \Fr{\GoodLie{n}}S(X^\textup{z})}\right)
%\\
%\pi_*(\Fr{\GoodLie{n}}S(X^\textup{z}))
%\ar[r]_{\beta_*}
%&%r1c2
%\pi_*(\Fr{\RestLie{n}}S(X^\textup{z}))
%\ar[ru]_-(.4){\rho\gamma'_*}&%r1c3
%%\Fr{\RestLie{n}}S(X^\textup{z})/\Fr{\GoodLie{n}}S(X^\textup{z})
%%\ar[r]
%%&%r1c4
%%0
%}\]
%Here, $C\subset \pi_*(\Fr{\RestLie{n}}S(X))$
%
%
%
%
%
%
%Now let $C\subset \pi_*(\Fr{\RestLie{n}}S(X))$ be the subset
%\[C=\{\Q^Ib\,:\,b\in B_{0,\ldots,0,a_1},\,I\in\admis{\Q},\ i_1\leq|b|,\ I\textup{ contains 0}\},\]
%and let $D\subset \pi_*(\Fr{\PRLie{n}}S(X))$ be the subset
%\[D=\left\{\Q^Ib\,:\,\genfrac{}{}{0pt}{}{b\in B_{a_{n+1},\ldots,a_1},\,I\in\admis{\Q},\ i_1\leq|b|,}{\textup{if }a_{n}=\cdots=a_2=0\textup{ then $I$ doesn't contain 0}}\right\}.\]
Now define
\[C=\{(I,b)\,:\,b\in B_{0,\ldots,0,a_1},\,I\in\admis{\Q},\ i_1\leq|b|,\ I\textup{ contains 0}\},\]
\[D=\left\{(I,b)\,:\,\genfrac{}{}{0pt}{}{b\in B_{a_{n+1},\ldots,a_1},\,I\in\admis{\Q},\ i_1\leq|b|=a_{n+1}?,}{\textup{if }a_{n?}=\cdots=a_2=0\textup{ then $I$ doesn't contain 0}}\right\}\]
and maps $c:C\to\pi_*(\Fr{\RestLie{n}}S(X))$ and $d:D\to \pi_*(\Fr{\PRLie{n}}S(X))$ by the formula $(I,b)\mapsto \Q^Ib$.
Then:
\begin{itemise}
\setlength{\parindent}{.25in}
\item $c$ is an injection with linearly independent image, as is $d$, and $\pi_*(\Fr{\RestLie{n}}S(X))=\langle c(C)\rangle\oplus\alpha\langle d(D)\rangle$.
\item $\beta_*$ is mono
\item $\gamma_*c$ is an injection with linearly independent image, and $\pi_*(\Fr{\RestLie{n}}S(X^\textbf{z}))=\langle \gamma_*c(C)\rangle\oplus\im(\beta_*)$.
\end{itemise}
\end{proof}
