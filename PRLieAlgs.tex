...
The usual trick, along with the fact that $\Delta$ is a symmetric chain map, shows that $d(\textup{SQ}^{i,s}(x))=\rho\Delta D^{s-i+1}(dx\otimes dx)$, so that if $x\in Z^r_{-s,t}$, $\textup{SQ}^{i,s}(x)\in Z^{2r-1}_{-s-i,2t}$. To examine the indeterminacy, $x\in E^r_{-s,t}$ is determined up to boundaries of $y\in Z^{r-1}_{-(s-r+1),t-r+2}$, and the following equation%footnote:
...
\textup{SQ}^{t-i+1,s}(x)+\rho\Delta D^0(x\otimes x),&\textup{if }i=n+1;\\
d(\textup{DUB}_i(x)),&\textup{if }i=n;\\
d(\textup{DUB}_i(x))+\textup{DUB}_i(dx),&\textup{if }2\leq i\leq n-1.
\end{cases}\]
\end{cor}
...
since $\rho\phi_0D^0(x\otimes x)$ vanishes as $t>i\geq2$.
%\begin{alignat*}{2}
%&
%\rho\Delta(D^{i-n-1}(x\otimes x)+D^{i-n}(x\otimes dx))+\\
%&\ \ +\rho\Delta_{n-i+1}(1+\twist)D^0(x\otimes x)+\rho\Delta_{n-i}(1+\twist)D^0(dx\otimes x)+\\
