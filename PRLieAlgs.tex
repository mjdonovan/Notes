...
Suppose that $X\in s\mathsf{C}$ is any simplicial object. Then we can form the bisimplicial object $bX\in ss\mathsf{C}$ defined by $(bX)_{s,t}=\Fr{\mathsf{C}}^{t+1}X_s$. For each $s$, the singly simplicial object $(bX)_{s,\bullet}$ is augmented to $X_s$ via the structure map $\mathsf{C}X_s\to X_s$. As such there is a map $\Delta bX\to X$. One may alter the proof of \cite[Thm 3.4(i)]{MillerSullivanConjecture.pdf} to see that this is a functorial acyclic fibration from a cofibrant object. In summary:
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item  $\Delta bX$ is almost free by construction, thus cofibrant.
\item Consider the map $bX\to \textup{const}^e(X)$ in $ss\mathsf{C}$, where $ \textup{const}^e(X)_{s,t}:=X_s$. This induces a map of the spectral sequences obtained by filtering the associated total chain complexes by the first variable. This is an isomorphism from $E^1$, so that $\Delta bX\to\Delta\, \textup{const}^e(X)=X$ is a weak equivalence.
\item As $\Delta bX\to X$ is a weak equivalence, in order that it is a surjection on basepoint components, it is enough that it is simply a surjection, which it is by construction.
\end{enumerate}
The construction $bX$ is a bisimplicial version of the simplicial bar construction $B_\bullet(\Fr{\mathsf{C}},\Fr{\mathsf{C}},Y):=(\Fr{\mathsf{C}})^{\bullet+1}Y$ on an object $Y\in\mathsf{C}$. In fact, $B_\bullet(\Fr{\mathsf{C}},\Fr{\mathsf{C}},Y)=\Delta b\,\textup{const}(Y)$, where $\textup{const}(Y)\in s\mathsf{C}$ is the constant object on $Y$.
...
The functor $\Ind{\mathsf{C}^n}$ is always left adjoint to a functor $K:\GR{n}\to\mathsf{C}^n$ which sends a graded vector space $V$ to the corresponding trivial object in $\mathsf{C}^n$. That is, $KV$ has $V$ as underlying vector space, and all operations in $KV$ are zero. Note then that the right adjoint, $K$, preserves weak equivalences and fibrations, both of which are determined on the level of underlying simplicial groups. Thus, $\Ind{\mathsf{C}^n}$ is a left Quillen functor, so that we can easily define the left derived functors $\derived_*\Ind{\mathsf{C}^n}$.
...
To see that the triangle commutes is simple, both when $n=1$ and when $n\geq2$. %to note that an object of $\LL{n}$ contains no more structure than an object of $\PRLie{n}$ taken with the \emph{non-top} $Q$ operations if one first kills all non-top operations, and then kills the restriction of the 
...
Here, $a_2$ is an acyclic fibration because $\forget_{\nontop{}}$ preserves acyclic fibrations, and all other decorations come directly from the above discussion. The ``$\Delta b$'' used in the cofibrant replacement $a$ is taken in $s\nontop{n}$. The lemma \ref{LemLevelwiseFreeWillSuffice} shows that $a$ remains a weak equivalence after applying $\Ind{\nontop{n}}$. Supposing then that we can find a map $i$ making the right hand square commute, it will lift to a map $\widetilde{i}$. By 2-of-3, $\widetilde{i}$ will be a weak equivalence of cofibrant objects, so that it too will remain an equivalence after applying the left Quillen functor $\Ind{\nontop{n}}$. Thus $i$ too will remain an equivalence after applying $\Ind{\nontop{n}}$.
