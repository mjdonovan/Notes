...
To understand the number of this differential, note that the equation for calculating the $r$ in which $d_r$ is required is ``$t_\textup{src}+r-1=t_\textup{tar}$''. Now the effect on $t$ of both `squaring' and `precomposing with $\SqShift^0$' is $t\mapsto 2t+1$. This operation doubles $t_\textup{tar}-t_\textup{src}=r-1$.
