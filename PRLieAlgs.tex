...
The point will be that the top row is a coequaliser diagram of vectorspaces, so that $j''$ induces a map $j'$ as long as $j''$ vanishes on $\textup{im}((\alpha_{\sqcup}-\beta_{\sqcup})\circ \textup{in}_{\mathsf{C}\mathsf{C}})$. The map $\overline{j}$ is exactly that defined in the statement of the proposition.

The smash product $X\wedge Y$ is a subalgebra of $X\sqcup Y$, which can be described using the following coequaliser of vectorspaces:
\[\xymatrix@R=4mm@1{
\mathsf{C}(\mathsf{C}(X)\oplus\mathsf{C}(Y))
\ar@<.5ex>[r]^-{\mathsf{C}(\rho\oplus\rho)}
\ar@<-.5ex>[r]_-{\beta}
&%r1c1
\mathsf{C}(X\oplus Y)
\ar[r]
...
where $\beta$ is the unique $\mathsf{C}$-map induced by the sum of the inclusions $\mathsf{C}(X)\to\mathsf{C}(X\oplus Y)$ and $\mathsf{C}(Y)\to\mathsf{C}(X\oplus Y)$. Here $\mathsf{C}(X\oplus Y)$ is spanned by formal expressions $e^0(x_i,y_k)$, which guarantees the unicity of the map $j$. The vector space $\mathsf{C}(\mathsf{C}(X)\oplus\mathsf{C}(Y))$ is spanned by doubly-formal expressions $e^0(e^1_{i_1}(x_{i_1i_2}),e^1_{k_1}(y_{k_1k_2}))$. The image of this expression under the map $(\mathsf{C}(\rho\oplus\rho)-\beta)$ is
\[e^0(e_{i_1}(x_{i_1i_2}),e_{k_1}(y_{k_1k_2}))-e(e^0_{i_1}(x_{i_1i_2}),e^0_{k_1}(y_{k_1k_2}))\]
