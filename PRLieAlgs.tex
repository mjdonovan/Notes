...
Let $\mathsf{CR(s\Comm)}$ be the category of cofibrant replacement functors in $s\Comm$. That is, an object of $\mathsf{CR(s\Comm)}$ is an augmented endofunctor $(c,\epsilon)$, such that $c:s\Comm\to s\Comm$ is a functor whose consists only of cofibrant objects, and $\epsilon:c\Rightarrow I$ is a natural acyclic fibration. Morphisms in $\mathsf{CR(s\Comm)}$ are to be natural transformations which commute with the augmentations.
...
We'll define the $r^\textup{th}$ derived functor $(D^r_cF)$ of $F$ with respect to Andr\'e-Quillen homology. The definition is inductive: one sets $(D^0F)(X)=F(cX)$, and
\[(D^sF)(X):=\hofib((D^{s-1}F)(cX)\to (D^{s-1}F)(QcX)),\]
where $Q$ is the indecomposables functor, and $\hofib$ is some fixed functorial construction of the homotopy fibre. These functors fit into a tower, via the following composite natural transformation:
\[(D^sF)(X)\to (D^{s-1}F)(cX)\overset{(D^{s-1}F)(\epsilon)}{\to} (D^{s-1}F)(X).\]
For clarity, we'll record the construction of $(D^1F)(X)$, $(D^2F)(X)$ and the map between them, in the following diagram. Note that every composable pair of  parallel arrows is to be a homotopy fibre sequence.
...
(D^2F)(X)\ar[dr]\ar[dd]\\
&(D^1F)(cX) \ar[rr]\ar[rd]\ar[dd]         &           &FcccX \ar[rr]\ar[rd]\ar'[d][dd]^{Fcc\epsilon}         &           &   FcQccX \ar'[d][dd]^{FcQc\epsilon}           \ar[dr]  &                  \\
(D^1F)(X)\ar[dr]&        &  (D^1F)(QcX) \ar[rr] \ar[dd]  &                     &  FccQcX \ar[rr] \ar[dd]  &             & FcQcQcX
         \ar[dd] \\
&(D^1F)(X) \ar'[r][rr] \ar[dr] &        &   FccX \ar'[r][rr] \ar[dr] &        &   FcQcX
\ar[dr] &                   \\
&        &   0 \ar[rr]      &               &   0 \ar[rr]      &                    &
0
}\]
...
Moreover, this construction is natural in the functor $F$, is homotopical as long as $F$ preserves weak equivalences between cofibrant objects. Finally, given a morphism $(c,\epsilon)\to (c',\epsilon')$ of cofibrant replacements, one obtains natural weak equivalences $(D^t_c)X\to(D^t_{c'})X$.

Suppose that $(c,\epsilon)$ and $(c',\epsilon')$ are any two competing cofibrant replacement functors. Then we can form a third cofibrant replacement, $(cc',\epsilon\epsilon')$, which maps to each of them, via the natural transformations $c\overset{\epsilon'}{\from}cc'\overset{\epsilon}{\to}c'$. Thus we have natural weak equivalences of towers:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
(D^1_cF)X
\ar[r]&%r1c5
(D^0_cF)X\makebox[0pt][l]{$\mbox{}=FcX$}\\
\cdots 
\ar[r]
&%r1c1
(D^{t}_{cc'}F)X
...
\ar[d]^-{\sim}
\ar[r]&%r1c5
(D^0_{cc'}F)X\makebox[0pt][l]{$\mbox{}=Fcc'X$}
\ar[u]_-{\sim}
\ar[d]^-{\sim}\\
\cdots 
\ar[r]
...
(D^1_{c'}F)X
\ar[r]&%r1c5
(D^0_{c'}F)X\makebox[0pt][l]{$\mbox{}=Fc'X$}
}\]
...
For this section, we will always use the cofibrant replacement functor $(c,\epsilon)$ in which $c:s\Comm\to s\Comm$ is (the diagonal of) the standard bar construction, $(cX)_n:=T^{n+1}X_n$. Here, $T$ is the monad from the free/forget adjunction between vector spaces and nucas. We'll only be interested in deriving two different endofunctors of $s\Comm$, the identity $I$, and the ``$s^\textup{th}$ power'' functor $P^sX:=X^s$.

As these functors are particularly well behaved, we will be able to use a modified version of the derived functor construction, which we'll write $(\calD^sF)$, for $F$ either $I$ or $P^s$. Again, the definition is inductive: one sets $(\calD^0F)=F$ \textbf{(NOTE: one fewer replacement)}, and
\[(\calD^sF)(X):=\ker((\calD^{s-1}F)(cX)\to (\calD^{s-1}F)(QcX)).\]
Now there are evident maps of towers
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
(D^1_cI)X
\ar[r]&%r1c5
(D^0_cI)X\makebox[0pt][l]{$\mbox{}=cX$}\\
\cdots 
\ar[r]
&%r1c1
(\calD^{t}Ic)X
...
\ar[d]^-{}
\ar[r]&%r1c5
(\calD^0Ic)X\makebox[0pt][l]{$\mbox{}=cX$}
\ar[u]_-{}
\ar[d]^-{}\\
\cdots 
\ar[r]
...
(\calD^1I)X
\ar[r]&%r1c5
(\calD^0I)X\makebox[0pt][l]{$\mbox{}=IX$}
}\]
in which the upward maps result from mapping strict fibres to homotopy fibres, and the downward maps are induced by $\epsilon:Ic\to I$.

\textbf{We'll show that these vertical maps are all weak equivalences.}
...
The functors $(\calD^sI):s\Comm\to s\Comm$ preserve weak equivalences.
...
For integers $t\geq2$ and $q\geq0$, the map $\pi_q(\calD^{2t+q-1}I)X\to\pi_q(\calD^{t}I)X$ is zero.
\end{prop}
\begin{proof}%[Proof of \ref{convergenceProp}]
The first claim is \ref{towerIsHomotopical}. Apply $(\calD^t\DASH)X$ to the diagram of functors constructed in \ref{towerWithPowers}, to obtain the following diagram in $s\Comm$:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
By \ref{connectivityOfDerivedPowers}, $(\calD^tP^{t+q})(A)$ is $q$-connected, and the result follows.
\end{proof}
...
\setlength{\parindent}{.25in}
\item For any functor $F$, there is a natural isomorphism $(\calD^s(\calD^tF))\cong (\calD^{s+t}F)$ so that the evident diagram commutes:
\[\xymatrix@R=4mm{
(\calD^s(\calD^tF))
\ar[r]^-{\cong}
\ar[d]^-{(\calD^s\delta)}
...
\item If $F$ is the prolongation of a functor $\Comm\to\Comm$, one notes that $(\calD^sF)$ can be defined one level at a time. That is,  we may define functors $(\calD^s_nF):\Comm\to\Comm$ such that $(\calD^sF)(X)_n=(\calD^s_nF)(X_n)$, by defining $(\calD^0_nF):=F$, and
\[(\calD^s_nF)(Y):=\ker((\calD^{s-1}_nF)(T^{n+1}Y)\to (\calD^{s-1}_nF)(QT^{n+1}Y)).\]
\end{enumerate}
\begin{lem}
For $Y\in\Comm$, and integers $r,n\geq0$ and $s>0$:
\begin{itemise}
\setlength{\parindent}{.25in}
\item $(\calD^r_nI)Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consiting of those mega\-monomials which have a doubling in each of the $\calT_1,\ldots,\calT_r$ functors.
\item $(\calD^r_nP^{s})Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consiting of those mega\-monomials which have a doubling in each of the $\calT_2,\ldots,\calT_r$ functors, and a power of $s$ in the $\calT_1$ functor.
\end{itemise}
...
(\calD^r_nI)(Y)
&:=
\ker((\calD^{r-1}_nI)(\calT_r T_r^n Y)\to (\calD^{r-1}_nI) (Q\calT_rT_r^n X_n))%
\\
&=
(\calD^{r-1}_nI)(\calT_r T_r^n Y)\cap\langle \textup{meganomials with a doubling in $\calT_r$}\rangle\\
&=
...
The functors $(\calD^rI)$ and $(\calD^rP^{s})$ preserve surjective maps, and there is a commuting diagram of functors:
\[\xymatrix@R=4mm{
\cdots 
\ar[r]
&%r1c1
...
The functors $(\calD^rI):s\Comm\to s\Comm$ preserve all weak equivalences.
\end{cor}
\begin{proof}
This is trivial when $r=0$. Suppose, by induction on $r$, that $(\calD^{r-1}I)$ preserves weak equivalences, and that $f:X\to X'$ is a weak equivalence in $s\Comm$. Then we have a commutative diagram in $s\Comm$:
\[\xymatrix@R=4mm{
0\ar[r]&
(\calD^rI)X\ar[d]^{(\calD^rI)(f)}\ar[r]&
(\calD^{r-1}I)(cX)\ar[d]^{(\calD^{r-1}I)(cf)}\ar[r]&
(\calD^{r-1}I)(QcX)\ar[d]^{(\calD^{r-1}I)(Qcf)}\ar[r]&
0\\0\ar[r]&
(\calD^rI)X'\ar[r]&
(\calD^{r-1}I)(cX')\ar[r]&
(\calD^{r-1}I)(QcX')\ar[r]&
...
Now the rows are short exact sequences, as $(\calD^{r-1}I)$ preserves surjective maps. The maps $cf$ and $Qcf$ are weak equivalences, as $c$ is a cofibrant replacement, and $Q$ is left Quillen. Thus, the induction hypothesis implies that the $(\calD^{r-1}I)(cf)$ and $(\calD^{r-1}I)(Qcf)$ are weak equivalences. The homotopy long exact sequence and the 5-lemma together imply that ${(\calD^rI)(f)}$ is a weak equivalence.
\end{proof}
...
For $A\in s\Comm$ connected, and any $t>0$ and $s\geq2$, $(\calD^tP^{s})(A)$ is $(s-t)$-connected.
\end{cor}
\begin{proof}
We'll prove this by induction on $t$. When $t=1$:
\[(\calD^1P^{s})A:=\ker(P^{s}(cA)\to P^{s}(QcA))=P^{s}(cA)\]
Thus $(\calD^1P^{s})A$ is $(s-1)$-connected, by \ref{connectivityOfPowerQuillen}.

Now suppose, by induction, that $(\calD^{t-1}P^{s})(B)$ is $(s-(t-1))$-connected for any connected $B$. Then by \ref{towerWithPowers}, there's a short exact sequence:
\[\xymatrix{
0\ar[r]&
(\calD^{t}P^{s})(A)\ar[r]&
(\calD^{t-1}P^{s})(cA)\ar[r]&
(\calD^{t-1}P^{s})(QcA)\ar[r]&
0
}\]
in which the rightmost two objects are each $(s-t+1)$-connected. The associated long exact sequence shows that $(\calD^{t}P^{s})(A)$ is $(s-t)$-connected.
\end{proof}
...
The verticals in this diagram constitute a pointwise weak equivalence between those diagrams from which we obtain $\widetilde{X}^2:=\hofib(X\to \Tot_2\overline{X})$ and $(D^2_\sigma I)(X)$ respectively.
...
\ar[d]^-{\sim}
\ar[r]&%r1c5
(D^0_{c\sigma} I)X\makebox[0pt][l]{$\mbox{}={c\sigma} X$}
\ar[u]_-{\sim}
\ar[d]^-{\sim}\\
\cdots 
\ar[r]
...
so that $\calS^{(N)}$ is the disjoint union $\calS^{(N)}_0\cup\calS^{(N)}_1\cup\calS^{(N)}_2\cup\calS^{(N)}_3$ of four nonempty subsets.
%Next, define
...
Let $\overline{t}_d:\calS_1^{(N)}\to\calS^{(N)}_2$ be the bijection $(\emptyset,\ldots,\emptyset,I^1)\mapsto (\emptyset,\ldots,\emptyset,t_dI^1)$. 
...
For $N\geq2$, if we decompose $H^{(N)}_0\in\PRLie{N}$ as 
...
\begin{prop}
For $N\geq2$, the graded vector space $E^{2,N}_{0,a_{N+1},a_N,\ldots,a_1}\in\GR{N+1}$ is
...
E^{2,N}_{a_{N+2},0,a_N,\ldots,a_1}
&=
H_{a_{N+2}+1}\left(H^{(N)}_0\right)_{a_N,\ldots,a_1}\\
...
\end{alignat*}
using the convention that $\calS^{(0)}_1=\calS^{(0)}_2=\calS^{(0)}_3=\emptyset$.
%\[E^{2,N}_{0,a_{N+1},a_N,\ldots,a_1}=(\Ind{\LL{N+1}}H^{(N)})_{a_{N+1},\ldots,a_1}.\]
%\[\Ind{\LL{N+1}}H^{(N)}=
%\left\langle \imath\rule{0mm}{\heightof{$\calS_1^{(N)}$}}\right\rangle\oplus 
%\left\langle b(S)\ \middle|\ S\in\calS_1^{(N)}\right\rangle \oplus\left\langle b(S)\ \middle|\ S\in\calS_3^{(N)}\right\rangle\]
...
%%%%%\end{cor}
%%%%%
%%%%%As $cX\to QcX$ is always surjective, this kernel is also the homotopy fibre. Now there are natural maps $D^s(X)\to c^sX\to X$, forming a tower above $X$, which factors through the tower of inclusions of powers of $X$:
%%%%%\[\xymatrix@R=4mm{
%%%%%\cdots 
%%%%%\ar[r]
%%%%%&%r1c1
