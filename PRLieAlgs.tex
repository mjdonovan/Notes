...
\Ixcludeversion{cofreeFunctors}
\Ixcludeversion{AdamsDerivation}
...
The filtration of this bounding cycle is at least $2(s-r+1)-(s-i-1)=(s+i)-(2r-3)$, so that $\Sq^i$ has indeterminacy vanishing by $E^{2r-2}$.
...
\normalsize
where $T_1$, $T_2$ and $T_3$ are as follows, $\rho\mathbb{D}_{n-i-1}(dx\cdot dy)\in F_{s+r}$ can be ignored, and course $\textup{DEL}_i(x)-\textup{DEL}_i(x+dy)\in F_s$ is the quantity of interest.
\begin{align*}
...
\begin{prop}
On $E^2$, for $i<t$, the spectral sequence operations $\delta_i$ defined by $\textup{DEL}_i$ equal the operations $\delta_i^\textup{alg}$. On $E^1$, setting $t=i$, the spectral sequence operation $\delta_t:E^1_{-s,t}\to E^1_{-s,2t}$ equals $\delta_t^\textup{alg}$.
\end{prop}
\begin{proof}
