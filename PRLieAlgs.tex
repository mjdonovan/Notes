% !TEX root = z_output/PRLieAlgs.tex
\documentclass[10pt]{article}
%%%USINGsubsectionNUMBERS
\newcommand{\NOFULLPAGE}{\relax}

\usepackage[backref=true,style=alphabetic, citestyle=alphabetic,
...
\def\Ixcludeversion#1{\includeversion{#1}}

\excludeversion{ConvHRM_InclusionOfBarConstructions}
...
\excludeversion{CompressingSequencesOfSteenrodOperations}
...
%\item[a)] $(\caldup{r}_nI)Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consisting of those mega\-monomials which have a doubling in each of the $\calT_1,\ldots,\calT_r$ functors.
%\item[b)] $(\caldup{r}_nb)Y\subset (\calT T^n)(\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consisting of those mega\-monomials which have a doubling in each of the $\calT_1,\ldots,\calT_r$ functors.
%\item[c)] $(\caldup{r}_nP^{s})Y\subset (\calT_1T^n_1)\cdots (\calT_rT^n_r)Y$ is the subset consisting of those mega\-monomials which have a doubling in each of the $\calT_2,\ldots,\calT_r$ functors, and a power of $s$ in the $\calT_1$ functor.
%\end{enumerate}
%
%\end{lem}
...
%We can probably assue $f_{s+1}$ has no unit part.
...
\\%r1c2
f_{s+1}f_s^s\cdots f^1_1x_0\ar@{|->}[r]^-{\chi}&%r2c1
f_{s+1}\chi(f_s^s\cdots f^1_1x_0)%r2c2
}\]
...
\textbf{unless $\chi(f_s^s\cdots f^1_1x_0)$ itself has some quadratic part!}%footnote:
...
%The dashed arrow is the map to be constructed, while the dotted arrow is to demonstrate that the composite $X^{\otimes4}\to D^1\overline{D}^1X$ is $\Sigma_4$-equivariant. 
In order to construct the dashed arrow, one starts with the commuting diagram
%\[\xymatrix@R=5mm@C=20mm@!0{
%&%r1c1
...
%\[\textup{SQ}^{i,s}(x)=\cancel{\textup{SQ}^{i+1,s+1}}(x+dh)+d\rho\Delta [D^{s-i-1}(h\otimes h)+D^{s-i}(h\otimes dh)+D^{s-i+1}(dh\otimes x)],\]
%but also (strangely)
...
For now, let $\frakg_\bullet$ simply be the bar construction on a constant simplicial Lie algebra... Hopefully we can relax this soon to $\frakg$ simply being almost free.


We're interested in using what \textbf{I believe} is a bisimplicial commutative coalgebra:
\[\textbf{B}_{\bullet\bullet}:=\overline{B}_\bullet U\frakg_\bullet\]
and we're also going to use the (somewhat artificial) simplicial chain complex
...
&%r2c1
&%r2c2
N_{p+1}^\textup{h}(N_1^\textup{v}\textbf{B}\otimes N_1^\textup{v}\textbf{B})^*\ar[d]^-{(D_1^\textup{v})^*}\\%r2c3
N_{p}^\textup{h}N_2^\textup{v}(\textbf{B}\otimes\textbf{B})^*\ar[dr]^-{\psi^*}
&%r3c1
&%r3c2
N_{p+1}^\textup{h}N_1^\textup{v}(\textbf{B}\otimes\textbf{B})^*\ar[dl]_-{\psi^*}\\%r3c3
&%r4c1
T_{p+2}(\textbf{B})^*&%r4c2
%r4c3
}\]
...
This composite vanishes except on terms $[x|y]$ where $x$ and $y$ are generators of $\frakg_p$, which have image $x\otimes y$.
\end{lem}
\begin{proof}
...
%\left[\prod_{i=1}^Ix_i^{a_i} \otimes\prod_{i=1}^Ix_i^{1-a_i}\middle| \prod_{j=1}^Jy_j^{b_j} \otimes\prod_{j=1}^Jy_j^{1-b_j}\right]
...
Having put in place these preparations, let's start working on the theorem. Suppose that $\alpha\in (N_{n}\textup{Ab}(\frakg_\bullet))^*$ is a cocycle. %We may assume without loss of generality (or are we \textbf{forced} to assume) that  $\alpha$ evaluates to zero... wait... wtf am I talking about?
Then $\alpha$ may be viewed an a permanent cocycle at $E_2^{n,1}$ in the spectral sequence of $\textup{Ab}^*$. We'll apply the chain-level Steenrod operation $S^k$ of Singer to the class $r^*\alpha$. As $\alpha$ is a permanent cycle, $S^k(r^*\alpha)=\psi^*K^*_{n+1-k}\oldphi(r^*\alpha\otimes r^*\alpha)$, which has two terms:
\[\underbrace{\psi^*(D^\textup{v}_0)^*(D^\textup{h}_{n+1-k})^*\oldphi(r^*\alpha\otimes r^*\alpha)}_{\textup{filtration $n+k-1$}}+
\underbrace{\psi^*(D^\textup{v}_1)^*(TD^\textup{h}_{n-k}T)^*
\oldphi(r^*\alpha\otimes r^*\alpha)}_{\textup{filtration $n+k$}}\]
...
so that $\bar{h}^*$ is zero away from the two columns $p=n+k-1$ or $p=n+k$, where it equals $h^*$. Now on these two columns, $\delta^\textup{v}\bar{h}^*+\bar{h}^*\delta^\textup{v}$ is the identity for $q>1$, so that:
\[(\textup{Id}+(\delta^\textup{v}\bar{h}^*+ \bar{h}^*\delta^\textup{v}))\textup{Term}_1=0\]
...
\[(\textup{Id}+(\delta^\textup{v}\bar{h}^*+ \bar{h}^*\delta^\textup{v}))\textup{Term}_2=0.\]
As $\delta^\textup{h}\bar{h}^*+ \bar{h}^*\delta^\textup{h}$ increases filtration, we have compressed $S^k(r^*\alpha)$ to the filtration $n+k$ expression $(\delta^\textup{v}\bar{h}^*+ \bar{h}^*\delta^\textup{v})\textup{Term}_1$, modulo even higher filtration.

up to even higher filtration ($n+k+1$), we have compressed $S^k(r^*\alpha)$ to the expression $(\delta^\textup{v}\bar{h}^*+ \bar{h}^*\delta^\textup{v})\textup{Term}_1$. The point now is that there is a commuting diagram
\[\xymatrix@R=4mm@C=15mm{
N_{p+1}^\textup{h}N_1^\textup{v}\textbf{B} \ar[r]^-{d^\textup{h}h_{p+1}+h_{p}d^\textup{h}}
\ar[d]^-{r}&%r1c1
N_{p}^\textup{h}N_2^\textup{v}\textbf{B} \ar[r]^-{D^\textup{v}_0\circ\psi}&%r1c2
N_{p}^\textup{h}(N_1^\textup{v}\textbf{B}\otimes N_1^\textup{v}\textbf{B})\ar[d]^-{r\otimes r}
\\%r1c3
N_{p+1}^\textup{h}\textup{Ab}(\frakg) \ar[r]^-{\textup{Ab}(\xi)}&%r1c1
N_{p}^\textup{h}\textup{Ab}(\frakg\wedge \frakg) \ar[r]^-{j}&%r1c2
N_{p}^\textup{h}(\textup{Ab}(\frakg)\otimes \textup{Ab}(\frakg))
}\]
This diagram looks even better if you recall that $\xi$ is the factoring through $\frakg\wedge\frakg$ of the commutator ${d_0^\textup{h}\psi_{p+1}+\psi_{p} d_0^\textup{h}}$ (writing $\psi$ for the almost free diagonal), although this observation is of little utility here. After showing that it commutes, postcomposing each row with the maps $D^\textup{h}_{n+1-k}$ demonstrates that this compression of $S^k(r^*\alpha)$ equals, at $E^2$, $r^*\Sq^{k\pm 1}(\alpha)$ (\textbf{which $\pm$?}).
...
be this composite inclusion. In order to calculate with this larger complex, we'll need to be explicit about the structure maps in the larger simplicial bar construction $\Ind{\nontop{}}\BarConst{\LL{}}(X)$. Using the natural isomorphism $\Ind{\nontop{}}(\Fr{\LL{n}}\DASH) \cong\Fr{\PRLie{n}}\DASH$ of functors $\GR{n}\to\PRLie{n}$, we can write:
...
added subscripts to the various free constructions involved, in order to disambiguate them in what follows. The subscript in $\Fr[out]{\PRLie{n}}$ denotes simply that this is the outermost free construction.
...
%%%%%\begin{lem}
%%%%%$D^s_n(Y)\subset (\calT_1T^n_1)\cdots (\calT_sT^n_s)Y$ is the subset consisting of those mega\-monomials which have a doubling in each of the $\calT_1,\ldots,\calT_s$ functors.
%%%%%\end{lem}
%%%%%\begin{proof}
...
%%%%%\begin{prop}
%%%%%For $s,t\geq0$, $D^tP^{s+1}(X)_n\subset (\calT_1T^n_1)\cdots (\calT_tT^n_t)X_n$ is the subset consisting of those mega\-monomials which have a 2-fold product in each of the $\calT_2,\ldots,\calT_t$ functors, and which have a $\min\{s,2\}$-fold product in $\calT^1$.
%%%%%\end{prop}
%%%%%\begin{cor}
...
%\begin{itemise}
%\setlength{\parindent}{.25in}
%\item $2i_n\leq i_{n+1}\leq s+i_1+\cdots+i_{n}$ for all $n$
%\item $2j_n\leq j_{n+1}\leq s+(j_1-1)+\cdots+(j_{n}-1)$ for all $n$
%\item $2k_n\leq k_{n+1}\leq \ell_J+k_1+\cdots+k_{n}+1$ for all $n$
%\end{itemise}
\end{prop}
\begin{proof}
