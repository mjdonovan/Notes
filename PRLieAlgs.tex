...
\subsubsection{A universal(?) cofibration}
...
\end{proof}
Now suppose that $k$ is a zero-square algebra, and that $\alpha,\beta$ are homotopy classes in $\pi_n (k)$ and $\pi_m (k)$ respectively. Then we'll be able to use the complex $C_{n,m}$ to construct a homology class $g_{\alpha,\beta}\in H_{n+m+1}(k)$ whose diagonal is $\alpha\otimes\beta+\beta\otimes\alpha$. Firstly, note that there is a commuting diagram
\[\xymatrix@R=4mm{
0\ar[r]
\ar@{ >->}[d]
&%r1c1
...
which furnishes $c_1(k^{\times2})$ with a copy of $S^n\sqcup S^m$. Next, there's a diagram %\newdir{ >}{{}*!/-7pt/\dir{>}}
\[\xymatrix@R=4mm{
S^{n+m}\ar@{->}[r]
\ar@{ >->}[d]
&%r1c1
