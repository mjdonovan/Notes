...
where $\gamma_i:\pi_t(X^s)\to\pi_{t+i}(X^s\wedge_{\Sigma_2} X^s)$ is induced by the chain map $\Theta_i:N_t(X^s)\to N_{t+i}(X^s\wedge_{\Sigma_2} X^s)$ defined by
\[\Theta_i(x):=\Delta^{t-i}(x\otimes x)+\Delta^{t-i-1}(x\otimes dx).\]
...
\[x\mapsto \Delta_{t-i}(x\otimes x)+\Delta_{t-i+1}(x\otimes dx)\]
