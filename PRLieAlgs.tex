...
\item the restriction of $D^k$ to $C^iR\otimes C^jS$ is zero unless $i\geq k$ and $j\geq k$; and
\item the restriction of $D^k$ to $C^kR\otimes C^kS$ is the identity mapping onto $C^kR\otimes C^kS=C^k(R\otimes S)$.
\end{itemise}
...
\begin{align*}
\overline{D}^k:(CX\otimes CY)_{\fraks+k,\frakt}&\to C(X\otimes_\textup{v}Y)_{\fraks,\frakt}&\quad&\textup{(zero unless $k\leq \fraks$)}\\
\overline{\Delta}:C(X\otimes_\textup{v} Y)_{\fraks,\frakt}&\to C(X\otimes Y)_{\fraks,\frakt}\\
\overline{\Delta}_k:C(X\otimes_\textup{v} Y)_{\fraks,\frakt+k}&\to C(X\otimes Y)_{\fraks,\frakt}&\quad&\textup{for $0\leq k\leq \frakt$}\\
\overline{\phi}_k:C(X\otimes_\textup{v} Y)_{\fraks,\frakt+k}&\to C(X\otimes Y)_{\fraks,\frakt}&\quad&\textup{for $0\leq k\leq \frakt$}\\
\overline{\rho}:C(X\otimes X)_{\fraks,\frakt}&\to C(X\otimes_{\Sigma_2} X)_{\fraks,\frakt}&\quad&\textup{(proj onto coinvariants)}\\
\end{align*}
...
\subsection{Steenrod operations algebraically defined on $E^2$}
In this case, the `surprising' operations are easy to define. That is, we can define Steenrod operations $\Sq^i_\textup{alg}:E^2_{s,t}(X)\to E^2_{s+i,2t}(X\otimes_{\Sigma_2}X)$ for all $i\geq 0$ and $r\geq2$, which survive to $E^{2r-1}$ and have indeterminacy vanishing by $E^{2r-2}$, using the chain-level formula:
...
\subsection{Correct formula for $\delta$-ops}
\newcommand{\twist}{\sigma}
Write $\twist F$ as shorthand for the function $TFT$. Then we have equations:
\[d_\textup{co}\Delta_{k-1}+\Delta_{k-1}d_\textup{co}= \phi_{k}+(1+\twist)\Delta_{k}\]
\[d_\textup{si}D^{j}+D^{j}d_\textup{si}=(1+\twist)D^{j-1}\]
showing that the chain homotopy $d_\textup{bi}(\Delta_{k}\sigma^kD^{j})+(\Delta_{k}\sigma^kD^{j}) d_\textup{bi}$ equals
\[\phi_{k+1}\sigma^kD^{j}+(1+\twist)\Delta_{k+1}\sigma^kD^{j}+ \Delta_{k}(1+\twist)\sigma^kD^{j-1}.\]
Now write $\mathbb{D}_r$ for the sum
\[\mathbb{D}_r=\sum_{\beta-\alpha=r}\Delta_\alpha\twist^\alpha D^\beta.\]
Let's calculate $d\mathbb{D}_{r}+\mathbb{D}_{r}d$ (when $r\geq0\textbf{?}1$). Write
\[\mathbb{D}_{r}=\sum_{j\geq0}\Delta_{j}\twist^{j}D^{j+r}\]
\begin{alignat*}{2}
d\mathbb{D}_r
&+
...
% Comment
\end{alignat*}
using the observation that $(1+\twist)F\twist G+F(1+\twist)G=(1+\twist)(FG)$. Now using the fact that $(1+\twist)\Delta_0=\Delta+\phi_0$:
\[d\mathbb{D}_r+\mathbb{D}_rd=(1+\twist)\mathbb{D}_{r-1}+\Delta\sigma D^{r-1}+ \sum_{j\geq0} \phi_{j}\sigma^{j-1}D^{j+r-1}.\]
Presumably the right definition is then
\[x\mapsto \mathbb{D}_{n-i}(x\otimes x)+\mathbb{D}_{n-i\pm1}(dx\otimes x)\]
