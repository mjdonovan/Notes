...
Have, for $i,P$ fixed, a function 
 \[\Hom(B,X)\xrightarrow{(i^*,P_*)}\Hom(B,Y)\times_{\Hom(A,Y)}\Hom(X,Y)\]
which is always a fibration in $\calD$, acyclic if either $i$ or $P$ is acyclic. \rednote{Ask Saul what the deal is.}
...
\item Say all model categories will be simplicial\footnote{Every cofibrantly generated, proper (weak equivalences are preserved under pulling back (pushing out) along (co)fibrations), stable model category is in fact Quillen equivalent to a simplicial model category.}. 
