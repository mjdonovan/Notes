...
\item Explain that $\Sp(\calC)$ is spectral.
\item Observe that any spectral category is stable: tensor with $S^{-1}\in\Sp^\Sigma$ to desuspend.
\item Indicate why if $\calC$ is stable, the $\Sp(\calC)$ is Quillen equivalent.
\end{enumerate}
...
\begin{itemise}
\item $\calM\times \calD\to\calM$ written $(X,d)\mapsto X\otimes d$. \rednote{(tensored)}
\item $\calD^\op\times\calM\to\calM$ written $(d,Z)\mapsto Z^d$. \rednote{(cotensored)}
\item $\calM^\op\times\calM\to\calD$ written $(X,Z)\mapsto \underline{\calM}_\calD(X,Z)$. \rednote{(enriched?)}
\end{itemise}
satisfying
...
\item A \textbf{symmetric sequence} over $\calC$ is $X=\{X_n\}$ with a left action of $\Sigma_n$ on $X_n$.
\item A \textbf{symmetric spectrum} over $\calC$ is such with coherently associative \rednote{[ask]} $\Sigma_p\times\Sigma_q$-equivariant morphisms $S^p\otimes X_q\to X_{p+q}$. A morphism of such is just a compatible collection of $\Sigma_n$-equivariant maps. Call this category $\Sp^\Sigma(\calC)$.
\end{itemise}
\subsubsection*{Relationship with $\Sp^\Sigma$}
\begin{itemise}
\item Suppose $X\in\Sp^\Sigma(\calC)$ and $K\in\Sp(\sSet_*)=\Sp^\Sigma$. Then define the following colimit:
\[(K\otimes X)_n=\bigvee_{p+q=n}\Sigma_n^+\otimes_{\Sigma_p\times\Sigma_q}(K_p\otimes X_q).\]
The smash product $K\wedge X\in\Sp(\calC)$ is the coequaliser of the maps in $\calC^\Sigma$
\[\xymatrix{
K\otimes S\otimes X\ar@<.5ex>[r]\ar@<-.5ex>[r]&K\otimes X
}\]
given by the action of $S=(S^0,S^1,\ldots)$ on $X$ and $K$ respectively. \rednote{[I'm not sure how this action works, or what the point is. See 3.6.3.]}
\item For $X,Y\in\Sp(\calC)$, define $\Hom_{\Sp(\calC)}(X,Y)\in \Sp^\Sigma$ as follows.
\begin{itemise}
\item Define $\sh_n:\Sp(\calC)\to\Sp(\calC)$ by $(\sh_n(X))_m=X_{n+m}$. This has a left over $\Sigma_n$ action.
\item Define $\Hom_\Sigma(X,Y)\in\sSet_*^\Sigma$ by $\Hom_\Sigma(X,Y)_n:=\Map(X,\sh_nY)$, which has a $\Sigma_n$ action.
\item Define $\Hom_{\Sp(\calC)}(X,Y)\in\Sp^\Sigma$ as the equaliser of the two maps:
...
%\textit{More to say. Could just say at this point that $\Sp(\calC)$ ends up tensored and enriched over the symmetric monoidal category of symmetric spectra $\Sp^\Sigma$.}
\item There's an adjunction:
\[F_n:\calC\longleftrightarrow\Sp(\calC):\Ev_n.\]
Set $\Ev_n(X)=X_n$, and $(F_nX)_m=\Sigma_m^+\otimes_{\Sigma_{m-n}}S^{m-n}$. \rednote{[This is some coequaliser?]}
\end{itemise}
...
\item \textit{If $\calC$ is a simplicial, cofibrantly generated model category, this gives a cofibrantly generated model category, with an SP7 in the level model structure on symmetric spectra.} \rednote{This seems weird, as the SP7 doesn't seem to care much for the model category structure of $\calC$. Maybe it shouldn't. Maybe it does, via the assumptions on $\calC$.}
\item This also makes $\Sp^\Sigma$ a simplicial model category!!!! We'll use this very soon. \rednote{[Figure out the logic here. Are we using what we already have to define the category of Symmetric spectra simultaneously?]}
\end{itemise}
...
\item \textit{Let $\calC$ be a simplicial, cofibrantly generated, proper, stable model category.
Then $\Sp(\calC)$ supports the structure of a spectral model category such that the adjoint functors $\Sigma^{\infty}$ and evaluation $\Ev_0$ are a Quillen equivalence
between $\calC$ and $\Sp(\calC)$ with the stable model structure.}
\item \rednote{[See proof on page 22, second paragraph break.]}
\end{itemise}
