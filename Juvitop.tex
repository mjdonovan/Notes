% !TEX root = z_output/Juvitop.tex
%%% BEGIN NEW ENTRY FORMAT
%
%\begin{}
%\KanSemResponse
%{``Title'' --- Speaker --- DD/MM/2011}
%\begin{abstract}
%
%\end{abstract}
%
...
%Not so good:
\excludeversion{LuisKoszulDuality} % very incomplete
\excludeversion{OlgaLevelStructuresAndAutomorphisms} % will never finish

\def\excludeversion#1{\includeversion{#1}}

%Babytop Fall 2011
\excludeversion{GeoffroyTopologicalHochschildHomology} %ok
\excludeversion{RuneBabytop} %ok
\excludeversion{SaulCrystallineCohomology} %ok

%Misc Fall 2011
\excludeversion{BehrensGoodwillieCalcIntro} %ok

%Juvitop Fall 2011
\excludeversion{SaulSimplicialLocalisation} %ok
\excludeversion{AlexandreBousfieldLocalisation} %ok
\excludeversion{InnaSimplicalModelCats} %ok
\excludeversion{MichaelStableModelCats} %ok
\excludeversion{SaulInftyOneCats} %ok
\excludeversion{GeoffroyWeakAssociativityandCompleteSegalSpaces} %unchecked, looks ok-ish
\includeversion{InnaKTheory} %ok
...
\end{exmp*}
Meanwhile, we have a Quillen equivalence \smash{$|\DASH|:\sSet\longleftrightarrow\Top:\Sing.$}
...
\item $F$ preserves filtered homotopy colimits. That is, whenever $X$ is a CW complex, with a filtration by subcomplexes $X_1\subset X_2\subset\cdots$, then \smash{$\pi_* F(X)\overset{\sim}{\from}\varinjlim \pi_*F(X_i)$} 
\end{enumerate}
\begin{defn*}
...
\begin{thm*}[Goodwillie; orig.\ Brown, Adams]
...
We do this by constructing a new category, whose objects are the same as those of $\calE$, but in which the morphisms $X\to Y$ are the descriptions of $X$ as a subquotient of $Y$.\footnote{One might hope that this would split short exact sequences, after all, in a short exact sequence \smash{$\xymatrix{
0\ar[r]&
A\ar[r]&
B\ar[r]&
C\ar[r]&
...
\begin{alignat*}{2}
K_i(\calC)&=\pi_i(\Omega B(B(\textup{iso}\,\calC)))%&\qquad&\text{()}
\\&=\pi_{i+1}\left(B\left(\coprod_{n\geq0}B\Sigma_n\right)\right)%&\qquad&\text{()}
\\&=\pi_i^s%&\qquad&\text{()}
%\\&=%&\qquad&\text{()}
%\\&=%&\qquad&\text{()}
\end{alignat*}
...
with direct sums taken over the maximal ideals $\frakm$ of $R$. Here we define $K_*(R)$, for $R$ a ring, to be the $K$-theory of the category of finitely generated projective $R$-modules.
 ...
 Let $\calA$ be the category of finitely generated projective $R$-modules, and $\calB$ the full subcategory of torsion modules. Then $\calA/\calB$ is equivalent to the category of finitely generated (projective) $F$-modules, so that $K_*(\calA/\calB)$ with $K_*(F)$. Devissage identifies $K_*(\calB)$ with \smash{$\displaystyle\bigoplus_{\frakm}K_*(R/\frakm)$}.
