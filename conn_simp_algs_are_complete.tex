...
Since the release of \cite{BK_pairings.pdf} and \cite{BousKanSSeq.pdf}, the relationship between the two approaches has been clarified by the introduction of the theory of cubical diagrams to homotopy theory \cite{GoodwillieCalcII}. %Our calculation will rely on a relationship between cosimplicial objects and cubical diagrams first noted by Sinha,[Munson-Voli\'c]. 
Our approach to proving the completeness theorem will be to define the Adams tower of a simplicial algebra using a construction analogous to Bousfield and Kan in \cite{BK_pairings.pdf} (Section \ref{sec:derWRTab}), and to use the theory of cubical diagrams %(cf.\ \cite[\S6]{SinhaSpacesOfKnots.pdf}, \cite{CubicalHomotopyTheory.pdf}) 
...
%\[\calP_0[n]
%=
%\left\{S\subseteq [n]\,\middle|\, %S\neq\emptyset\right\}\quad \textup{and}\quad \calP[n]=\left\{S\subseteq [n]\right\}.\]
%These sets are partially ordered by inclusion, and we think of them as poset categories,
Then an $(n+1)$-cube in $s\calC$ is a functor $\calP[n]\to s\calC$, and the % the category indexing $(n+1)$-cubical diagrams.
%We may observe that $(D_{n+1}F)X$ is simply (the iterative definition of) the homotopy total fiber of 
...
%Although $(D_{n+1}F)X=\textup{hototfib}({D}_{n+1}^{\smash{\square}}F)X$ under an iterative definition of the homotopy total fiber, by \cite[Prop 3.6]{LuisGoodwillie.pdf} the Adams tower is homotopy equivalent to the tower with terms:
%\[({D}_{n+1}^{\smash{\square}}F)X:= \hofib(({D}_{n+1}^{\smash{\square}}F)(\emptyset)\to\holim(({D}_{n+1}^{\smash{\square}}F)|_{\calP_0[n]}))\]
...
%\[E^0_{p,q}=\begin{cases}
%N_q\bigl(P^{p}(bA)/P^{p+1}(bA)\bigr),&\textup{if }{p\geq s};\\
%0,&\textup{o.w.}
%%\\,&\textup{if }
%\end{cases}\]
\[E^0_{p,q}=N_q\bigl(P^{p}(bA)/P^{p+1}(bA)\bigr)\textup{ if $p\geq s$, and }E^0_{p,q}=0\textup{ if $p<s$.}\]
%N_q\bigl(\coker(P^{p+1}(bA)\to P^{p}(bA))\bigr)\cong N_q\bigl((QbA)^{\otimes (s+p)}_{\Sigma_{s+p}}\bigr).\]
