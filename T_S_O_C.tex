...
\end{prop}
\subsection{(Co)homology theories and their connection with spectra}
...
\itm For every CW-triad $(X;A,B)$ (i.e. $A\cup B=X$) with $A\cap B=C$ we have a
...
\setcounter{thm}{12}
\begin{const}
Given a (co)homology theory on $\scrS$, we obtain a reduced (co)homology theory
on $\scrK^\bullet$ by composing with $\Sigma^\infty$.
\end{const}
\begin{lem}
...
\end{lem}
\begin{defn}
Let $\scrL$ be one of the above categories of spectra. A (co)homology theory on
$\scrL$ is called \emph{additive} if whenever $\{X_\lambda\}\subset\scrL$ is a
family whose wedge is still inside $\scrL$:
...
We make the same definitions for reduced (co)homology theories on
$\scrK^\bullet$. An unreduced (co)homology theory is additive when the
corresponding reduced theory is, which is to say that the isomorphisms occur
...
\end{nonexample}
\begin{prop}
Every reduced (co)homology theory $\widetilde h$ on $\scrC^\bullet$,
$\scrC_\textup{fd}^\bullet$ or $\scrC_\textup{f}^\bullet$ is obtained from a
(co)homology theory $h$ on $\scrS$, $\Specf$ or $\Specfd$ via construction 3.13
(composition with $\Sigma^\infty$). $h$ is unique up to equivalence, and it
additive iff $\widetilde h$ is.
\end{prop}
\begin{prop}\hfil
\begin{itemise}
\itm[(i)] Let $\phi$ be a morphism of reduced (co)homology theories on
$\scrC_\textup{f}^\bullet$. If $\phi$ is an isomorphism on $S^0$ then it is an
equivalence.
\itm[(ii)] Let $\phi$ be a morphism of reduced \textbf{additive} (co)homology
theories on any of the above categories $\scrK^\bullet$ or $\scrL$ of spaces or
spectra. If $\phi$ is an isomorphism on $S^0$ then it is an equivalence.
\end{itemise}
\end{prop}
...
$\scrS$.
\itm[(ii)] If $h$ were already defined on $\scrS$, the natural map $h\to k$ is
an equivalence.
\end{itemise}
