...
\[\{i^*_\lambda\}:[\vee_\lambda E(\lambda),F]\to\prod_\lambda[E(\lambda),F],
\qquad\{i^*_\lambda\}(f)=\{fi_\lambda\}.\]
That is: wedge is the \lparen arbitrary\rparen\ coproduct.
\itm[(ii)]For all $F$ there is an isomorphism of abelian groups:
\[\langle(i_k)_*\rangle:\bigoplus_{k=1}^m\lambda[F,E(k)]\to
[F,\vee_{k=1}^m E(k)]\]
That is: wedge is the finite product.
...
That is: wedge acts as the \lparen arbitrary\rparen\ product if $F$ is finite.
\end{itemise}
\end{prop}
...
\end{prop}
\begin{ToFollow}
A useful mapping telescope construction for spectra.
\end{ToFollow}
}   %%% End comment %%%
...
\end{defn}
\begin{ReadersNote}
By II.1.16, any representable cohomology theory $X\mapsto E^*(X)=[X,E]$ is
additive.
\end{ReadersNote}
\begin{nonexample}
...
\end{lem}
\begin{lem}
Direct limits commute, in the sense that if $\Lambda,\Lambda'$ are two quasi-ordered sets:
...
derived functors arising from any short exact sequence of inverse systems:
...
}\scrR(\scrA)\to0$ is a flabby resolution, and $\varprojlim\phi=\delta$.
\end{proof}
\begin{ToFollow}
...
providing a cononical a-campact resolution, which can be used to compute the
derived functors.
\end{ToFollow}
}   %%% End comment %%%
...
there is a spectrum $X$ and morphisms $in:X_n\to X$ such that:
\begin{itemise}
\itm[(i)] $i_{n+1}f_n\simeq i_n$;
\itm[(ii)] The induced map $\varinjlim\pi_*(X(n))\to\pi_*(X)$ is an isomorphism;
\itm[(iii)]There is an exact sequence: $0\to\varprojlim^1 h^{k-1}(X(n))\to
...
$X$. By III.1.15, $\widehat h^*$ is a functor $\scrS\to\AG$.
\end{const*}
\begin{prop}
...
h^*$ turns arbitrary unions into inverse limits. In particular, $\widehat
h^*(\vee X_\beta)=\prod\widehat h^*(X_\beta)$.
\end{prop}
\begin{thm}
...
\itm[(ii)] For every cohomology theory $h^*$ on $\scrS$ and every spectrum $X$,
the natural map $\rho:h^*(X)\to\widehat h^*(X)$ is surjective.
\itm[(iii)]\lparen By a little more than Yoneda's lemma:\rparen\ Every morphism
$\phi:E^*(\DASH)\to\F^*(\DASH)$ of cohomology theories on $\Specf$ is induced by
a morphism of spectra, which is unique up to \textbf{weak} homotopy. A
representing spectrum is unique up to equivalence.
\end{itemise}
\end{thm}
\begin{thm}
