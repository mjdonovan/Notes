...
for $\{E(\lambda)\}$ any family\rparen:
...
spectral sequence:
\[E_2^{pq}={\textstyle\varprojlim^p}F^q(X_\lambda)\implies F^*(X).\]
`Suitable $F$' includes any $F$ for which there is some $N$ such that
$\varprojlim^p F^*(X_\lambda)=0$ for all $q>N$ and all $X\in\scrS$ \lparen
resp.\ $\CW$\rparen.
\end{thm}
\begin{proof}
