...
\begin{Calculus II}
\section*{Calculus II}
We'll discuss functors $\calc_Y\to\cald$, for some (unpointed) base topological space $Y$. Here $\calc_Y$ is either $\calu_Y$ or $\calt_Y$, and $\cald$ is $\calu$, $\calt$ or $\cals p$.
...
\begin{itemise}
\setlength{\parindent}{.25in}
\item Taking a colimit seems like the right thing to do \textbf{when $F$ is stably 1-excisive}. In this case, its terms get closer and closer to it.
...
\subsubsection*{$f$ is analytic}
$f:C\to\R$ is analytic \Iff condition \textbf{(A)} holds:\\
\textbf{Condition (A):} 
for every compact set $K\subset C$ there exists a constant $C$ such that for every $x_0\in K$ and every non-negative integer $n$ the following estimate holds:
\[\left|\frac{d^nf }{dx^n}(x_0)\right|\leq C^{n+1}n!.\]
\textbf{Condition (B):} There exist constants $q\in\Z$ and $\rho\in\Z_{\geq0}$ such that for all $x_\emptyset\in C$:
\[\left|\frac{d^nf }{dx^n}(x_\emptyset)\right|
\leq e^{-q}\cdot e^{(n-1)\rho}
.\]
\emph{This would be stronger than \textup{(A)}, except that the constant $q$ could be negative.}\\
\textbf{Condition (C):}
For $q,\rho$ as above, for all $x_\emptyset,x_1,\ldots,x_n$ such that $|x_i-x_\emptyset|\leq e^{-{(\rho+1)}}$:
\[\left|\sum_{U\in \calP(n)}(-1)^{n-|U|}f\left(x_\emptyset+\sum_{i\in U}(x_i-x_\emptyset)\right)\right|
\leq e^{-q}\cdot e^{(n-1)\rho}
\cdot\prod_{i=1}^n|x_i-x_\emptyset|\]
\emph{This is actually stronger than \textup{(B)} under the assumption that all the derivatives actually exist.}
\subsubsection*{An analogue of condition (C)}
\begin{itemise}
\setlength{\parindent}{.25in}
\item $x_\emptyset,x_1,\ldots,x_n$ by CW-compexes with CW-inclusions $\alpha_i:X_\emptyset\to X_i$ for each $i$, such that:
\[\dist(\alpha_i)\leq e^{-(\rho+1)}\quad \textup{i.e.}\quad \textup{$\alpha_i$ is $k_i$-connected where $k_i\geq\rho+1$.}\]
\item $x_\emptyset+\sum_{i\in U}(x_i-x_\emptyset)$ by $\scrx_U$, where $\scrx$ is the corresponding pushout cube.
\item The alternating sum by the total fibre $\widetilde{f}(F(\scrx))$.%footnote:
...
\item The absolute value (on the LHS) by the distance from the total fibre to the terminal object ($Y$). That is, $e^{-\cartesianness(\widetilde{f}(F(\scrx)))}$.
\item Each absolute value (on the RHS) by the distance $\dist(\alpha_i)$.
\end{itemise}
Condition (C) becomes:
...
\end{defn*}
\noindent Thus we have seen that $F$ is $\rho$-analytic \Iff it satisfies an appropriate analogue of condition (C), (as in the definition, the test cube can always be taken to be a pushout cube%footnote:
