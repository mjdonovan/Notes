...
\section*{Calculus II}
\section*{Analogies:}
If $D\subset \R$ is a domain, $f:D\to\R$ is analytic \Iff condition (A) holds:\\
\textbf{Condition (A):} 
for every compact set $K\subset D$ there exists a constant $C$ such that for every $x_0\in K$ and every non-negative integer $n$ the following estimate holds:
\[\left|\frac{d^nf }{dx^n}(x_0)\right|\leq C^{n+1}n!.\]
\textbf{Condition (B):} There exist constants $q\in\Z$ and $\rho\in\Z_{\geq0}$ such that for all $x_\emptyset\in D$:
...
\item The distance $\dist(f)$ from $X$ to $Y$ along $f:X\to Y$ should vary inversely with $\conn(f)$:
\[\dist(f):=e^{-\conn(f)}.\]
\end{itemize}
\subsection*{An analogue of condition (C)}
\begin{itemise}
\setlength{\parindent}{.25in}
...
\subsection*{The definition of $\rho$-analyticity}
\begin{defn*}
...
\section*{Connectivity and cubes}
\begin{prop*}[II.1.5]
\[\vcenter{
...
\[\conn(FX\downarrow FY)\geq -c+(n+1)\cdot\conn(X\downarrow Y).\]
