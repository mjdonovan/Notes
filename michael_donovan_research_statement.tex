...
\item `vertical' $\delta$ operations $\delta_i^\textup{v}:E^r_{-s,t}\to E^r_{-s-1,t+i+1}$, defined for $i$ satisfying $2\leq i<t-(r-2)$,
\end{itemise}
subject to certain compatibilities.
\end{conjecture}
That this conjecture is the summary of a rather large body of work, and relies on one remaining ingredient, about which I have strong beliefs, but which I have not yet fully investigated. I expect to complete the proof before long.
...
where $\hat X_\textup{AQ}$ is the completion of $X$ with respect to Andr\'e-Quillen homology.
...
We independently prove a vanishing line theorem:
...
Give a generating set of operations for all Adams spectral sequences
...
Although I will not give the details of the derivation of these spectral sequences here, they are generalized Grothendieck spectral sequences as introduced by Blanc and Stover [ref]. It is unusual, even in topology, to see such an infinite sequence of spectral sequences, and to make them effective as a calculation tool, one must endow them with extra structure. I prove
\begin{thm}
