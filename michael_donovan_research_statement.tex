...
%
%I think it's best to restrict to simply connected spaces when you talk about the unstable Adams spectral sequence.
%
%Nice statement. How about a little speculation at the end? At least about the E_1 monad and so on? How about other aspects of the homotopy theory of simplicial commutative algebras? or the Blanc-Stover spectral sequence machinery? or in fact classical homotopy theory - what does your method have to say about David Blanc's E_2 term (derived functors of indecomposables of a \Pi-algebra)? 
...
My research has so far focused on the unstable Adams spectral sequence for simplicial commutative algebras. The \emph{stable} Adams spectral sequence for topological spaces  was introduced by Adams in 1958 \cite{MR0096219}, and its unstable counterpart was introduced by Bousfield and Kan in 1972 \cite{BousKanSSeq.pdf,BK_pairings.pdf}. On the other hand, the unstable Adams spectral sequence for simplicial commutative algebras was constructed by Radulescu-Banu only in 2006 \cite{radelescuBanu.pdf} (see also \cite{BlumRiehlResolutions.pdf}). Deeper study of the  classical case has since been performed by numerous authors, for example Bousfield and Kan's construction  \cite{BK_pairings_products.pdf} of a Whitehead product on the classical unstable Adams spectral sequence. The unstable Adams spectral sequence was also generalised to other cohomology theories by Bendersky, Curtis and Miller in \cite{BendCurtisMiller.pdf}. These authors made the first progress toward computation of the spectral sequence, and that work has continued ever since, in the hands of Bendersky, Thompson, Tilman Bauer, and others. Such study of the simplicial algebras case had not been performed, and has been a focus of my research.

Motivated and inspired by this and other work, I took on the task of performing this analysis for the Adams spectral sequence for simplicial algebras. As it turns out, a lot more is possible in the simplicial algebra context than in the context of simplicial spaces.
The next section will be a summary of the main results I have proved, and conjectures I have made. Following this section, I will remind the reader of the definition of \emph{spectral sequences}, and make some comments on classical unstable Adams spectral sequence for comparison with my work. Following this I will discuss operations in the Adams spectral sequence for simplicial algebras. The final section will discuss the structures I have had to study in order to gain access to the Adams $E^2$ page --- an infinite regress of composite functor spectral sequences.
...
Radulescu-Banu has constructed in \cite{radelescuBanu.pdf}, for $X$ a simplicial algebra, the \emph{completion $X{\hat{\,}_\textup{\!\!AQ}}$ of $X$ with respect to Andr\'e-Quillen homology}. Further, he defines an Adams spectral sequence converging to the homotopy $\pi_*(X{\hat{\,}_\textup{\!\!AQ}})$. Reframing his construction in terms of constructions of Blumberg and Riehl \cite{BlumRiehlResolutions.pdf} and Bousfield \cite{BousCosimpResnHtpySS.pdf}, one can identify the $E^2$ term:
...
The $E^2$ term of a spectral sequence gives an upper bound for the groups to which it converges, in this case the homotopy groups $\pi_*(X{\hat{\,}_\textup{\!\!AQ}})$, as there may be differentials in the spectral sequence, causing cancellation.
For background of spectral sequences, see the beginning of section \ref{backgroundSection}. I prove the following two convergence theorems, which show that the spectral sequence does indeed calculate as desired. Firstly, a completeness theorem, originally conjectured by Radulescu-Banu \cite[\S1.6.4]{radelescuBanu.pdf}, and the analogue of the classical fact that simply-connected spaces are $\Z$-complete:
\begin{theorem}
...
\end{theorem}
Next, a vanishing line theorem, the analogue of a result of Bendersky \cite{BenderskySomeCalculations.pdf}:
...
\end{enumerate}
\end{theorem}
Now it is possible to state the computation of $E^2$:
...
\end{conjectureAlpha}
This conjecture is an analogue for the Adams spectral sequence of the results from [Goerss, Theorem 6.2, prop 6.5] regarding Quillen's spectral sequence, and I hope to complete its proof soon. %It is the summary of a rather large body of work, and relies on one remaining ingredient, namely the final conjecture in this document. I do not expect this result to be particularly challenging, but I have not yet fully investigated it.
%
...
%\end{conjecture}
It may be viewed as a measurement of the failure of Koszul duality for the commutative monad over $\F_2$ --- rationally, the Koszul double dual of the commutative monad is again the commutative monad, but over $\F_2$, double dualization creates new operations of two different kinds.

Using this calculation, we will be able to calculate the Adams filtration on the homotopy groups of $S^d_\calC$. One discovers that the Adams spectral sequence runs rather faster than Quillen's spectral sequence \cite[\S6]{MR1089001} and \cite[\S6]{quillenCohCommRings.pdf}, which is the analogue of the classical lower-central-series spectral sequence, c.f.\ \cite{Curtis_LCS.pdf,Rector_UnstAdams.pdf}. This is an echo of the results of Bousfield and Curtis \cite{BChtpySSniceSpaces.pdf} in classical topology, which show that for `nice' topological spaces, the unstable Adams spectral sequence is a sped-up version of the lower-central-series spectral sequence.
...
Assuming conjecture \ref{E2monadConjecture}, we may describe ${^{\textup{M}\!}}E^1_{***}$ as the free $\calC\calA\Delta$-algebra on the underlying vectorspace of $H_*X$. Without this assumption, a less tidy description can be given.
\end{theorem}

Finally, to give a sense of the tools used in order to perform my analysis of the Adams spectral sequence $E^2$ page, I state:
\begin{theorem}
...
Now the Adams spectral sequence serves to calculate the homotopy groups $\pi_{t-s}(X{\hat{\,}_\textup{\!\!AQ}})$ of the  completion of $X$ with respect to Andr\'e-Quillen homology. These groups support a commutative algebra structure, and an unstable action of operations $\delta_i$ \cite{DwyerHtpyOpsSimpComAlg.pdf, BousOpnsDerFun.pdf}. Drawing inspiration from further work of Dwyer \cite{DwyerHigherDividedSquares.pdf}, and from Bousfield and Kan \cite{BK_pairings_products.pdf}, I prove:
\begin{theorem}
The commutative product, $\delta^\textup{v}$-operations and $\Sq_\textup{h}$-operations defined in theorem \ref{KoszulOperationsDefinedOnE2} extend as appropriate to the higher pages of the Adams spectral sequence. They are compatible with the commutative product and $\delta$-operations on the target $\pi_{t-s}(X)$ of the spectral sequence.
\end{theorem}
The construction involves three elements: a dual version of the constructions of \cite{DwyerHigherDividedSquares.pdf}, the direct construction of a product map on the Adams resolution, and a lifting of the resulting pairing on the spectral sequence into the correct filtration. Having constructed the operations, I give a rather intricate argument shows that they coincide with the `Koszul' operations as originally defined on $E^2$.
...
Now given that we have calculated the $E^2$ page of the Adams spectral sequence for $S^d_\calC$, and that we know the homotopy of $S^d_\calC\sim (S^d_\calC){\hat{\,}_\textup{\!\!AQ}}$, we might hope to be able to calculate all of the differentials.
\begin{conjectureAlpha}
All of the differentials in the Adams spectral sequence for $S^d_\calC$ are determined by simple commutation relations between the product, Steenrod operations, $\delta$ operations and the differential.
\end{conjectureAlpha}
...
\emph{Spectral sequences} are a powerful theoretical and computational tool, common in algebraic topology. %However, they have a reputation for being complicated, so we'll remind the reader of a definition.
For us, a \emph{spectral sequence}%footnote:
\footnote{In detail, our spectral sequences will have one homological grading and $n+1$ cohomological gradings, written as $E^r_{-s_{n+1},\ldots,-s_1,t}$, and differentials $d^r$ of degree $(-r,r-1,0,\ldots,0)$. For the Adams spectral sequence, $n$ will equal $0$.} will be a sequence $\{E^r\,;\,2\leq r<\infty\}$ of graded $\F_2$-vector spaces with a differential $d^r:E^r\to E^r$ for each $r$ such that $E^{r+1}$ is the homology $H(E^r;d^r)=\ker(d^r)/\im(d^r)$.
...
We'll explain further by the example of the unstable Adams spectral sequence for topological spaces, which indeed motivated the present work. It was introduced as a tool for computing the homotopy groups of spheres. Given a connected topological space $X$, the homotopy groups $\pi_*(X{\hat{\,}_\textup{\!\!2}})$ of the $2$-completion of $X$ have a natural decreasing filtration, and the $E^r_{-s,t}\implies \pi_{t-s}(X{\hat{\,}_\textup{\!\!2}})$, i.e.\ 
\[E^\infty_{-s,t}=F_s\pi_{t-s}(\hat X_2)/F_{s+1}\pi_{t-s}(X{\hat{\,}_\textup{\!\!2}}).\]
...
taken in the category of unstable coalgebras over the Steenrod algebra \cite{BousKanSSeq.pdf}. The question of how the homotopy groups of the completion $X{\hat{\,}_\textup{\!\!2}}$ determine those of $X$ has been answered in particular when $X$ is a nilpotent space with finitely generated homotopy groups, in which case $\pi_*(X{\hat{\,}_\textup{\!\!2}})=(\pi_*(X)){\hat{\,}_\textup{\!\!2}}$ is the group-theoretic $2$-completion, allowing us to recover the $2$-component of $\pi_*(X)$. In particular, a full calculation of this spectral sequence for each sphere $X=S^d_\calC$ would yield a full calculation of the 2-torsion in the homotopy groups of spheres.
...
Although I will not give the full details of the derivation of these spectral sequences here, they are generalized Grothendieck spectral sequences as introduced by Blanc and Stover \cite{Blanc_Stover-Groth_SS.pdf}. They all have essentially the same construction. For example, ${^{0\!}}E^2_{-s,t}=\mathbb{R}^s\textup{Prim}(H_*X)$ is the right derived functors of the primitives functor $(\calL{\hat{\,}}\calP{\hat{\,}})\textup{-coalgs}\to \textup{vect}$, and we find that this functor factors as
...
We'll end with a theorem that we use, along with the results of May's thesis on the cohomology of restricted Lie algebras \cite{MayRestLie.pdf}, in the proof of theorem \ref{AdamsE2PageCalculation}.
\begin{thm*}
...
I would like to investigate whether the resolution techniques used to build the Adams spectral sequence extend to define a K\"unneth spectral sequence for simplicial algebras, which calculates the Andr\'e-Quillen homology of a product, or even a homotopy limit, of simplicial algebras. Such work would correspond to Stover's van Kampen spectral sequence \cite{StoverVanKampen.pdf} in classical topology.

A technique used here which may have useful generalisations is that used in constructing operations on the composite functor spectral sequences described above. I would like to attempt to make general statements about Koszul (co)actions on Blanc and Stover's generalised composite functor spectral sequences. I would like to investigate the possibility of finding obstruction-theoretic applications for such structure.
...
In \cite{radelescuBanu.pdf}, Radulescu-Banu shows how to form a homotopically correct Adams resolution for a simplicial commutative algebra $X$. He equips the cofibrant replacement functor $c$ with the structure of a comonad, after which the sequence
\[\frakX^s:=c(KQc)^{s+1}X\]
