...
Goerss [ref] showed that the Andr\'e-Quillen homology $H_*X$ of $X$ is a Lie coalgebra with an unstable left coaction of the dual of a certain algebra $\calP$ of cohomology operation. One might say that Goerss calculated the Koszul dual comonad to the commutative monad over $\F_2$; we will denote this monad $\calL\hat{\,}\calP\hat{\,}$. A main result of my work will be to complete the proof of the following
\begin{conjecture}
The monad $\calC\calA\Delta$ which is the Koszul dual of the comonad $\calL\hat{\,}\calP\hat{\,}$ has algebras the bigraded vector spaces $W_{-s,t}$ equipped with:
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item a commutative product $W_{-s,t}\otimes W_{-s',t'}\to W_{-s-s'-1,t+t'+1}$;
\item `horizontal' Steenrod operations $\Sq^i_\textup{h}:E^r_{-s,t}\to E^r_{-s-i,2t+1}$, zero unless $3\leq j\leq s+1$; and
...
\end{conjecture}
That this conjecture is the summary of a rather large body of work, and relies on one remaining ingredient, which is the final conjecture in this document. I do not expect this result to be particularly challenging, but I have not yet fully investigated it.
...
The $E^2$ page of the Adams spectral sequence for $S^d$ is the free $\calC\calA\Delta$-algebra on the fundamental class $\imath\in E^2_{0,d}$.
\end{conjecture}
This is an analogue for the Adams spectral sequence of the results from [Goerss, thm 6.2, prop 6.5] regarding the Quillen spectral sequence.
A corollary of this would be the existence of the following `May-Koszul' spectral sequence for calculation of the Adams $E^2$ page, formed by filtering the comonad $\calL{\hat\,}\calP{\hat\,}$ by a certain length filtration.
\begin{cor*}
...
Now the Adams spectral sequence serves to calculate the homotopy groups $\pi_{t-s}(X)$ of the  completion of $X$ with respect to Andr\'e-Quillen homology. These groups support a commutative algebra structure, and an unstable action of operations $\delta_i$ [dwyer,bousfield].
\begin{thm*}
...
Now given that we have conjectured the $E^2$ page of the Adams spectral sequence for $S^d$, and that we know the homotopy of $S^d$, we might hope to be able to calculate all of the differentials.
\begin{conjecture}
...
\end{conjecture}
A proof of this conjecture would require a close examination of the operations in question at the chain level, which I have not yet completed.

Finally, it is worth mentioning the following vanishing line theorem, which follows from the first conjecture, or may be proved independently:
