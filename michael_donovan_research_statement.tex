...
Goerss [ref] showed that the Andr\'e-Quillen homology $H_*X$ of $X$ is a Lie coalgebra with an unstable left coaction of the dual of a certain algebra $\calP$ of cohomology operation. One might say that Goerss calculated the Koszul dual comonad to the commutative monad over $\F_2$; we will denote this comonad $\calL\hat{\,}\calP\hat{\,}$. A main result of my work will be to complete the proof of the following
\begin{conjecture}
...
\item `horizontal' Steenrod operations $\Sq^i_\textup{h}:W_{-s,t}\to W_{-s-i,2t+1}$, zero unless $3\leq j\leq s+1$; and
...
\end{conjecture}
This conjecture is the summary of a rather large body of work, and relies on one remaining ingredient, which is the final conjecture in this document. I do not expect this result to be particularly challenging, but I have not yet fully investigated it.
