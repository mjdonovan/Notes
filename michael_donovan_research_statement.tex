...
Radelescu-Banu has constructed in \cite{radelescuBanu.pdf} an Adams spectral sequence for $X$:
...
\begin{thm*}
It is in fact possible to define the operations suggested on the Adams $E^2$ page:
...
As such we can calculate the Adams filtration on the homotopy groups of $S^d_\calC$. One discovers that the Adams spectral sequence runs rather faster than Quillen's spectral sequence \cite[\S6]{MR1089001} and \cite[\S6]{quillenCohCommRings.pdf}, which is the analogue of the classical lower-central-series spectral sequence, c.f.\ \cite{Curtis_LCS.pdf,Rector_UnstAdams.pdf}. This is an echo of the results of bousfield and Curtis \cite{BChtpySSniceSpaces.pdf} in classical topology, which show that for `nice' topological spaces, the unstable Adams spectral sequence is a sped-up version of the lower-central-series spectral sequence.
...
\end{conjecture}
This conjecture is an analogue for the Adams spectral sequence of the results from [Goerss, thm 6.2, prop 6.5] regarding Quillen's spectral sequence, and I hope to complete its proof soon. %It is the summary of a rather large body of work, and relies on one remaining ingredient, namely the final conjecture in this document. I do not expect this result to be particularly challenging, but I have not yet fully investigated it.
%
...
Next, I prove the following two convergence theorems. Firstly, a vanishing line theorem, the analogue of a result of Bendersky \cite{BenderskySomeCalculations.pdf}:
...
\end{thm*}
Next, a completeness theorem, originally conjectured by Radelescu-Banu \cite[Conjecture 6.1]{radelescuBanu.pdf}, and the analogue of the classical fact that simply-connected spaces are $R$-complete:
\begin{thm*}
...
\end{conjecture}
\noindent I expect to prove this conjecture soon by completing a close examination of the operations in question at the chain level. Finally, to give a sense of the tools used in order to perform my analysis of the Adams spectral sequence $E^2$ page, I state:
\begin{thm*}
...
Each of these spectral sequences supports numerous operations, and these are appropriately compatible under convergence.
\end{thm*}
...
taken in the category of unstable coalgebras over the Steenrod algebra \cite{BousKanSSeq.pdf}. The question of how the homotopy groups of the completion $\hat X_2$ determine those of $X$ has been answered in particular when $X$ is a nilpotent space with finitely generated homotopy groups, in which case $\pi_*(\hat X_2)=\pi_*(X)\hat{_2}$ is the group-theoretic $2$-completion, allowing us to recover the $2$-component of $\pi_*(X)$. In particular, a full calculation of this spectral sequence for each sphere $X=S^d_\calC$ would yield a full calculation of the 2-torsion in the homotopy groups of spheres.
...
Although I will not give the full details of the derivation of these spectral sequences here, they are generalized Grothendieck spectral sequences as introduced by Blanc and Stover \cite{Blanc_Stover-Groth_SS.pdf}. They all have essentially the same construction. For example, ${^{0\!}}E^2_{-s,t}=\mathbb{R}^s\textup{Prim}(H_*X)$ is the right derived funtors of the primitives functor $(\calL{\hat{\,}}\calP{\hat{\,}})\textup{-coalgs}\to \textup{vect}$, and we find that this functor factors as
...
We'll end with a theorem that we use, along with the results of May's thesis on the cohomology of restricted Lie algebras \cite{MayRestLie.pdf}, in the proof of the main theorem.
\begin{thm*}
