...
\[\xymatrix{
0\ar[rr]|{\ \cdot\ \cdot\ \cdot\ }&&H^*(F^{p+2})\ar[rr]\ar[ld]_\rho&&H^*(F^{p+1})\ar[rr] \ar[ld]_\rho&&H^*(F^{p})\ar[ld]_\rho
...
Note the self: Next discuss the map as defined. Show that it intertwines the differentials, by noting that the $x_\alpha$ all transgress, and so are killed by any differential which doesn't make it to the opposite edge.
