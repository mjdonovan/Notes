...
&\vdots\ar[d]\\
&\makebox[0cm][r]{$K(\pi_n(X),n)=\,$}X_n\ar[r]^{k_n(X)\ \ \ \ \ \ \ \ \ }\ar[d]&K(\pi_{n+1}(X),n+2)\\
...
$[K,X]$ has a natural group structure.
\end{prop*}
\begin{proof}
$X\to\Omega\Sigma X$ is a map inducing isomorphisms on the low homotopy groups,
by the suspension theorem. Whitehead's theorem shows that these spaces have the
same $2n-1$-type, implying the result, via
...
\begin{lem*}[Not in text]
In the Serre long exact sequence for $K(\pi,n-1)\to*\to K(\pi,n)$ with
coefficients in $G$, the transgression is inverse to the cohomology suspension. In particular:
...
$S^n$ and $K(\Z,n)$ have the same $n+1$-type, $[K,S^n]=[K,K(\Z,n)]=H^n(K)$. This
classical result, the Hopf classification theorem, is dual to the Hurewicz
theorem.
...
and the elements $E_infty^{r,q}$ are the subquotients in a filtration for
$[S^rK,X]$.
