...
\includeversion{Lie algebras in characteristic 2 and their homotopy operations}
%\includeversion{Unstable Lie algebras over Goerss' P-algebra}
...
\excludeversion{Composite functor spectral sequences}
...
In this section, we'll give conditions under which one can define natural operations on the cohomology groups $H^*$. The method here is simply the direct generalisation of Goerss' approach in [GoerssBook,\S5]. \textbf{Pull back from this.} Instead say ``avoid excessive generality''.
...
(where $\mu$ here stands for the multiplication map of the monad $F_\calC U_\calC$). \textbf{housekeeping:} order of exposition. \textbf{relate to next lemma!}
...
\[x=\theta(v)+\sum_{0\leq k< m/2}\sigma_k(w_k),\]
where $v\in(S^2(H_*(NU)))_{m}$ and $w_k\in H_{m-k}(NU)$ ($0\leq k<m/2$). Here, $\theta:S^2\pi_*(V)\to\pi_*(S^2V)$ is the map induced by the Eilenberg-Mac Lane shuffle map $\nabla$:
\[\theta([x]\otimes [y]+[y]\otimes[x])=[\nabla(x\otimes y+y\otimes x)],\ \ \theta([x]\otimes[x])=[\nabla(x\otimes x)],\]
where brackets denote the homotopy class of a cycle. These formulae are well defined as $\nabla$ is symmetric.
...
Here we'll introduce decomposition maps for the categories $\calL(n)$ and $\calW(n)$, and prove a certain compatibility between them. The definition is simple enough, and the reader can verify that it is well defined. Let $\calC$ be any of the categories $\calW(n)$ for $n\geq1$, or $\calL(n)$ for $n\geq0$. Then for $X,Y\in\calC$ we define $j:Q(X\wedge Y)\to QX\otimes QY$ by
\[[z_1,\cdots ,z_a]\lambda_{i_1}\cdots \lambda_{i_r}\overset{j}{\mapsto}\begin{cases}
%0,&\textup{if }r>0\textup{ or }s>2;\\
z_1\otimes z_2,&\textup{if }r=0,\,a=2,\,z_1\in X\textup{ and } z_2\in Y,
\\0,&\textup{otherwise.}
\end{cases}
\]
Here, each of $z_1,\ldots,z_a$ is an element of either $X$ or $Y$, and $[z_1,\ldots,z_s]$ represents some choice of bracketing. The $\lambda$-operations must of course all be the top operation (the restriction) if we are working in $\calL(n)$, where no $\calU(n)$-operations are defined. The map $j_{\calW(0)}$ is defined similarly, replacing right $\lambda$-operations with left $P$-operations. [\textbf{or uniformize} all this using the phrase $\calU(n)$-operations.]
\begin{prop}
For any $V\in\vect{+}{n}$, writing $\calC$ for $\calL(n)$, the composite
\[j_\calC\quadratic:\left(F_\calC V\overset{\quadratic}{\to}(F_\calC V)^{\wedge 2}\to Q_\calC((F_\calC V)^{\wedge 2})\overset{j_\calC}{\to}V^{\otimes 2}\right)\]
equals the composite $F_{\calL(n)} V\to F^{(2)}_{\calL(n)} V\subseteq V^{\otimes 2}$. If $\calC$ stands instead for $\calW(n)$, then $j_\calC\quadratic$ equals the composite $F_{\calW(n)} V\to F_{\calL(n)} V\to F^{(2)}_{\calL(n)} V\subseteq V^{\otimes 2}$.
\end{prop}
\begin{proof}
Consider the case $\calW(n)$ for $n\geq1$. One observes that $j_\calC\quadratic$ vanishes except on quadratic grading 2, and checks the result on terms $[x,y]$, $x\lambda_i$ (for $i<|x|$) and $\restn{x}$:
\begin{alignat*}{2}
j_\calC\quadratic([x,y])&=j_\calC([\overline{x}+\overline{\overline{x}},\overline{y}+\overline{\overline{y}}]+[\overline{x},\overline{y}]+[\overline{\overline{x}},\overline{\overline{y}}])\\
&=j_\calC([\overline{x},\overline{\overline{y}}]+[\overline{\overline{x}},\overline{y}])=x\otimes y+y\otimes x,
\end{alignat*}
which is precisely the representation of $[x,y]$ in $F^{(2)}_{\calC}V\subseteq V^{\otimes2}$. Similarly, $j_\calC\quadratic(x\lambda_i)$ vanishes (as $(\DASH)\lambda_i$ is linear), while $j_\calC\quadratic(\restn{x})$ equals $x\otimes x$ as desired. The other cases, including that of $\calW(0)$, are barely any different.
\end{proof}
...
\[(QB_{p+1}\pi_* L)_{q+i-1}\overset{j_{\calW(n+1)}\circ Q_{\calW(n+1)}\xi_{\calW(n+1)}}{\to}(S^2(QB_{p}\pi_* L))_{q+i-1}\overset{S^2(Q\chi)}{\to}
(S^2(\pi_*QG_pL))_{q+i-1}\overset{\theta}{\to}
