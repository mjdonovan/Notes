...
\todo{Derived functors of $Q^{\calW(n)}:\calW(n)\to \calV(\mathds{1},n)$ are $H^*_{\calW(n)}:\calW(n)\to\calV(n+1,\mathds{1})$}{and we're interested in these for $n=0$.}
...
\item point is that $Q^{\calW(n)}$ factors as these followed by $Q^{\calL(n)}:\calL(n)\to \calV(\mathds{1},n)$}
