...
\item give the $\calV(m,n)$ notation here: finite type with various gradings.}
...
\todoeasy{Define $Q^\calW:\calW\to \calV^1$, and thus the cohomology functor on $\calW$}{Give the bar construction $B_\bullet^{\calW}$
...
\end{align*}
whose images are linearly independent and span the quadratic filtration 2 part $UV^{*,(2)}$ of $UV^*$, and this restricted $P^i$ is an injection. We define $\gamma^i$ to be the composite
...
\[\lambda_{i_1}\cdots \lambda_{i_r}[z_1,\cdots ,z_a]\overset{j_\calW}{\mapsto}\begin{cases}
...
\item Do I insert a section of conventions, including, say model category structures on cugas, categories of vector spaces, good and bad Lie algebras, higher Eilenberg-Zilber maps,\ldots?
\end{enumerate}
