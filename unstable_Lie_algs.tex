...
We'll be interested in certain categories of Lie algebras whose underlying objects lie in $\vect{+}{n}$. For example, a $\vect{+}{n}$-graded Lie algebra is an element $L\in\vect{+}{n}$ with a structure map $\Lambda^2 L\to L$ which treats gradings as follows
...
We'll now give an account of the (\textbf{some?}) homotopy operations on the homotopy of a simplicial (ungraded) $S(\LieOperad)$-, $\Lambda(\LieOperad)$- or $\Gamma(\LieOperad)$-algebra. \textbf{Actually}, let's do a simplicial object of $\calL(n)$.

The homotopy operations we'll present will be in the form of a natural transformation, for $V\in s\vect{+}{n}$:
\[\widetilde{\nabla}:F^{(2)}_{\calL(n+1)}(\pi_*V)\to \pi_*(F^{(2)}_{\calL(n)}V)\]
This map is nothing more than the Eilenberg-Mac Lane shuffle map $\nabla$ disguised. This is a symmetric natural chain map $\nabla:N_*(V)\otimes N_*(V)\to N_*(V\otimes V)$. Then, for cycles $x,y,z\in ZN_*(V)$, we define
\[\widetilde{\nabla}([x]\otimes[y]+[y]\otimes [x])=[\nabla(x\otimes y+y\otimes x)]\textup{, and }\widetilde{\nabla}([x]\otimes[x])=[\nabla(x\otimes x)].\]
\textbf{Actually I don't really see why this should be well defined.}
...
Now for a vector space $V$, define
\[S_2V:=(V^{\otimes 2})_{\Sigma_2}\qquad \textup{and}\qquad S^2V:=(V^{\otimes 2})^{\Sigma_2},\]
define $\trace:S_2V\to S^2V$ by the formula $v\otimes w\mapsto v\otimes w+w\otimes v$.


One can codify the difference 

These different levels of structure will play a central role in what follows. 
