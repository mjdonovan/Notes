...
\todo{Construct monad $U$ for $\calW$}{Write in terms of monads $\calP^\textup{u}$ and $S(\scrL)$.\item Decide whether to give the distributive law.}
...
\item show that the Sq-ops and products are instances of those on the cohomology of a Lie algebra (in particular that $j_\calW$ restricts to $j_\calL$), then bring in argument using spectral sequence and Singer's work.
...
\todo{Construct the relevant elements $\Sq^J\delta_I\imath_\alpha$}{$\imath_\alpha\in H^0_{r_\alpha}X$ is the functional which projects $QX\cong X$ onto $\F_2[r_\alpha]$.
...
\todo{Identify products of $ee\Sq^{J_n}_\textup{v}e\cdots e\Sq^{J_1}_\textup{v}e\delta_I\imath_\alpha\in E^2(n+2)_*^{00**\cdots *}$ in $E^2(n)_*^{*0**\cdots *}$}{I still find this hard to write down}
