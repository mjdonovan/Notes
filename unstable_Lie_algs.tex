...
\excludeversion{Contents Page}
\includeversion{Conventions and notation}
\excludeversion{Lie algebras in characteristic 2 and their homotopy operations}
\excludeversion{Cohomology operations for unstable Lie algebras over P}
\excludeversion{Categories of graded Lie algebras and their homotopy operations}%Seems depreciated
...
as it measures the nonlinearity in an expression in $F_\calC V$. For example, if $\calC$ is the category of non-unital commutative algebras over $\F_2$, then for $v,w\in V$, using dashes to denote the generators coming from second copy of $V$:
\[\crossterms(vw)=(v+v')(w+w')+vw+v'w'=vw'+wv'.\]
...
In the example of commutative algebras, the map $j_\calC$ is defined by $x_1x_2\mapsto x_1\otimes x_2$, for $x_i\in X_i$. We'll write $\quadratic_\calC$ for the composite
...
and in our running example $\quadratic_\calC$ is the composite $F_\calC V\to F_\calC^{(2)} V=S_2V\overset{\trace}{\to}S^2V$.

In our examples, the following equation of maps $F_\calC F_\calC V\to V\otimes V$ will always be satisfied:
\[\quadratic_\calC\circ u_{F_\calC V} +\quadratic_\calC\circ {F_\calC u_V}=\quadratic_\calC\circ\mu_V,\]
where $\mu$ here stands for the multiplication map of the monad $U_\calC F_\calC $, which is to say that for $\calC$-expressions $f$ and $g$ we have $\quadratic(fg)(v)=\quadratic(f)u(g)(v)+u(f)\quadratic(g)(v)$.
...
is essentially $\quadratic_\calC$, in that if $v\in V_s\cap N_sX$ \textbf{(explain?)} represents an element of $N_sQ_\calC X$, writing $d_0v=f(w_j)$ for $w_j\in V_{s-1}$, we have $\psi_\calC(v)=\quadratic_\calC(f)(w_j)\in S^2(V_{s-1})$.
\end{lem}
We will use $\psi_\calC$ to define operations on $\calC$-cohomology. We will construct such operations from certain generic operations, defined for any $U\in s\calV$:
\begin{gather*}
...
Given these external operations, we can define natural $\calC$-cohomology operations on $H^*_\calC X$ (for $X\in s\calC$), by setting $U:=Q_\calC B_\bullet^\calC X$, the standard bar construction, and using the composites
\begin{gather*}
(N_{n_1}U)^*\otimes (N_{n_2}U)^*\overset{m_\textup{ext}}{\to} (N_{n_1+n_2}S^2U)^*\overset{\psi_\calC^*}{\to} (N_{n_1+n_2+1}U)^*,\textup{ and }\\
(N_mU)^*\overset{S^k_\textup{ext}}{\to} (N_{m+k}S^2U)^*\overset{\psi_\calC^*}{\to}(N_{m+k+1}U)^*
\end{gather*}
Note the shift introduced by $\psi_\calC^*$. We \textbf{need to choose notation} for this section.
...
where $w_k\in H_{m-k}(NU)$ ($0\leq k\leq\lfloor m/2\rfloor$), and $y_j,z_j\in H_{*}(NU)$.
Then the map $(\ExtCohOp^k)^*$ sends $x$ to this $x_k$. If we only wish to determine the $(\ExtCohOp^k)^*$ for $k<m/2$ (and not the dual of the top operation $(\ExtCohOp^{m/2})^*$ when $m$ is even), we can rewrite this equation as 
\[x=\widetilde{\nabla}(v)+\sum_{0\leq k< m/2}\sigma_k(w_k),\]
where $v\in(S^2(H_*(NU)))_{m}$ and $w_k\in H_{m-k}(NU)$ ($0\leq k<m/2$). Here, $\widetilde{\nabla}:S^2\pi_*V\to \pi_*S^2V$ is the map that we discuss in \ref{the top homotopy operations for Lie algebras}.
...
\section{Lie algebras in characteristic 2}
...
Fresse [] explains how to construct the monads $S(\LieOperad)$, $\Lambda(\LieOperad)$ and $\Gamma(\LieOperad)$ which give rise to this structure, starting with the Lie operad $\LieOperad$. For a vector space $V$, it is standard that the functor
...
may also be equipped with a monad structure, and that the trace map
\[\trace:S(\LieOperad)(V)\to \Gamma(\LieOperad)(V)\]
is a map of monads. Then, he defines a third monad
...
] For each $n\geq0$, we'll be interested in certain categories of Lie algebras whose underlying objects lie in $\vect{+}{n}$. For example, a $\vect{+}{n}$-graded Lie algebra is an element $L\in\vect{+}{n}$ with a structure map $\Lambda^2 L\to L$ which treats gradings as follows
\[L^{t}_{s_n,\ldots,s_1}\otimes L^{t'}_{s'_n,\ldots,s'_1}\to L^{t+t'+1}_{s_n+s'_n,\ldots,s_1+s'_1}.\]
In other words, we view the Lie operad as an operad in $\vect{+}{n}$, such that
\[\LieOperad(r)=(\LieOperad(r))^{r-1}_{0,\ldots,0},\]
and then a $\vect{+}{n}$-graded Lie algebra is an algebra over the corresponding monad $\Lambda(\LieOperad)$ on $\vect{+}{n}$.
...
The monad of this adjunction is the appropriately chosen submonad $\Gamma'(\LieOperad)$ of $\Gamma(\LieOperad):\vect{+}{n}\to \vect{+}{n}$. As before, the free construction $F_{\calL(n)}V$ admits a quadratic grading, which we denote $F_{\calL(n)}^{(k)}V$ or $\Gamma'(\LieOperad)^{(k)}V$.
...
Here we'll introduce decomposition maps for the categories $\calL(n)$ and $\calW(n)$, and prove a certain compatibility between them. The definition is simple enough, and the reader can verify that it is well defined. Let $\calC$ be any of the categories $\calW(n)$ for $n\geq1$, or $\calL(n)$ for $n\geq0$. Then for $X,Y\in\calC$ we define $j_\calC:Q(X\wedge Y)\to QX\otimes QY$ by
\[[z_1,\cdots ,z_a]\lambda_{i_1}\cdots \lambda_{i_r}\overset{j_\calC}{\mapsto}\begin{cases}
%0,&\textup{if }r>0\textup{ or }s>2;\\
z_1\otimes z_2,&\textup{if }r=0,\,a=2,\,z_1\in X\textup{ and } z_2\in Y,
\\0,&\textup{otherwise.}
\end{cases}
\]
Here, each of $z_1,\ldots,z_a$ is an element of either $X$ or $Y$, and $[z_1,\ldots,z_s]$ represents some choice of bracketing. If we are working in $\calL(n)$ then all of the $\lambda$-operations must be the top operation (the restriction), as where no $\calU(n)$-operations are defined. The map $j_{\calW(0)}$ is defined similarly, replacing right $\lambda$-operations with left $P$-operations. [\textbf{or uniformize} all this using the phrase $\calU(n)$-operations.]
\begin{prop}
For any $V\in\vect{+}{n}$, then $\quadratic_{\calL(n)}$ equals the composite $F_{\calL(n)} V\to F^{(2)}_{\calL(n)} V\subseteq S^2V$, and $\quadratic_{\calW(n)}$ equals the composite $F_{\calW(n)} V\to F_{\calL(n)} V\to F^{(2)}_{\calL(n)} V\subseteq S^2V$.
\end{prop}
\begin{proof}
Consider the case $\calW(n)$ for $n\geq1$. One observes that $\quadratic_\calC$ vanishes except on quadratic grading 2, and checks the result on terms $[x,y]$, $x\lambda_i$ (for $i<|x|$) and $\restn{x}$:
\begin{alignat*}{2}
\quadratic_{\calW(n)}([x,y])&=j_{\calW(n)}([\overline{x}+\overline{\overline{x}},\overline{y}+\overline{\overline{y}}]+[\overline{x},\overline{y}]+[\overline{\overline{x}},\overline{\overline{y}}])\\
&=j_{\calW(n)}([\overline{x},\overline{\overline{y}}]+[\overline{\overline{x}},\overline{y}])=x\otimes y+y\otimes x,
\end{alignat*}
which is precisely the representation of $[x,y]$ in $F^{(2)}_{\calC}V\subseteq V^{\otimes2}$. Similarly, $\quadratic_{\calW(n)}(x\lambda_i)$ vanishes (as $(\DASH)\lambda_i$ is linear), while $\quadratic_{\calW(n)}(\restn{x})$ equals $x\otimes x$ as desired. The other cases, including that of $\calW(0)$, are barely any different.
\end{proof}
...
\begin{Categories of graded Lie algebras and their homotopy operations}
\vfil\pagebreak
\section{Categories of graded Lie algebras and their homotopy operations}
...
\end{Categories of graded Lie algebras and their homotopy operations}
...
\[c=(1+s_0d_1)(1+s_1d_2)\cdots (1+s_{m-1}d_m):\mathbb{B}_m\to \mathbb{B}_m.\]
By the simplicial identities, $c$ has image inside $N_m^\textup{h}\mathbb{B}$, and by further inspection, $c$ is an idempotent.
In light of the fact that all the simplicial operators but $d_0$ preserve the subspaces $V_i$, $c$ restricts to an idempotent on $V_m$, and moreover $\im((\textup{id}-c)|_{V_m})\subset F_{m-1}V_m$, so we may define $V'_m=c(V_m)$.
...
Recall that the operations of \S\ref{section: vertical Koszul operations n positive} are the maps induced on cohomology by the \textbf{chain map?}
...
If we write $V=Q_{\calL(n)}G_\bullet L$ for the double complex yielding the spectral sequence, then there is a weak equivalence (in $s\calV$)
...
Anyway, let's start with an element of $V_{p+1,q+i-1}\cong (QB^{\calW(n+1)}_{p+1}\pi_*L)_{q+i-1}$. We may apply $c:V_{p+1}\to V_{p+1}$ to this element, in order to obtain an element of $V'_{p+1}$ which represents the homology class of interest (\textbf{explain}). We name the element in question:
\[e:=f^{p+1}_{p+1}(u_j)\in V'_{p+1}\textup{ for various }u_j\in V_{p}\]
and calculate:
\begin{alignat*}{2}
%e&:=f^{p+1}_{p+1}\cdots f^1_1x_0\in V'_{p+1}\\
\chi(e)&=z_{f_{p+1}^\textup{rep}\chi(u_j)}\\
\psi_{\BSW}(\chi(e))&=\quadratic(f_{p+1}^\textup{rep})(\chi(u_j))
\end{alignat*}
since $\chi(u_j)$ may be taken (by the construction above) to be contained in the span of the $z$ classes. Now $f_{p+1}^\textup{rep}$ is a $\calL(n)$-expression which implements the $\calW(n+1)$-expression $f_{p+1}$ at the chain level. Taking $\quadratic(f_{p+1}^\textup{rep})$, we extract the part of $f_{p+1}^\textup{rep}$ corresponding to the quadratic grading 2 part of $f_{p+1}$. That is, we may write
\[f_{p+1}=\textstyle\sum_j \lambda_{n_j}(x_j) + \textup{TOPOPS}(y(u_j))+w,\]
where $x_j\in \{u_j\}$, $y\in S^2_\textup{pr?}(\textup{these classes})$ with $n_j<|x_j|$, $e$ is a $\calL(n+1)$-expression of quadratic grading 2, and $w\in F_{\calW(n+1)}$ is the quadratic grading$\makebox[0cm][l]{}\neq2$ part of $f_{p+1}$. Then
\[\quadratic(f_{p+1}^\textup{rep})(\chi(u_k ))=\sum_j \quadratic(\lambda^\textup{rep}_{n_j})(\chi(x_j))+\quadratic (\textup{TOPOPS}^\textup{rep}(y(\chi(u_k))))\]
Now in this context, $\quadratic:F_{\calL(n)}V\to S^2V$ really just means the projection onto the quadratic grading 2 part of $F_{\calL(n)}V$, which is by definition contained in $(\LieOperad(2)\otimes V^{\otimes 2})^{\Sigma_2}$. Moreover, appealing to the fact [see Dwyer remark 4.4 taken with Goerss' comments \S3] that the operation $\lambda_{i-1}$ on $\pi_*(M)$ (for $M\in s\calL(n)$) is defined by the composite
\[\pi_*(M)_\textup{restrictable part of $M$}\overset{\sigma_{i-1}}{\to}\pi_{*+i-1}(S^2_\textup{pr}M)\to \pi_{*+i-1}(M),\]
the summands $\sum_j \quadratic(\lambda^\textup{rep}_{n_j})(\chi(x_j))$ precisely equal the sum of the maps $\sigma_{i-1}\circ Q\chi\circ(\gamma_i)_* $.
Similarly, appealing to the fact that the ``top operations'' $S^2_\textup{pr?}\pi_*M\to \pi_*M$ are
\[S^2_\textup{pr?}(\pi_*(M))\overset{\nabla}{\to}\pi_{*}(S^2_\textup{pr}M)\to \pi_{*}(M),\]
we find
\[\quadratic (\textup{TOPOPS}^\textup{rep}(y(\chi(u_k))))=\nabla(y)(\gamma(u_k))=\widetilde{\nabla} S^2(Q\chi)(jQ\xi)\]

%other than terms $\lambda_i(x)$ and simple brackets $[x,y]$ has no contribution to $\quadratic(f_{p+1}^\textup{rep})$ (as each generating $\calW(n+1)$-operation is built at the chain level from a quadratic operation using the partially restricted Lie algebra structure $\calL(n)$). More precisely, $f_{p+1}^\textup{rep}$ may be written \[f_{p+1}^\textup{rep}=\sum_i\sum\]



So we're there, mod notation!
\end{proof}
