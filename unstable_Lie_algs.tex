...
Corresponding to the above structure, $S^n_{\calC}$ contains an element $z$ in dimension $n$, $CS^n_{\calC}$ contains an element $h$ in dimension $n+1$, there is a map $\imath:S^n_{\calC}\to CS^n_{\calC}$ sending $z$ to $d_0h$, and $\imath^*$ represents the differential under the isomorphisms
...
\[\xymatrix@R=4mm{
\bigsqcup_{n\geq0}\bigsqcup_{N_{n+1}X}S_\calC^n
\ar[r]\ar[d]^-{\bigsqcup\bigsqcup\imath}
&%r1c1
\bigsqcup_{n\geq0}\bigsqcup_{ZN_{n}X}S_\calC^n\ar[d]\\%r1c2
\bigsqcup_{n\geq0}\bigsqcup_{N_{n+1}X}CS_\calC^n
\ar[r]&%r2c1
\BSW X%r2c2
}\]
where under the top horizontal map, the copy of $S^n_\calC$ corresponding to $x\in N_{n+1}X$ maps identically onto the copy of $S^n_\calC$ corresponding to $dx=\imath^*x$. It will be useful to write $h_x$ for the element of $\BSW X$ corresponding to $h$ in the copy of $CS^n_\calC$ corresponding to $x\in N_{n+1}X$, and similarly, $z_x$ for the element of $\BSW X$ corresponding to $z$ in the copy of $S^n_\calC$ corresponding to $x\in ZN_{n}X$. The comonad structure maps $\epsilon:\BSW X\to X$ and $\Delta:\BSW X\to \BSW^2X$ are then determined by the equations
\[\epsilon(h_x)=x,\ \epsilon(z_y)=y,\ \Delta(h_x)=h_{h_x},\textup{ and }\Delta(z_y)=z_{z_y}\textup{ for $x\in N_{n+1}X$ and $y\in ZN_nX$.}\]

Moreover, $\BSW X$ is homotopy equivalent to a coproduct of spheres. Indeed, for each $n$, let $B_nX=\im (d:N_{n+1}X\to ZN_nX)$, and choose a section $h_0$ of the surjection $d:N_{n+1}X\to B_nX$. Then $\BSW X$ contains the contractible subobject $C_0:=\bigsqcup_n\bigsqcup_{h\in \im (h_0)} CS^n_{h_0(f)}$, and $\BSW X/C_0$ is homotopic to a wedge of spheres:
%\[\BSW X/C_0\cong \left(\bigsqcup_{h\in N_{n+1}X\setminus\im (h_0)}CS^n_h/\partial(CS^n_h)\right) \sqcup\left(\bigsqcup_{f\in ZN_nX\setminus B_nX}S^n_f\right)\]
\[\BSW X/C_0\cong \left(\bigsqcup_{N_{n+1}X\setminus\im (h_0)}F_\calC (CS^n_{\vect{}{}}/\partial(CS^n_{\vect{}{}}))\right) \sqcup\left(\bigsqcup_{f\in ZN_nX\setminus B_nX}S^n_\calC\right)\]
We would like to find a \emph{subspace} of $\pi_*(\BSW X)$ which freely generates it as a $\calC\textup{-$\Pi$-algebra}$. Even better, we have the following rendition of [Stover? Blanc-Stover?]. We give the proof since we will need to be explicit about some parts of it in what follows.
\begin{prop}
For $X\in\calC$, $\pi_*(B_\bullet^{\BSW}X)$ is an almost free (monadic over $\vect{}{}$) simplicial $\calC\textup{-$\Pi$-algebra}$ weakly equivalent to $\pi_*(X)$.
\end{prop}
\begin{proof}
That the augmentation to $\pi_*(X)$ is a weak equivalence follows from Stover's result \cite[2.7]{StoverVanKampen.pdf}. The only change from Blanc-Stover is that $\pi_*(B_\bullet^{\BSW}X)$ is almost free over the category $\vect{}{}$, rather than the category of pointed sets.
For a set $S$, write $\F_2\langle S\rangle$ for the vector space generated by the symbols $\underline{s}$ for $s\in S$. There's a natural map $\F_2\langle d\rangle :\F_2\langle N_{n+1}X\rangle \to \F_2\langle ZN_nX\rangle $, and a natural monomorphism $\alpha:\ker(\F_2\langle d\rangle)\to \pi_*(\BSW X)$, defined by
\[\alpha(\underline{x_1}-\underline{x_0})=[h_{x_1}-h_{x_0}],\textup{ for $x_1,x_2\in N_{n+1}X$ with $dx_1=dx_2$}.\]
Moreover, there's a natural map $\beta:\F_2[ZN_nX]\to\pi_*(\BSW X)$ (which is not monomorphic) defined by
\[\beta(\underline{x})=[z_{x}],\textup{ for $x\in ZN_{n}X$}.\]
From the above expression for $\BSW X/C_0$, one sees that $\im (\alpha)$ and $\im (\beta)$ are linearly independent subspaces of $\pi_*(\BSW X)$, and $\pi_*(\BSW X)$ is free on $\im (\alpha)\oplus\im (\beta)$. From this description it is clear that the generating subspaces are preserved by maps in the image of $\BSW $. Thus, every face and degeneracy map except for $s_0$ and $d_0$ preserves the generators.

In order that $s_0$ preserves generators, we note that the diagonal of $\BSW$ sends the subspaces $\im (\alpha_X)$ and $\im (\beta_X)$ into the subspaces $\im (\alpha_{\BSW X})$ and $\im (\beta_{\BSW X})$. That $\im (\beta_X)$ maps into $\im (\beta_{\BSW X})$ is immediate. For $\im (\alpha_X)$, the image of $h_{x_1}-h_{x_0}$ under the diagonal is $h_{h_{x_1}}-h_{h_{x_0}}$, which is in $\im(\alpha_{\BSW X})$, since $dh_{x_1}=z_{dx_1}=z_{dx_0}=dh_{x_0}$.
\end{proof}
