...
\item Do I insert a section of conventions, including, cugas and model category structures on cugas, free functors using standard notation $F_?$ thought of as monads without further notation, indecomposable, categories of vector spaces, good and bad Lie algebras, higher Eilenberg-Zilber maps, never substituting out stars for gradings and having all gradings at the very right,tensor products of vector spaces,\ldots?
...
which represents the free restricted Lie algebra. He further constructs a norm map $\textup{Tr}:S(\LieOperad)\to \Gamma(\LieOperad)$, and defines a monad $\Lambda(\LieOperad)$ by the formula
...
The maps $\phi_1$ and $\phi_2$ can then be applied respectively to all of the sphere and cone classes appearing in $\BSW X$.
In fact, this diagram can even be formed in $s\vect{+}{n}$, before application of $F_{\calL(n)}:\vect{+}{n}\to\calL(n)$, and $\phi_1$ and $\phi_2$ are just the levelwise application of the diagonal map on a simplicial vector space. To understand the effect of $\phi_{\textup{ch}}$ on homotopy, it is enough to identify where the generators of $\pi(\BSW X)$ are sent in $\pi(\BSW X)\sqcup\pi(\BSW X)$, which is easy.
\begin{lem}
$\BSW X$ is naturally a (strict) commutative cogroup object, having comultiplication map $\phi_{\textup{ch}}$, counit map $0:\BSW X\to 0$, and inverse map $\textup{id}:\BSW X\to \BSW X$. In particular, $\hom(\BSW X,\DASH)$ takes values in $\ensuremath{\F_2}$-vector spaces.
\end{lem}
\begin{proof}
There are a few axioms to check, for example, left counitality: that the composite 
\[\xymatrix@R=4mm@1{
\BSW X\ar[r]^-{\phi_{\textup{ch}}}
&%r1c1
\BSW X\sqcup \BSW X\ar[r]^-{\textup{id}\sqcup0}
&%r1c2
\BSW X%r1c3
...
Using the notation $*$ for the group operation on $\hom_{s\calL(n)}(\BSW X,Y)$, we have the following:
\begin{lem}
For maps $f,g:\BSW X\to Y$ we have 
\[Q_{\calL(n)}(f*g)=(Q_{\calL(n)}f+Q_{\calL(n)}g):Q_{\calL(n)}\BSW X\to Q_{\calL(n)}Y.\]
\end{lem}
\begin{proof}
It's enough to check that $Q_{\calL(n)}(\phi_\textup{ch}):Q_{\calL(n)}\BSW X\to Q_{\calL(n)}(\BSW X\sqcup \BSW X)$ equals the diagonal map $Q_{\calL(n)}\BSW X\to Q_{\calL(n)}\BSW X\oplus Q_{\calL(n)}\BSW X$. For this, $Q_{\calL(n)}$ converts all the colimits involved in the construction of $\BSW X$ to colimits in $s\vect{+}{n}$, and $\phi_1$ and $\phi_2$ are both precisely the diagonal map.
\end{proof}
...
\[\makebox[0mm][r]{$\overline{\xi}_{\textup{ch}}:\ $}\BSW^2X\overset{\phi_{\textup{ch}}}{\to}(\BSW^2X)^{\sqcup2}\overset{a\sqcup b}{\to}(\BSW X)^{\sqcup2}\]
where $a,b:\BSW^2X\to(\BSW X)^{\sqcup2}$ are the composites
\[\xymatrix@R=2mm{
\makebox[0mm][r]{$a:\ $}\BSW^2X\ar[r]^-{\phi_{\textup{ch}}}
&%r1c1
(\BSW^2X)^{\sqcup2}\ar[r]^-{\epsilon^{\sqcup2}}
&%r1c2
(\BSW X)^{\sqcup2}\\
\makebox[0mm][r]{$b:\ $}\BSW^2X\ar[r]^-{\epsilon}
&%r1c1
(\BSW X)\ar[r]^-{\phi_{\textup{ch}}}
&%r1c2
(\BSW X)^{\sqcup2}
...
\begin{lem}
The composite 
$\BSW^2X\overset{\overline{\xi}_{\textup{ch}}}{\to}(\BSW X)^{\sqcup2}\to(\BSW X)^{\times2}$ is zero.
\end{lem}
\begin{proof}
This follows from the easy observation that both composites $(\textup{id}\sqcup0)\overline{\xi}_{\textup{ch}}$ and $(0\sqcup\textup{id})\overline{\xi}_{\textup{ch}}$ equal $\epsilon:\BSW^2X\to \BSW X$.
\end{proof}
In particular, $\overline{\xi}_{\textup{ch}}$ factors through the smash product, defining a natural map $\xi_{\textup{ch}}:\BSW^2X\to (\BSW X)^{\wedge 2}$.
...
\[N_*N_*(Q_{\calL(n)}B^\BSW_{\bullet}(Q_{\calU(n)}X^c))^t_{s_{n+2},\ldots,s_1}\to
  N_*N_*(Q_{\calL(n)}(B^\BSW_{\bullet}(Q_{\calU(n)}X^c))^{\wedge 2})^t_{s_{n+2}-1,s_{n+1},\ldots,s_1}.\]
\end{lem}
\begin{proof}
...
\[\psi_\textup{ch}:N_*N_*(Q_{\calL(n)}B^\BSW_{\bullet}(Q_{\calU(n)}X^c))^t_{s_{n+2},\ldots,s_1}\to
N_*N_*(S^2(Q_{\calL(n)}B^\BSW_{\bullet}(Q_{\calU(n)}X^c)))^{t-1}_{s_{n+2}-1,s_{n+1},\ldots,s_1}.
\]
