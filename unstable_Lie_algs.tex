...
\section{Cohomology operations for unstable Lie algebras}
...
\todo{Define functors $Q_P:\calW(0)\to\calL(0)$ and $Q_{\Lambda(n)}:\calW(n)\to \calL(n)$ for $n\geq1$}{Be careful to explain that when $n>0$ you divide by nontop operations only, and the opposite for $n=0$
...
\todo{Define the spectral sequence $H^*_{\calW(n)}\Longleftarrow H^*_{\calW(n+1)}$}
