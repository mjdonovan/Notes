...
\end{alignat*}
since, by construction, $\chi(u_j)$ is contained in the span of the $z$ classes.  Taking $\quadratic(f_{p+1}^\textup{rep})$, we extract the part of $f_{p+1}^\textup{rep}$ corresponding to the quadratic grading 2 part of $f_{p+1}$, in $F_{\calW(n+1)}^{(2)}$. That is, we may write
\[f_{p+1}=\textstyle\sum_j \lambda_{n_j}(x_j) + \textup{!}(y(u_j))+w,\textup{(! reminds us it's `TOPOPS')}\]
where $x_j\in \{u_j\}$ [so that the sum is in $F_{\calU(n+1)}^{(2)}(V_p)$], $y\in F_{\calL(n+1)}^{(2)}(V_p)$ with $n_j<|x_j|$, and $w\in F_{\calW(n+1)}(V_p)$ is the quadratic grading$\makebox[0cm][l]{}\neq2$ part of $f_{p+1}$. Then
\[\quadratic_{\calL(n)}(f_{p+1}^\textup{rep})(\chi(u_k ))=\sum_j \quadratic_{\calL(n)}(\lambda^\textup{rep}_{n_j})(\chi(x_j))+\quadratic_{\calL(n)} (\textup{!}(\widetilde{\nabla}(y)(\chi(u_k))))\]
Now in this context, $\quadratic_{\calL(n)}:F_{\calL(n)}V\to S^2V$ really just means the projection onto the quadratic grading 2 part of $F_{\calL(n)}V$, which is by definition contained in $(\LieOperad(2)\otimes V^{\otimes 2})^{\Sigma_2}$. Moreover, appealing to the fact [see Dwyer remark 4.4 taken with Goerss' comments \S3] that the operation $\lambda_{i-1}$ on $\pi_*(M)$ (for $M\in s\calL(n)$) is defined by the composite
