...
In particular, for $X_0$ and $X_1$ models, there is an isomorphism $i:(\pi X_0)\wedge (\pi X_1)\to\pi(X_0\wedge X_1)$ (natural for maps of pairs of models).
...
\subsection{Decomposition maps and cohomology operations}
...
Suppose that $\calC$ is an algebraic category as above. A \emph{decomposition map} for $\calC$ is simply a natural map
\[j_\calC:Q_\calC(X_0\wedge X_1)\to Q_\calC(X_0)\otimes Q_\calC(X_1),\]
such that $j_\calC\circ Q_\calC( \tau)=\tau\circ j_\calC$, where $\tau$ stands for either of the two symmetry isomorphisms. \textbf{In fact, we may choose only to require that a decomposition map is defined for pairs $X_0,X_1$ of \emph{free} objects in $\calC$, right?}
...
\section{Operations on Blanc-Stover's $\Pi$-algebra cohomology spectral sequence}
...
\subsection{The $\Pi$-algebra cohomology spectral sequence}
So you just sub it all in.

\subsection{Compatible decomposition maps for $\Pi$-algebras}
