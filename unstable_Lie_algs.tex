...
\subsection{Quadratic part, quadratic decomposition maps and cohomology operations}
...
\textbf{Go ahead and introduce them, dude.}
...
In what follows, we'll need to use Singer's techniques for deriving operations in spectral sequences. He defines operations in the cohomology spectral sequence of a bisimplicial cocommutative coalgebra. While the comultiplicative structure that we can define on the resolution $Q_{\calL(n)}G_\bullet L$ is not that of a coalgebra, we can still use his techniques after a single simple observation. That is, for every bisimplicial vector space $V$, if we write $E_r^{**}(V)$ for the cohomology spectral sequence obtained from the double cochain complex $(N_*N_*V)^*$, then Singer's methods yield external operations of the type:
\[E_r^{**}(V)\to E_r^{**}(S^2V)\textup{ and }E_r^{**}(V)\otimes E_r^{**}(V)\to E_r^{**}(S^2V).\]
When $V$ is a cocommutative coalgebra, the diagonal map $V\to S^2V$ induces a map $E_r^{**}(S^2V)\to E_r^{**}(V)$, postcomposition with which yields Singer's operations. We will briefly reprise Singer's results in this language, in order that we have them available in what follows.
...
\[\Sq^k_\textup{ext}:E^{p,q}_r(V)\to E^{p,q+k}_r(V)\]
\begin{alignat*}{2}
...
\end{alignat*}
The operations $\Sq^k_\textup{ext}$ are all defined at $E_2$, and determine the operations at each $E_r$, $r>2$. The operations $\Sq^k_\textup{ext}$ commute with differentials. Finally, the $\Sq^k_\textup{ext}$ converge to certain maps on $E_\infty$??? A line about products $\mu_\textup{ext}$???
...
The reader might now guess the next result:
\begin{thm}
At $E_2$, the operations $\Sq_\textup{h}^k$, $\Sq_\textup{v}^k$ and $\mu$ are equal to the $H^*_{\calW(n+1)}$-cohomology operations on $E_2=H^*_{\calW(n+1)}H_*^{\calU(n)}X$ of the same name, as defined in [\S?]. At $E_\infty$, the operations $\Sq^k$ are compatible with the operations $\Sq^k_\textup{h}$ defined on the target $H^*_{\calW(n)}(X)$, as defined in [\S?].
\end{thm} 
\begin{proof}
...
Similarly, our understanding of the construction of $\lambda$-operations and iterated brackets shows that $E'$ is annihilated by $\pi_*(j)\circ\gamma\circ Q(i)$. As $E'$ is also annihilated by $j$, and $j(\textup{br}(\overline{x}\otimes\overline{y})) =\overline{x}\otimes\overline{y}$, the diagram commutes. \textbf{This is poorly written.}
\end{proof}
