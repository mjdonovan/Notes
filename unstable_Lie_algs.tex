...
\excludeversion{first half}
\includeversion{second half}
...
\todo{Various intro here}{Say what a $\calC$-$\Pi$-algebra is, even with all the gradings. Construct the free $\calC$-$\Pi$-algebra on a set (\textbf{what happens on a vector space?}). Say what a model is. Move the discourse on spheres in $\textup{ch}_+\vect{}{}$ to somewhere above. Also, put some discourse on the smash product into this paragraph.}
...
...
which we can use to define operations using the EZAW maps that I'll introduce here, with the specialness property. (Goerss calls them `higher EZ maps'.)
\textbf{Go ahead and introduce them, dude.} Use the language $\Sq_\textup{ext}^k$ and $\mu_\textup{ext}$, and denote them, for $U\in s\vect{}{}$:
\[\Sq^k_\textup{ext}:H^m((CU)^*)\to H^{m+k}((CS^2U)^*)\]
and an exterior product
\[\mu_\textup{ext}:H^{m_1}((CU)^*)\otimes H^{m_2}((CU)^*)\to H^{m_1}((CS^2U)^*).\]
...
Fix $X\in\calW(n)$, and define $L=Q_{\calU(n)}B^\calW(n)X$. Let $G_\bullet L$ be the bisimplicial object obtained from the simplicial bar construction using the comonad $\BSW$. Then the spectral sequence we are interested in is the spectral sequence of the bisimplicial object
\[Q_{\calL(n)}G_\bullet L.\]
Now $L$ is an almost free object of $\calL(n)$, with $\pi_*L=H_*^{\calU(n)}X$. Thus $\pi_*(G_\bullet L)$ is an almost free resolution of $H_*^{\calU(n)}X$ in $s\calW(n+1)$. Moreover, $\pi_*(Q_{\calL(n)}G_\bullet L)=Q_{\calW(n+1)}\pi_*(G_\bullet L)$, so that
...
In what follows, we'll need to use Singer's techniques for deriving operations in spectral sequences. He defines operations in the cohomology spectral sequence of a bisimplicial cocommutative coalgebra. While the comultiplicative structure that we can define on the resolution $Q_{\calL(n)}G_\bullet L$ is not that of a coalgebra, we can still use his techniques after a single simple observation. That is, for every bisimplicial vector space $V$, if we write $E_r^{**}(V)$ for the cohomology spectral sequence obtained from the double cochain complex $(N_*N_*V)^*$, then Singer's methods yield external operations of type:
...
\begin{alignat*}{2}
\Sq^k_\textup{ext}:E^{p,q}_r(V)&\to E^{p,q+k}_r(S^2V)&\qquad&\text{if }0\leq k \leq q,\\
\Sq^k_\textup{ext}:E^{p,q}_r(V)&\to E^{p+k-q,2q}_{r+k-q}(S^2V)&\qquad&\text{if }q\leq k\leq q+r-2,\\
\Sq^k_\textup{ext}:E^{p,q}_r(V)&\to E^{p+k-q,2q}_{2r-2}(S^2V)&\qquad&\text{if }q+r-2\leq k.
\end{alignat*}
The operations $\Sq^k_\textup{ext}$ are all defined at $E_2$, and determine the operations at each $E_r$, $r>2$. The operations $\Sq^k_\textup{ext}$ commute with differentials. Finally, the $\Sq^k_\textup{ext}$ converge to well defined maps on $E_\infty$, and there is a commuting diagram
\[\xymatrix@R=4mm{
E^{p,q}_\infty(V)\ar[r]^-{\Sq_\textup{ext}^k}\ar[d]^-{\pi}
&%r1c1
E^{p,q+k}_\infty(S^2V)\ar[d]^-{\pi}
\\%r1c2
E_0^{p,q}H^*((CV)^*)\ar[r]^-{\Sq_\textup{ext}^k}&%r2c1
...
whenever $q\leq k$ (which summarises also Singer's computation of how the $\Sq^k_\textup{ext}$ interact with the filtation on cohomology).

Similar theory holds for an exterior product operation
\[\mu_\textup{ext}:E_r^{p_1,q_1}(V)\otimes E_r^{p_2,q_2}(V)\to E_r^{p_1+p_2,q_1+p_2}(S^2V)\]
...
The $\Sq^k_\textup{ext}$ and $\mu_\textup{ext}$ commute with $\lambda^*$, so that there are commuting diagrams
\[\xymatrix@R=4mm{
H^{m}((CU)^*)\ar[r]^-{\Sq^k_\textup{ext}}
\ar[d]^-{\lambda^*}
&%r1c1
H^{m+k}((CS^2U)^*)\ar[d]^-{(S^2\lambda)^*}
&%r1c3
H^{m_1}((CU)^*)\otimes H^{m_2}((CU)^*)\ar[r]^-{\mu_\textup{ext}}
\ar[d]^-{\lambda^*}
&%r1c1
H^{m_1+m_2}((CS^2U)^*)\ar[d]^-{(S^2\lambda)^*}
\\%r1c5
H^{m}((CV)^*)\ar[r]^-{\Sq^k_\textup{ext}}
&%r2c1
H^{m+k}((CS^2V)^*)
&%r2c3
H^{m_1}((CV)^*)\otimes H^{m_2}((CV)^*)\ar[r]^-{\mu_\textup{ext}}
&%r2c1
H^{m_1+m_2}((CS^2V)^*)
}\]
\end{thm}

Singer separates the operations $\Sq^k_\textup{ext}:E^{p,q}_2\to E_{2}$ according to the pattern of change in the spectral sequence coordinates of their output. Indeed, define
\begin{alignat*}{2}
\Sq^k_\textup{v,ext}=\Sq^k_\textup{ext}:E^{p,q}_2(V)&\to E^{p,q+k}_2(S^2V)&\qquad&\text{if }0\leq k \leq q,\\
\Sq^k_\textup{h,ext}=\Sq^{q+k}_\textup{ext}:E^{p,q}_2(V)&\to E^{p+k,2q}_2(S^2V)&\qquad&\text{if }0\leq k \leq p,
\end{alignat*}
...
\[E_2^{p,q}(S^2(Q_{\calL(n)}G_\bullet L))^{s_n,\ldots,s_1}_t\overset{(\psi_\BSW)^*}{\to} E_2^{p+1,q}(Q_{\calL(n)}G_\bullet L)^{s_n,\ldots,s_1}_{t+1}\]
We may then define internal spectral sequence operations
\begin{alignat*}{2}
\Sq^k=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{ext}:(E^{p,q}_r)^{s_n,\ldots,s_1}_t&\to (E^{p+1,q+k-1}_r)^{2s_n,\ldots,2s_1}_{2t+1}&\qquad&\text{if }0\leq k-1 \leq q,\\
\Sq^k=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{ext}:(E^{p,q}_r)^{s_n,\ldots,s_1}_t&\to (E^{p+k-q,2q}_{r+k-q-1})^{2s_n,\ldots,2s_1}_{2t+1}&\qquad&\text{if }q\leq k-1\leq q+r-2,\\
\Sq^k=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{ext}:(E^{p,q}_r)^{s_n,\ldots,s_1}_t&\to (E^{p+k-q,2q}_{2r-2})^{2s_n,\ldots,2s_1}_{2t+1}&\qquad&\text{if }q+r-2\leq k-1.
\end{alignat*}
Which, at $E_2$, we may write (dropping internal degrees) as \textbf{(precisely?:)}
\begin{alignat*}{2}
\Sq^k_\textup{v}=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{v,ext}=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{ext}:E^{p,q}_2&\to E^{p+1,q+k-1}_2&\qquad&\text{if }0\leq k-1 \leq q,\\
\Sq^k_\textup{h}=\psi_{\BSW}^*\circ\Sq^{k-1}_\textup{h,ext}=\psi_{\BSW}^*\circ\Sq^{q+k-1}_\textup{ext}:E^{p,q}_2&\to E^{p+k,2q}_2&\qquad&\text{if }0\leq k-1 \leq p,
\end{alignat*}

And similarly, a pairing
\[\mu=\psi_{\BSW}^*\circ\mu_\textup{ext}:(E^{p,q}_r)^{s_n,\ldots,s_1}_t\otimes (E^{p',q'}_r)^{s'_n,\ldots,s'_1}_{t'}\to (E^{p+p'+1,q+q'}_r)^{s_n+s'_n,\ldots,s_1+s'_1}_{t+t'+1}\]
The reader might now guess the next results:
\begin{thm}\label{E2CompFuncOperationsID}
At $E_2$, the operations $\Sq_\textup{h}^k$, $\Sq_\textup{v}^k$ and $\mu$ are equal to the $H^*_{\calW(n+1)}$-cohomology operations on $E_2=H^*_{\calW(n+1)}H_*^{\calU(n)}X$ of the same name, as defined in [\S?].
\end{thm}
...
\end{thm} 
\begin{proof}[Proof of \ref{E2CompFuncOperationsID}]
We will prove this theorem in various parts. Firstly, we'll work on the operations $\Sq_\textup{h}^k$ and $\mu$ at $E_2$.
For this we will rely on a commuting diagram, where we write $L=Q_{\calU(n)}X^c\in s\calL(n)$ and $Y=G_\bullet L\in ss\calL(n)$:
...
\textbf{Still need to do Koszul operations.}
\end{proof}
...
\[\xymatrix@R=4mm{
T_{m}(Q_{\calL(n)}\scrW L)
\ar[r]^-{\psi_{\BSW}}
\ar[d]^-{\epsilon}
&%r1c1
T_{m-1}(S^2(Q_{\calL(n)}\scrW L))\ar[d]^-{\epsilon}
\\%r1c3
N_{m}(Q_{\calL(n)}L)
\ar[r]^-{\psi_{\calW(n)}}
&%r1c1
...
\[N_0^\textup{h}N_*^\textup{v}(Q\scrW L)=N_*Q_{\calL(n)}\BSW Q_{\calU(n)}B_\bullet^{\calW(n)}X.\]
...
As it will not matter in this proof whether we look at $z$ or $h$ classes, we will write $K=k_{f^0(g^1_{i_1}(h^2_{i_1i_2}))}$ for either of these two.
We then define $\Phi$ by the formula
\[K\mapsto q(f)(g^1_{i_1}(h^2_{i_1i_2}))\in S^2Q_{\calW(n)}B^{\calW(n)}X.\]
This definition makes sense (and yields a nontrivial map) because $f^0$ is an operator in $Q_{\calU(n)}F_{\calW(n)}$.
The chain map $d\Phi+\Phi d$ is a sum of three terms:
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item[(a)] $d\circ\Phi:N_0^\textup{fil}N_*^\textup{int}(Q\scrW X)\overset{\Phi}{\to} N_*(S^2(QX))\overset{d}{\to} N_{*-1}(S^2(QX))$
\item[(b)] $\Phi\circ d^\textup{int}:N_0^\textup{fil}N_{*}^\textup{int}(Q\scrW X)\overset{d^\textup{int}}{\to} N_0^\textup{fil}N_{*-1}^\textup{int}(Q\scrW X)\overset{\Phi}{\to} N_{*-1}(S^2(QX))$
\item[(c)] $\Phi\circ d^\textup{fil}:N_1^\textup{fil}N_{*-1}^\textup{int}(Q\scrW X)\overset{d^\textup{fil}}{\to} N_0^\textup{fil}N_{*-1}^\textup{int}(Q\scrW X)\overset{\Phi}{\to} N_{*-1}(S^2(QX))$
\end{enumerate}
Let's start by identifying (a) and (b) applied to $K$:
\begin{alignat*}{2}
(d\circ\Phi)(K)&=d( q(f)(g^1_{i_1}(h^2_{i_1i_2})))\\
&=q(f)(g^0_{i_1}(h^1_{i_1i_2}))\\
&=q(f)(u(g^0_{i_1})(h^1_{i_1i_2}))\quad\textup{(as we calculate in $S^2(QX)$)}\\
...
It remains to show that $\Phi\circ d^\textup{fil}$ coincides with $\epsilon^{\otimes 2}\circ\psi_{\calW(n)}$. These two maps are only nonzero on the graded part $N_1^\textup{fil}N_{*-1}^\textup{int}(Q\scrW X)$ of the total complex, and an element therein is a linear combination
...
These coincidences also occur if we take quadratic parts of the $e_j(f_{ji_0})$, so that there holds the following equation in $N_{*-1}(S^2(QX))$:
...
=
\sum q(ue_j(f_{ji_0}))\left(s_{\alpha_{ji_0}} {g^1_{ji_0i_1}h^2_{ji_0i_1i_2}}\right).
\end{gather*}
The proof is completed upon noting that the LHS equals $(\epsilon^{\otimes 2}\circ \psi_{\BSW})(K')$, and that the RHS equals $(\Phi\circ d^\textup{fil})(K')$. For the LHS:
\begin{alignat*}{2}
...
\\
&=
\sum q(e_j(uf_{ji_0}))\left(s_{\alpha_{ji_0}} g^1_{ji_0i_1}h^2_{ji_0i_1i_2}\right)=\textup{LHS}
\end{alignat*}
\textbf{(clarify $\psi_{\BSW}$?)} where we may simplify $f_{ji_0}$ to $uf_{ji_0}$ since we take values in $S^2(QX)$. Next,
\begin{alignat*}{2}
d^\textup{fil}(K')
&=
...
&=
\sum u(e_j)\left( k_{f^0_{ji_0}g^1_{ji_0i_1}h^2_{ji_0i_1i_2}}\right)%
&\qquad&\text{(relevant $s_{\alpha_{ji_0}}$ are $\textup{id}$)}\\
\Phi(d^\textup{fil}(K'))&=
\sum u(e_j)\left( q(f_{ji_0})(g^1_{ji_0i_1}h^2_{ji_0i_1i_2})\right)%
\\
&=
\sum u(e_j)\left( q(f_{ji_0})(s_{\alpha_{ji_0}}g^1_{ji_0i_1}h^2_{ji_0i_1i_2})\right)%
&\qquad&\text{(relevant $s_{\alpha_{ji_0}}$ are $\textup{id}$)}\\
&=
\sum q(ue_j(f_{ji_0}))\left(s_{\alpha_{ji_0}} {g^1_{ji_0i_1}h^2_{ji_0i_1i_2}}\right) =\textup{RHS}.
\end{alignat*}
...
\item Uniformize notation for the total complex of a double complex.
