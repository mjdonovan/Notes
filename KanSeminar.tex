...
\includeversion{JeremyThomPracticeTalk}
\includeversion{ThomGlobalPropertiesPartTwo}
...
Let's just start trying to understand the cohomology thereof and the Steenrod operations thereupon. We know: 
...
Now the cohomology of $MBO(r)$ is universally zero in mod $p$ coefficients. He used pontrjagin classes and Steenrod powers. Thus we have an iso with mod $p$ coefficients (of zero objects). Thus $\phi$ is a homology isomorphism in this range. As everything is simply connects, it's a $\pi_*$-equivalence in this range!
...
\begin{itemize}\squishlist
\item There's a bijection between $L$-equiv classes on $V$ and maps $V\to MSO$.
\item $L$-equivalence classes on a large enough sphere are the same as cobordism classes.
\item Fantastic! But wait, the homotopy groups of $MSO(k)$ could be really hard to compute!
\item But wait! We have some guess (somehow) as to the homotopy type of $MSO(k)$ in a range.
\end{itemize}
\pagebreak
