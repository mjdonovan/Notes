...
\includeversion{SignatureThmTalk}
...
We could say $p(\xi):=c_{-ev}(\xi\otimes\C)$.
\item Ignore the even Chern classes --- they have order 2, as a complexification is isomorphic to its conjugate.
\item The total Pontrjagin class inherits a Cartan formula which holds modulo elements of order 2.
\item If $M^{4k}$ is an oriented $4k$-manifold, for each partition $I=(i_1,\ldots,i_r)$ of $k$, we have:
\[p_I[M]=p_{i_1}\cdots p_{i_r}[M]=\langle p_{i_1}(\tau_M)\cdots p_{i_r}(\tau_M),[M]\rangle\text{\ \ $\leftarrow$ Pontrjagin number}\]
\item \textbf{Pontrjagin numbers of $\CP^{2n}$:} Write $H^*(\CP^{2n})=\Z[a]/a^{2n+1}$. We know that:
$c(\tau)=(1+a)^{2n+1}$ and $c(\overline\tau)=(1-a)^{2n+1}$, so that $c(\tau\oplus\overline\tau)=(1-a^2)^{2n+1}$, and:
...
\[p(\tau):=c_{-ev}(\tau\otimes\C)=c_{-ev}(\tau\oplus\overline\tau)=(1+a^2)^{2n+1}.\]
Unraveling this, we obtain $p_I[\CP^{2n}]={2n+1\choose i_1}\cdots{2n+1\choose i_r}.$
