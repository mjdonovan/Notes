...
\begin{enumerate}
\item We know that for $k>n+1$, $\pi_{n+k}(T(\widetilde\gamma^k))\cong\Omega_n$.
\item The surjection $\pi_{n+k}(T(\widetilde\gamma_p^k))\to\Omega_n$ for $p,k\geq n$. Want to write as $\pi_{n+k}(MSO(k))$ for $k\geq n$.
\item Now as Haynes explained: For a finite $(k-1)$-connected complex, the homotopy groups stay finite until $\pi_{2k-2}$. Apply Hurewicz mod $\calC$ (the finite abelian groups) to see that the hurewicz map is a $\calC$-iso to this dimension inclusive. This is currently wrong.
\end{enumerate}
\end{enumerate}
