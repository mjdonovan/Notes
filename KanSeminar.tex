...
\excludeversion{Kervaire}
\excludeversion{SignatureThmTalk}
\includeversion{JandrPracticeKervaire}
...
\item The signature is a \emph{genus} --- a ring homomorphism from $\Omega_*\to\Q$. This can be proven directly, but we won't need to by the end of this talk.
...
0&&0\\
&\pi_i(X)\oplus\pi_i(Y)\ar[rr]^{\calC\oplus\calC}&&H_i(X)\oplus H_i(Y)
}\]
...
%\item Bound the rank of $\Omega_n$ above, by showing that $\pi_{n+k}(T(\widetilde\gamma_p^k))\to\Omega_n$ for $p,k>n$ is a surjection, and comparing the rank of $\pi_{n+k}(T(\widetilde\gamma_p^k))$ to that of $H_{n+k}(T(\widetilde\gamma_p^k))$. As this Thom space is $(k-1)$-connected, the Hurewicz is a $\calC$-iso up to dimension $2k-2$, which shows that the ranks agree where we want them to. Now $H^{n+k}(T(\widetilde\gamma^k);\Z[1/2])$ is calculated by using Pontrjagin classes, and also equals $H^{n}(\widetilde G_k;\Z[1/2])$. This equals $H^{n}(\widetilde G_k(\R^{p+k});\Z[1/2])$ for $p,k> n$ (looking at a CW-decomposition at top of page 81).
%
%\textbf{Summary:} Choose $k,p>n$. (Maybe $k,p\geq n+2$ so that this $\calC$-iso works).
%\[\Omega_n\leftfibration\pi_{n+k}(T(\widetilde\gamma^k_p))\overset{\calC\sim}{\to}H_{n+k}(T(\widetilde\gamma^k_p))=H_{n}(\widetilde G_k(\R^{p+k}))\]
%Now have, with $G'=\widetilde G_k(\R^{p+k})$:
%\[0\to\Ext(H_{n-1}(G'),\Z[1/2])\to H^{n}(G';\Z[1/2])\to \Hom(H_n(G'),\Z[1/2])\to0.\]
%Now the Ext group is finite\footnote{In fact, $\Ext(\Z/(2^r(2s+1))\Z,\Z[1/2])\cong \Z/(2s+1)\Z$, and of course $\Ext(\Z,\Z[1/2])=0$.} as $H_{n-1}(G')$ is finitely generated, and so the $\Z[1/2]$-ranks of the latter groups are equal. The $\Z[1/2]$-rank of the Hom group is just the rank of $H_n(G')$ ($\Z[1/2]$ being torsion free), showing that the rank of $H_n(G')$ is at most the $\Z[1/2]$-rank of $H^n(G';\Z[1/2])$. Now it's shown that:
%\[H^*(\widetilde G_{k};\Z[1/2])=\Z[1/2][p_1,p_2,\ldots,p_{\lfloor k/2\rfloor},e]/
%\begin{cases}
%e=0;&\ \text{$k$ odd};\\
%e^2=p_{k/2};&\ \text{$k$ even},
%\end{cases}\]
%where $p_i$ has dimension $4i$, and $e$ has dimension $k$. Now this in fact equals 
%$H^*(G';\Z[1/2])$ in dimensions less than both $k$ and $p$ (looking at a CW-decomposition at top of page 81). Thus, this group has rank $p(n/4)$ in dimension $n$. So, $\Omega_n$ is finitely generated, and has rank $p(n/4)$, for all $n$.


\end{enumerate}
%Assume:
%\begin{itemize}
%\item The cohomology ring $H^*(\widetilde G_{n};\Lambda)$, when $\Lambda$ is a domain containing $1/2$:
%\[H^*(\widetilde G_{n};\Lambda)=\Lambda[p_1,p_2,\ldots,p_{\lfloor n/2\rfloor},e]/
%\begin{cases}
%e=0;&\ \text{$n$ odd};\\
%e^2=p_{n/2};&\ \text{$n$ even},
%\end{cases}\]
%where $p_i$ has dimension $4i$, and $e$ has dimension $n$.
%\end{itemize}
\end{SignatureThmTalk}

\begin{JandrPracticeKervaire}
\KanSemResponse
{Jandr's Practice Talk on Thom's paper}
The plan:
\begin{enumerate}
...
\end{question}
The normal Kervaire invariant is defined for $(4k-2)$-dim framed manifolds. Define $q:H^{2k+1}(M;\Z)\to\Z_2$ a quadratic refinement ($q(x+y)=q(x)+q(y)+(x\smile y)[M]$) of cup product. Define $\Phi=\text{Arf}(q)$.

%To define $w$, take $M^n\subset \R^{n+N}$ giving $S^{n+N}\to\text{Th](\nu))=\Sigma^NM_+$
%\[q:(M_+\to K)\to(S^{n+M}\to\Sigma^NM_+\to\Sigma^NK)\]
%this gives an element of $\pi_n^s(K)=\Z_2$. ($K=K(\Z_2,2k+1)$)

\subsection*{Defining $\Phi$}
Define $\phi_0:H^*(M)\to\Z_2$ by letting $e_1,e_2$ be the generators of $H^5(\Omega S^6),H^{10}(\Omega S^6)$, and $u_2$ be the generator $H^{10}(\Omega S^6;\Z_2)$.
\begin{enumerate}
\item Find $f_X:M\to\Omega$ such that $f_X^*(e_1)=X$.
\item $\phi_0(M):=f^*_X(u_2)[M]$.
\end{enumerate}
Can show that $\phi_0$ descends to
\[\phi:H^5(M;\Z_2)\to\Z_2.\]
Let $\Phi$ be $\text{Arf}(\phi)$.
\subsection*{Constructing $M_0$}
Start with $S^5$, and let $\tau$ be its tangent bundle.  Pullback:
\[\xymatrix{
...
Think about Jandr's fat fingers. Need to do some smoothing, unless you're Milnor.

By Morse theory, $\partial W=S^9$ (writes down a morse function with only two critical points). Let $M_0$ be the cofibre of $S^9\to W$. Our proof will then also show that $\partial W$ must be exotic.

\subsection*{Showing that $\Phi(M_0)=1$:}
Now to $\Phi(M_0)=1$. The 0-sections of the two disk bundles meet transversely at a point. We have $S^5\vee S^5\to W$, which is a homotopy equivalence. So adding a 10-cell, $M_0$ has a CW-structure with one 0-cell, two 5-cells and one 10-cell (which shows that it's 4-connected).
...
Let's take $f_X:M_0\to K$ which collapses everything outside $D(\tau)_X$ to the basepoint and takes $D(\tau)_X\to K=D(\tau)/S(\tau)$. This is like a tubular neighbourhood thingo, so that $f_X^*(t)=X$, where $t$ is the Thom class. By some local degree argument, $f_X^*$ is an iso on $H^{10}$.

If $K$ were equal to $\Omega S^6$, we'd be done. But it's not! Now $K$ has the form $S^5\cup_f e^{10}$. Here, $f=[\imath_5,\imath_5]$, the Whitehead square. Now $\Omega S^6$ has a cell structure (Jandr likes it via Morse theory) with cells of dimension $5k$ for $k\in\N$, so that the 10-skeleton is $S^5\cup e^{10}$ too, and it turns out that you also get the Whitehead square. Thus $K\subset \Omega S^6$ is the 14-skeleton, and you're a winner. We're using $\Omega S^6$, cause it's an H-space, which'll be good.
\subsection*{Is $\Phi$ well defined?}
We've gotta show that $f_X$ does exist. We do obstruction theory:

If $(X,A)$ is a relative CW-complex, for each $n+1$-cell $\sigma$, let $g_\sigma:S^n\to X$ be the attaching map. If I'm given $f:X^n\to Y$, I'd like to extend this to the $n+1$-skeleton. To make this make sense, I need $\pi_1(Y)$'s action on $\pi_n(Y)$ to be trivial, so I can forget about basepoints. Let's also have $Y$ connected.

All I really want is for the compositions of $f$ with the $g_\sigma$ to be null. I can define:
\[c(f)\in C^{n+1}_{cell}(X,A;\pi_n(Y))\text{\ given by }c(f)(\sigma)=[f\circ g_\sigma]\]
...
\begin{thm*}
$c(f)=0\iff f$ can be extended to $X^{n+1}$.
$c(f)$ is a cocycle, and $[c(f)]=0$ \Iff $f|_{X^{n-1}}$ can be extended.
\end{thm*}
\noindent There's a bit left to do for well-defined-ness. We'll talk about why it descends to mod 2 cohomology. How come?
\[\phi_0(X+Y)=\phi_0(X)+\phi_0(Y)+x\cdot y\]
proven by considering a composite
\[\xymatrix{
M\ar[r]^M&M\times M\ar[r]^{f_X\times f_Y}&\Omega\times\Omega\ar[r]&\Omega
}\]
\subsection*{$M$ smooth implies $\Phi(M)=0$}
Just use Milnor's paper repeatedly. Milnor has a tool for killing homotopy groups. Given an $N$-manifold $M^N$ and $[f]\in\pi_n(M)$. Have $S^n\to M$, change it to get an embedding $S^n\times D^{N-n}\cofibration M$. Then apply surgery, to get
\[(M-(S^n\times D^{N-n}))\cup (D^{n+1}\times S^{N-n-1})\]
Two problems, you create a new hole, so can only do half the dimensions, i.e.\ you want $n<N/2$ or so. Also, you need a $\pi$-manifold, in order to choose the embedding (still only get certain dimensions). The killing procedure preserves $\pi$-manifold-ness. Milnor does all this very well.
\begin{defn*}
A $\pi$-manifold is one whose tangent bundle $\tau$ has $\tau\oplus\epsilon$ trivial.
\end{defn*}
This obviously implies that $\tau$ is stably trivial. It also shows that $\nu_M$ is stably trivial. In fact all these three are equivalent.
\begin{proof}
The $\Gamma$ shapes are all fibrations. The $M\to BO(n)$ classifies the tangent bundle. 
\[\xymatrix{
...
Write me.
\end{proof}
Another thing: if $V$ is a manifold with boundary which is connected, then it's homotopic to a CW-complex of lower dimension. So, get triviality on the nose.
\begin{enumerate}
\item A smooth 4-connected 10-manifold is $\pi$ (obstruction theory). This is the only place we use smoothness, to apply Milnor's procedure.
...
By applying Milnor's killing process, can assume $V$ is 4-connected. Then Poincar\'e duality shows that $V$ only gets homology in dimensions 0, 5, 6, 11. We have the following beautiful diagram from Poincar\'e duality, with $\Z_2$ coefficients:
\pagebreak
\[\xymatrix{
H^5(V)\ar[d]^\sim\ar[r]^{i^*}& H^5(M)\ar[d]^\sim\ar[r]^\delta&H^6(V,M)\ar[d]^\sim\\
H_6(V,M)\ar[r]^\partial&H_6(M)\ar[r]^{i_*}&H_5(V)
}\]
