...
\color{red} Cool --- I'd like to glue this in appropriately, but would like to
know the exact statement of Portryagin's result first. We should discuss this further some time.
}
...
\color{red} I have the following thought: it's not obvious right now that
...
Definition: A space $X$ is said to be a real space if it is equipped with an an involution $\tau$, i.e. if it is a $\mathbb{Z}_2$-space. We write $\overline{x}$ for $\tau(x)$.\\
\\
Definition: A real vector bundle over a real space $X$ is a complex vector bundle $p:E\to X$ satisfying the following conditions: $E$ is a real space; $p$ is $\mathbb{Z}_2$-equivariant; we obtain anti-linear isomorphisms $E_x \to E_{\overline{x}}$ by restricting the involution on $E$.\\
\\
Definition: A morphism between real vector bundles over a real space $X$ is a homomorphism of complex vector bundles commuting with the involutions.\\
\\
Definition: The Grothendieck group of the category of real vector bundles over a real space X is denoted by $\KR(X)$.\\
\\
Note that if $X$ is a real space with trivial involution and $E$ is a real vector bundle over $X$, then the involution on $E$ restricts to an involution on each fiber $E_{x}=E_{\overline{x}}$. This involution is linear over $\mathbb{R}$ and so each fiber decomposes as a direct sum of the $+1$-eigenspace and the $-1$-eignespace. A real vector bundle over $X$ (viewed as a space) gives a real vector bundle over $X$ (viewed as a real space) by complexification - the involution is complex conjugation. A real vector bundle over $X$ (viewed as a real space) gives a real vector bundle over $X$ (viewed as a space) by taking the $+1$-eigenspace. These constructions are inverse to one another, and are natural, so that $\KR(X)=\KO(X)$ whenever $X$ is a real space with trivial involution.\\
\\
...
\\
Alternatively, we can procede as follows.\\
\\
Note that $U$ has an involution given by complex conjugation and its fixed points are $O$. Thus $BU$ becomes a real space with fixed points $BO$; equip $\mathbb{Z}$ with the trivial involution. This allows us to make the following definition.\\
\\
Definition: Let $X$ be a based real space (the base point is fixed by the involution). We define $\tilde{\KR}(X)$ to be the group $[X,\mathbb{Z}\times BU]_{\mathbb{Z}_2}$ of $\mathbb{Z}_2$-homotopy classes of $\mathbb{Z}_2$-maps.\\
\\
Suppose $X$ is a based real space with trivial involution. Then \[\tilde{\KR}(X)=[X,\mathbb{Z}\times BU]_{\mathbb{Z}_2}=[X,\mathbb{Z}\times BU^{\mathbb{Z}_2}]=[X,\mathbb{Z}\times BO]=\tilde{\KO}(X)\]
Suppose $X$ is a based space. We can equip $X\vee X$ with the involution which swaps the factors. Then \[\tilde{\KR}(X\vee X)=[X\vee X,\mathbb{Z}\times BU]_{\mathbb{Z}_2}=[X,\mathbb{Z}\times BU]=\tilde{\K}(X).\]
\\
Let $S^{2,1}$ denote the one point compactification of $\mathbb{R}^2$ equipped with the involution $(x,y)\mapsto (x,-y)$.\\
\\
Theorem: Let $X$ be a real space. Then we have a natural isomorphism
\[\tilde{\KR}(S^{2,1}\wedge X)\cong\tilde{\KR}(X).\] Equivalently, we have an equivariant homotopy equivalence.
\[\Omega^{2,1}(\mathbb{Z}\times BU)\simeq\mathbb{Z}\times BU.\]
This allows us to define a $\mathbb{Z}_2$-equivariant spectrum $\KR$ with $\KR^{\mathbb{Z}_2}=\KO$. Lovely stuff.

\section{Equivariant Homotopy Theory}
\subsection{Some Useful Adjunctions}
Let $G$ be a topological group.\\
\\
Let $\mathscr{U}$ denote the category of spaces (compactly generated, weakly Hausdorff) with continuous maps and let $G\mathscr{U}$ denote the category of left $G$-spaces with $G$-equivariant continuous maps.\\
\\
Let $X$ and $Y$ be $G$-spaces. Then $\mathscr{U}(X,Y)$ is a $G$-space: for $f\in\mathscr{U}(X,Y)$, $(g\cdot f)(x)=g(f(g^{-1}x))$; we see $\mathscr{U}(X,Y)^G=G\mathscr{U}(X,Y)$.\\
\\
We have a functor $1_G:\mathscr{U}\to G\mathscr{U}$ which equips a space $X$ with a trivial $G$-action and acts as the identity on morphisms.\\
\\
We also have functors
\[(-)^G:G\mathscr{U}\to\mathscr{U}, \hspace{3pt} X\mapsto X^G; \hspace{10pt}
(-)/G:G\mathscr{U}\to\mathscr{U}, \hspace{3pt} X\mapsto X/G\]
\\
Theorem: $1_G$ is left adjoint to $(-)^G$ and right adjoint to $(-)/G$.\\
\\
Proof: Let $K$ be a space, which we endow with a trivial $G$-action and $X$ be a $G$-space. Then we have natural homeomorphisms
\[G\mathscr{U}(K,X)\cong\mathscr{U}(K,X^G);\hspace{10pt}
\mathscr{U}(X/G,K)\cong G\mathscr{U}(X,K).\]
...
\[G\times_H(-):H\mathscr{U}\to G\mathscr{U}; \hspace{10pt}
\textup{Map}_H(G,-):H\mathscr{U}\to G\mathscr{U}\]
Theorem: $U$ is right adjoint to $G\times_H(-)$ and left adjoint to $\textup{Map}_H(G,-)$.\\
\\
Proof: Let $X$ be a $G$-space and $Y$ be an $H$-space. We which to define natural isomorphisms
\[G\mathscr{U}(G\times_H Y,X)\cong H\mathscr{U}(Y,X);\hspace{10pt}
H\mathscr{U}(X,Y)\cong G\mathscr{U}(X,\textup{Map}_H(G,Y)).\]
Given a $G$-map $f:G\times_H Y\to X$, we can define an $H$-map: $Y\to X$, $y\to f[e,y]$. Given an $H$-map $f:Y\to X$ we can define a $G$-map: $G\times_H Y\to X$, $[g,y]\to gf(y)$. We note that the unit of the adjunction is $Y\to G\times_H Y$, $y\to [e,y]$ and the counit is $G\times_H X\to X$, $[g,x]\to x$. Defining the adjunction interms of these maps makes it clear that the adjunction is actually a homeomorphism of topological spaces.\\
\\
The unit and counit of the other adjunction are given by $X\to\textup{Map}_H(G,X)$, $x\mapsto (g\mapsto g^{-1}x)$ and $\textup{Map}_H(G,Y)\to Y$, $f\to f(e)$, respectively. Again, one sees that we have a homemorphism.\\
\\
Theorem: If $X$ is a $G$-space then we have G-homeomorphisms
\[G\times_H X\cong (G/H)\times X;\hspace{7pt}
\textup{Map}_H(G,X)\cong\mathscr{U}(G/H,X).\]
Proof: Define
\[G\times_H X\to (G/H)\times X,\hspace{3pt}[g,x]\to ([g],gx)\]
and
\[\textup{Map}_H(G,X)\to\mathscr{U}(G/H,X),\hspace{3pt}f\mapsto([g]\mapsto g(f(g))).\]
\\
Theorem: $G/H\times(-):\mathscr{U}\to G\mathscr{U}$ is left adjoint to $(-)^H:G\mathscr{U}\to\mathscr{U}$.\\
\\
Proof: Let $K$ be a space which we endow with a trivial $G$-action and $X$ be a $G$-space. Then
...
\\
Theorem: $(-)/H:G\mathscr{U}\to\mathscr{U}$ is left adjoint to $\mathscr{U}(G/H,-):\mathscr{U}\to G\mathscr{U}$.\\
\\
Proof: Let $K$ be a space which we endow with a trivial $G$-action and $X$ be a $G$-space. Then
\[\mathscr{U}(X/H,K)\cong H\mathscr{U}(X,K)\cong G\mathscr{U}(X,\textup{Map}_H(G,K))\cong G\mathscr{U}(X,\mathscr{U}(G/H,K)).\]
\\
...
\\
All of the above continues to hold in the based setting. We replace $\mathscr{U}$, $G\times_H Y$, $(G/H)\times X$, $\textup{Map}_H(G,Y)$ and $\mathscr{U}(G/H,X)$ with $\mathscr{T}$, $G_+\wedge_H Y$, $(G/H)_+\wedge X$, $\textup{Map}_H(G_+,Y)_*$ and $\mathscr{T}(G/H_+,X)$, respectively.\\
\\
