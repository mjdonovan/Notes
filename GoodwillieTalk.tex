...
\includeversion{II.1.20}
\includeversion{II.1.22}
\includeversion{III.1.4 and III.1.5}
...
\frame{\frametitle{An iterated Lipschitz condition}
...
\frame{\frametitle{Distances in $\calc$ and $\cald$}
...
$F:\calc\to\cald$ has \underline{$E_n(c,\kappa)$} if\ \ \ \pause $\xcancel{\textup{($F$ is $C^\infty$, and)}}$
\begin{center}
\pause
...
{\color{blue}$F$ is ``stably $n$-excisive''!}
\pause

\phantom{mover}

$\left(\raisebox{2mm}{\txt{$\cartesianness\left(F\scrx\right)\geq -c+\displaystyle\sum_{i=1}^{n+1}\conn\left(\scrx_\emptyset \rightarrow \scrx_i\right)$\\$\textup{whenever}\ {\scrX_\emptyset\rightarrow \scrX_{i}}\textup{ is $\kappa$-connected for all $i$}.$}}\right)$
\end{center}
}

\frame{\frametitle{$\rho$-analyticity}
$F$ is \underline{$\rho$-analytic} if: 
\[\textup{$F$ has $E_n(n\rho-q,\rho+1)$ for all $n$.}\]
%\begin{thm*}
%If $F$ is
...
\begin{center}
$\txt{$\dist(F(X)\overset{u_X}{\to} G(X))\leq e^c\dist(X\downarrow Y)^{n+1}$\\$\textup{whenever}\ \dist(X\downarrow Y)\leq e^{-\kappa}$}$

\pause
\phantom{mover}

{\color{blue}$F$ and $G$ ``agree to  $n^\textup{th}$ order at $Y$'' via $u$!}
\pause

\phantom{mover}

$\left(\raisebox{1.2mm}{\txt{$\conn(F(X)\overset{u_X}{\to} G(X))\geq -c+(n+1)\conn(X\downarrow Y)$\\$\textup{whenever}\ \conn(X\downarrow Y)\geq\kappa.$}}\right)$
\end{center}
}
...
\begin{II.1.6}
\frame{\frametitle{II.1.6}

}
\end{II.1.6}

\begin{II.1.20}
\frame{\frametitle{II.1.20}

}
\end{II.1.20}

\begin{II.1.22}
\frame{\frametitle{II.1.22}

}
\end{II.1.22}

\begin{III.1.4 and III.1.5}
\frame{\frametitle{II.1.4 and II.1.5}

}
\end{III.1.4 and III.1.5}

\begin{III.1.13}
\frame{\frametitle{II.1.13}
