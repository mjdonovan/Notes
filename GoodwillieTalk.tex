% !TEX root = z_output/GoodwillieTalk.tex
...
\frame{\frametitle{Partial derivatives --- $1^\textup{st}$ order} 
\tallstrut Given $f:\R^n\to\R^n$, let's find $\Delta_uf(x_{\emptyset })$:\\\pause
...
\path (0,2.2) node[shape=rectangle] {\underline{$\scrx$}};
\pause
...
\path[->,very thick,densely dotted] (0) edge node[above,shape=rectangle] {$a_{(f\scrx)}$} (1);

\path (0) node[shape=rectangle,left] {$f(x_{\emptyset })$};
\path (1) node[shape=rectangle,right] {$f(x_{1})$};

\path (4.5,2.2) node[shape=rectangle] {\underline{$f\scrx$}};
\end{tikzpicture}\\
\tallstrut \uncover<4->{The dotted vector $a_{{(f\scrx)}}=\Delta_uf(x_\emptyset )$ approximates $\partial_uf(x_{\emptyset })$.}
\uncover<5->{
\begin{block}{A local Lipschitz condition:}\tallstrut If $|a_{(f\scrx)}|\leq L\cdot|u|$ for all \emph{short} $u$, then:\vspace{-3.73mm}
\begin{center}
\tallstrut $|\partial_u f(x_\emptyset )|\leq L$ for all unit vectors $u$.
\end{center}   \end{block} }
}
%\frame{\frametitle{Partial derivatives --- $1^\textup{st}$ order} 
%\tallstrut Have: $f:\R^n\to\R^n$. Seek: $\Delta_uf(x_{\emptyset })$\\\pause
%\vspace{.5cm}
%\begin{tikzpicture}[scale=1.2]
%
%\path (0.3,0) node[name=2] {$\phantom{\bullet} $};
...
\section{Partial Derivatives 2} 
\frame{\frametitle{Partial derivatives --- $2^\textup{nd}$ order} 
\tallstrut Given $f:\R^n\to\R^n$, let's find $\Delta_v\Delta_uf(x_{\emptyset })$:\\\pause
\begin{tikzpicture}[scale=1.2]
...
\uncover<5->{\path[-,color=yellow,line width=3pt] (2) edge (12);}
\path[->] (0) edge node[above,font=\footnotesize,shape=rectangle]{$\Delta_uf(x_\emptyset )$} (1);
\path[->] (0) edge (2);
\path[->] (1) edge (12);

\path[->] (2) edge node[above,font=\footnotesize,shape=rectangle]{$\Delta_uf(x_2 )$} (12);

\pause

\path let \p1 = (2), \p2 = (1), \p3=(12) in node[name=pull]  at (\x1+\x2-\x3,\y1+\y2-\y3) {$\circ$};
\uncover<5->{\path[-,color=yellow,line width=3pt] (pull) edge (1);}
\path[->] (pull) edge (1);
\path[->] (pull) edge (2);
\path[->] (4,-.21) node[shape=rectangle] {$h_0(f\scrx)$} edge[color=gray] (pull);
\pause
\path[->,very thick,densely dotted] (0) edge node[name=midedge]{} (pull);
\path[->, shorten >=-2mm] (3.6,.21) node {$a_{(f\scrx)}$} edge [color=gray] (midedge);


%\path (4.5,2.2) node[shape=rectangle] {\underline{$f\scrx$}};
\end{tikzpicture}\\
\tallstrut \uncover<7->{The vector $a_{{(f\scrx)}}=-\Delta_v\Delta_uf(x_{\emptyset })$ approximates $- \partial_v \partial_uf(x_{\emptyset })$.}\\

\uncover<8->{
\begin{block}{A 2-fold local Lipschitz condition:}\tallstrut If $|a_{(f\scrx)}|\leq L\cdot|u|\cdot|v|$ for all \emph{short} $u,v$, then:\vspace{-3.73mm}
\begin{center}
\tallstrut $|\partial_v\partial_u f(x_\emptyset )|\leq L$ for all unit vectors $u,v$.
\end{center}\end{block}}

%\tallstrut \uncover<1->{$|\partial_v\partial_u f(x_\emptyset )|\leq L\cdot|u|\cdot|v|$ for all unit vectors $u,v$.}\\
}

%\frame{\frametitle{Partial derivatives --- $2^\textup{nd}$ order} 
%\tallstrut Have: $f:\R^n\to\R^n$. Seek: $\Delta_v\Delta_uf(x_{\emptyset })$\\\pause
%\vspace{.5cm}
%\begin{tikzpicture}[scale=1.2]
%
%\path (1.5-1.5,1)+(7-1+.7,1.8-.3) node[name=3] {$\phantom{\bullet} $};
...
\section{Partial Derivatives 3} 
\frame{\frametitle{Partial derivatives --- $3^\textup{rd}$ order} 
\tallstrut %Have: $f:\R^n\to\R^n$. Seek: $\Delta_w\Delta_v\Delta_uf(x_{\emptyset })$
Given $f:\R^n\to\R^n$, let's find $\Delta_w\Delta_v\Delta_uf(x_{\emptyset })$:\\\pause
\begin{tikzpicture}[scale=1.2]
\path (0.3,0) node[name=2] {$\bullet $};
\path (0,1) node[name=0d] {$\bullet $};
\path (0d) node[shape=rectangle,left] {\makebox[0cm][r]{$x_{\emptyset }$}};
...
\path (7+1.3+.2+.3-1.5,1-.5-.1) node[name=1] {$\bullet $};
\path (7+0.3-1.5,-0.42) node[name=2] {$\bullet $};
\path (1.5-1.5+.2,1)+(7-1+.7,1.8-.3) node[name=3] {$\bullet $};
...
\path[->] (2) edge (23);
\path[->] (12) edge (123);
\alt<4>{}{\path[->, shorten >=.25cm, shorten <=.25cm] (0) edge [-,line width=3pt,draw=white] (1);}
\uncover<4>{\path[->] (0) edge (1);}
\only<5>{\path[->] (0) edge (1);}
\only<6>{\path[->] (0) edge (1);}
...
\path[->,color=green] (3) edge (pull3);
\path[->,color=green] (0) edge (pull);
\pause
...
\path[->] (4,-.21) node[shape=rectangle] {$h_0(f\scrx)$} edge[color=gray] (pullpull);
...
\pause
\end{tikzpicture}\\
\tallstrut \uncover<8->{The vector $a_{{(f\scrx)}}=\Delta_w\Delta_v\Delta_uf(x_{\emptyset })$ approximates $\partial_w \partial_v \partial_uf(x_{\emptyset })$.}
\uncover<9->{
\begin{block}{A 3-fold local Lipschitz condition:}\tallstrut If $|a_{(f\scrx)}|\leq L\cdot|u|\cdot|v|\cdot|w|$ for all \emph{short} $u,v,w$, then:\vspace{-3.73mm}
\begin{center}
\tallstrut $|\partial_w\partial_v\partial_u f(x_\emptyset )|\leq L$ for all unit vectors $u,v,w$.
\end{center}\end{block}}
}
...
%\path (4.5,2.2) node[shape=rectangle] {\underline{$f\scrx$}};
%\end{tikzpicture}\\
%\tallstrut The vector $a_{{(f\scrx)}}=\Delta_w\Delta_v\Delta_uf(x_{\emptyset })$ approximates $\partial_w \partial_v \partial_uf(x_{\emptyset })$.
%\uncover<4->{
%\begin{block}{A 3-fold local Lipschitz condition:}\tallstrut If $|a_{(f\scrx)}|\leq L\cdot|u|\cdot|v|\cdot|w|$ for all \emph{short} $u,v,w$, then:\vspace{-3.73mm}
%\begin{center}
%\tallstrut $|\partial_w\partial_v\partial_u f(x_\emptyset )|\leq L$ for all unit vectors $u,v,w$.
...
\item Implies that all $(n+1)^\textup{st}$ derivatives are bounded by $e^c$.\pause
\item A local Lipschitz condition with \textbf{globally defined} constants.\pause
\end{itemise}
\begin{center}{\color{blue}This will define ``stably $n$-excisive functor''}\end{center}
...
\sbox{\saveboxXLZ}{
\fbox{\txt{For some $\alpha>0$:\rule[-.5em]{0pt}{.1em}\\$\left|\frac{d^{n+1}f}{dx^{n+1}}(x)\right|\leq \alpha^{n+2}{(n+1)}!$\\whenever $|x-y|\leq \alpha^{-1}$}}
...
%%% BEGIN: \usebox{\saveboxVSL}
\newsavebox{\saveboxVSL}
\sbox{\saveboxVSL}{
\fbox{\txt{\underline{All} $(n+1)^\textup{st}$ derivatives\\$\leq e^{n\rho-q}$}}
}%%% END:  \usebox{\saveboxVSL}

\section{Analyticity of functions} 
\frame{\frametitle{Iterated Lipschitz conditions and analyticity}
For $f:\R\to\R$ a $C^\infty$ function, given $y\in \R$:
...
\uncover<5->{\usebox{\saveboxVSL}}
%\only<5->{\ar@{=>}[u]}%_-{\textup{($\alpha\approx e^{\rho+1}$)}}
\only<6->{\ar@<-2ex>@{=>}[u]_-{\makebox[0cm][l]{\,\scriptsize$\alpha=\max\left\{e^\rho,e^{-q}\right\}$}}}
\only<6->{\ar@<-2ex>@{=>}[u];[]|-{\textup{\huge\smash{$\raisebox{-1.62mm}{$\times$}$}}}}
%\only<5->{\ar@{=>}[dll]}
\\
\uncover<7->{{\textup{\color{blue}%Will define
``$\rho$-analytic functor''}}}
\only<7->{\ar[rr]}&&\uncover<4->{\usebox{\saveboxZVB}}
\only<5->{\ar@<-2ex>@{=>}[u]}
\only<5->{\ar@<-2ex>@{=>}[u];[]|-{\textup{\huge\smash{$\raisebox{-1.62mm}{$\times$}$}}}}
}\]
}
...
\section{Derivative diagrams for functors}
\begin{functors appear}
\frame{\frametitle{{Derivative diagrams for functors} } 
\tallstrut Start with $F:\calc\to\cald$, and strongly cocartesian $n$-cube $\scrx$ in $\calc$ (equivalently a pushout cube, built from maps $\scrx_\emptyset \to\scrx_i$):\pause\\ %$\partial_w\partial_v\partial_uf(x_{\emptyset })$\\
\vspace{.5cm}
\begin{tikzpicture}[scale=1.2]
\path (0.3,0) node[name=2] {$\bullet $};
\path (1.3,0) node[name=12] {$\bullet $};
...
\path (7-1-1.5,1.5) node[name=0] {$\bullet $};
\path (0) node[left,shape=rectangle] {$F\scrx_\emptyset $};
...
\path[->] (4,-.21) node[shape=rectangle] {$h_0(F\scrx)$} edge[color=gray] (pullpull);
\path[->, shorten >=-2mm] (3.6,.21) node {$a_\scrx$} edge[color=gray] (midedge);
...
\section{agreement to nth order}
...
For maps of cubes, variously cartesian as marked:
\[\underbrace{\xymatrix@R=.4cm@C=.6cm{
\framebox[9mm][c]{\TF \tiny$k$\ST} \ar[rr] &&
\framebox[9mm][c]{\tiny$k$\ST}}}_{k}\qquad \raisebox{-1mm}{\textup{and}}\qquad \underbrace{\xymatrix@R=.4cm@C=.6cm{
\framebox[9mm][c]{\tiny$k$\ST} \ar[rr] &&
\framebox[9mm][c]{\tiny$k+1$\ST}}}_{\TF k}\]
\end{block}
\pause
\begin{center}
$\xymatrix@R=-8mm{
&
\fbox{\usebox{\saveboxQMZ}}{\alt<2>{\ar[dl]^-{k}}{\ar[dl]_-{\TF k}}}&%r1c1
\fbox{\usebox{\saveboxDRJ}}\ar[l]
\\%r1c2
\fbox{\usebox{\saveboxLKR}}\\
&\fbox{\usebox{\saveboxIHI}}\alt<2>{\ar[uu]^-{\geq k}}{\ar[uu]_-{\geq k+1}}\alt<2>{\ar[ul]_-{\TF k}}{\ar[ul]^-{k}}
&%r2c1
\fbox{\usebox{\saveboxFEF}}\alt<2>{\ar[uu]^-{k}}{\ar[uu]_-{k+1}}
\ar[l]
}$\end{center}\pause
}
}
...
\section{Proposition II.1.20}
\begin{II.1.20}
\frame{
\begin{block}{Proposition II.1.20}
For a cube of cubes, variously cartesian as marked:
\[ 
...
\end{block}\pause\vspace{-4mm}
\[\underbrace{\xymatrix@R=.4cm@C=.6cm{
\framebox[9mm][c]{\tiny$k$\ST} \ar[rr] &&
\framebox[9mm][c]{\tiny$k+1$\ST}}}_{\TF k}\quad \raisebox{-1mm}{\textup{induction,  and}}\quad 
\underbrace{\xymatrix@R=.4cm@C=.6cm{
\framebox[9mm][c]{\TF \tiny$k$\ST} \ar[rr] &&
\framebox[9mm][c]{\tiny$k$\ST}}}_{k}\]
}
\end{II.1.20}
\section{II.1.22}
\begin{II.1.22}
...
Given a $\calp_0(n+1)$-shaped diagram of $a$-cartesian cubes:
\[ 
\vcenter{\xymatrix@R=.4cm@C=.6cm{
&&
\framebox[9mm][c]{\small$a$\tiny\ST}   \ar'[d][dd]\ar[rd]\\&
\framebox[9mm][c]{\small$a$\tiny\ST} \ar[dd]\ar[rr] &&
\framebox[9mm][c]{\small$a$\tiny\ST} \ar[dd] \\
\framebox[9mm][c]{\small$a$\tiny\ST}  \ar'[r][rr]\ar[rd] &&
\framebox[9mm][c]{\small$a$\tiny\ST}  \ar[rd] \\&
\framebox[9mm][c]{\small$a$\tiny\ST} \ar[rr] &&
\framebox[9mm][c]{\small$a$\tiny\ST}
}}\]
the cube of pullbacks is $(a-n)$-cartesian.
\end{block}
\pause
The previous with $k=a-n$.
}
\end{II.1.22}
...
\end{enumerate}
%Investigate $\cartesianness{T_nF(\scrx)}$ via 
Have $\calp_0(n+1)$-shaped diagram of $(n+1)$-cubes: \pause (here $n=2$)
\[ 
...
Each (outer) cube is $a=(-c+\sum_{i=1}^{n+1}(k_i+1))$-cartesian.\\\pause\vspace{1mm}
Cube of pullbacks is $(n-a)=(-(c-1)+\sum_{i=1}^{n+1} k_i)$-cartesian.
%\[\min\left\{1-(n+1)+\left(-c+\sum_{i=1}^{n+1} (k_i+1)\right)\right\} =-(c-1)+\sum_{i=1}^{n+1} k_i.\]
\end{center}
}
...
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item[\textup{(2)}] Let $X\downarrow Y$ be $k$-connected with $k\geq\kappa$. \pause Must show:
\[\conn(t_n:F(X)\to T_nF(X))\geq -c+(n+1)k.\]\vspace{-2mm}\pause
\begin{itemize}\squishlist
\setlength{\parindent}{.25in}
\item Let $\scry$ be the $(n+1)$-cube $U\mapsto F(X\star_Y U)$.\pause
\item $FX\overset{t_n}{\to}T_nF(X)$ \textbf{is} $a_\scry$.\pause
\item $\conn(t_n:F(X)\to T_nF(X))=\cartesianness(\scry)$.\pause
\item The initial arrows are $k$-connected:\vspace{-1mm}
\[
\xymatrix@R=.2cm{
\scry_\emptyset \ar[r]
\ar@{=}[d]
...
\item $F$ has $E_n(c,\kappa)$ $\implies$ $\cartesianness(\scry)\geq(-c+\sum_{i=1}^{n+1}k)$.
\end{itemize}
\end{enumerate}
...
\item[\textup{(2)}] $p_nF:F\to P_nF$ satisfies
$O_n(c,\kappa)$.
\end{enumerate}
...
\begin{enumerate}\squishlist
\setlength{\parindent}{.25in}
\item[\textup{(2)}] Compositions preserve connectedness of maps, so they preserve $O_n(c,\kappa)$.%Follows by ultrametric inequality: $\ell(f\circ g)\leq \max(\ell(f),\ell(g))$.
\end{enumerate}
}
\end{III.1.5}
\section{III.1.13}
...
&\qquad&\uncover<5->{\text{(by $O_n(c,\kappa)$)}}\\
&\uncover<5->{=}
